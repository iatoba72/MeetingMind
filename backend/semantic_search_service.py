...
from vector_storage_service import (
    VectorStorageService, get_vector_storage_service,
    DocumentType, SearchMode, SearchResult, VectorDocument
)
...
class SemanticSearchService:
    ...
    async def analyze_query(self, query: str, context: SearchContextInfo = None) -> QueryAnalysis:
        """
    ...
    def _extract_keywords(self, query: str) -> List[str]:
        ...
        stop_words = {'the', 'a', 'an', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for', 'of', 'with', 'by', 'is', 'was', 'are', 'were', 'be', 'been', 'have', 'has', 'had', 'do', 'does', 'did', 'will', 'would', 'could', 'should', 'may', 'might', 'can', 'about', 'what', 'when', 'where', 'why', 'how', 'who'}
    ...
    def _extract_speaker_mentions(self, query: str) -> List[str]:
        ...
        for pattern in speaker_patterns:
            ...
            speakers.extend([match[0] if isinstance(match, tuple) else match for match in matches])
    ...
    def _determine_intent(self, query: str, query_type: QueryType, entities: List[str]) -> str:
    ...
    async def search(self, 
                    query: str, 
                    context: SearchContextInfo = None,
                    limit: int = 10) -> List[EnhancedSearchResult]:
        """
        ...
        if cache_key in self.search_cache:
            ...
            if time.time() - cached_results[0].base_result.document.updated_at.timestamp() < self.cache_ttl:
        ...
        try:
            ...
            self._update_search_stats(query, query_analysis.query_type, len(final_results), search_time)
            
            self.logger.info(f"Search '{query}' returned {len(final_results)} results in {search_time:.2f}s")
    ...
    def _prepare_search_parameters(self, 
                                 ...
                                 context: SearchContextInfo) -> Dict[str, Any]:
        """Prepare search parameters based on query analysis and context"""
        
        # Default parameters
        params = {
            'document_types': [DocumentType.TRANSCRIPT_SEGMENT, DocumentType.MEETING_SUMMARY, DocumentType.INSIGHT],
        ...
        if query_analysis.query_type == QueryType.ACTION_ITEM:
            ...
            params['document_types'] = [DocumentType.INSIGHT, DocumentType.MEETING_SUMMARY]
            ...
            if query_analysis.speaker_mentions:
                params['metadata_filter']['speaker'] = query_analysis.speaker_mentions[0]
            ...
            params['document_types'] = [DocumentType.TOPIC, DocumentType.MEETING_SUMMARY]
    ...
    async def _enhance_search_result(self, 
                                   base_result: SearchResult,
                                   query: str,
                                   query_analysis: QueryAnalysis,
                                   context: SearchContextInfo) -> EnhancedSearchResult:
        """Enhance search result with additional context and explanations"""
        
        # Calculate context score
        context_score = await self._calculate_context_score(base_result, query_analysis, context)
        ...
        # Generate highlights
        highlights = self._generate_highlights(base_result.document.content, query_analysis.keywords)
    ...
    async def _calculate_context_score(self, 
                                     result: SearchResult,
                                     query_analysis: QueryAnalysis,
                                     context: SearchContextInfo) -> float:
        """Calculate additional context-based relevance score"""
        
        score = 0.0
        
        # Query type alignment
        if query_analysis.query_type == QueryType.ACTION_ITEM and result.document.document_type == DocumentType.ACTION_ITEM:
            score += 0.3
        elif query_analysis.query_type == QueryType.DECISION and 'decision' in result.document.metadata.get('insight_type', ''):
            score += 0.3
        
        # Speaker match
        if query_analysis.speaker_mentions:
            doc_speaker = result.document.metadata.get('speaker', '')
            if any(speaker.lower() in doc_speaker.lower() for speaker in query_analysis.speaker_mentions):
                score += 0.2
    ...
    def _generate_relevance_explanation(self, 
        ...
        # Keyword matches
        keyword_matches = [
            keyword for keyword in query_analysis.keywords
            if keyword.lower() in result.document.content.lower()
        ]
        if keyword_matches:
        ...
        if query_analysis.speaker_mentions:
            doc_speaker = result.document.metadata.get('speaker', '')
            matching_speakers = [
                speaker for speaker in query_analysis.speaker_mentions
    ...
    async def _find_related_documents(self, document_id: str, limit: int = 3) -> List[str]:
    ...
    def _update_search_stats(self, query: str, query_type: QueryType, result_count: int, search_time: float):
        ...
        total_time = self.search_stats['avg_response_time'] * (self.search_stats['total_searches'] - 1)
        self.search_stats['avg_response_time'] = (total_time + search_time) / self.search_stats['total_searches']
    ...
    async def get_search_suggestions(self, partial_query: str, limit: int = 5) -> List[SearchSuggestion]:
        ...
        common_suggestions = [
            ("action items from last week", "Find recent action items", QueryType.ACTION_ITEM),
            ("decisions made today", "Find decisions from today", QueryType.DECISION),
            ("what did John say about budget", "Find speaker comments on topic", QueryType.SPEAKER),
            ("meeting about project planning", "Find meetings by topic", QueryType.TOPIC),
        ...
        for popular_query, count in self.search_stats['popular_queries'].most_common(10):
            if partial_lower in popular_query.lower() and popular_query not in [s.suggestion for s in suggestions]:
                suggestions.append(SearchSuggestion(
    ...
    async def get_search_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'zero_result_queries': self.search_stats['zero_result_queries'][-10:],  # Last 10
            'cache_size': len(self.search_cache),
            'cache_hit_rate': len(self.search_cache) / max(1, self.search_stats['total_searches'])
        }
