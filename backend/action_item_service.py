...
class ActionItemService:
    ...
    async def extract_action_items_from_text(
        self, text: str, meeting_id: str, participants: List[Participant] = None
    ) -> List[ActionItemExtractionResult]:
        """
        ...
        try:
            extracted_items = []
            participant_names = (
                [p.display_name.lower() for p in participants] if participants else []
            )
    ...
    def _process_pattern_match(
        self, match, pattern_config: Dict, sentence: str, participant_names: List[str]
    ) -> Optional[ActionItemExtractionResult]:
    ...
    def _deduplicate_action_items(
        self, items: List[ActionItemExtractionResult]
    ) -> List[ActionItemExtractionResult]:
    ...
    def _is_similar_action_item(
        self, item1: ActionItemExtractionResult, item2: ActionItemExtractionResult
    ) -> bool:
    ...
    async def update_action_item_progress(
        ...
        update_text: str,
        updated_by: str,
        new_status: Optional[ActionItemStatus] = None,
    ) -> ActionItem:
        ...
        try:
            ...
            if (
                ...
                and action_item.status == ActionItemStatus.IN_PROGRESS
            ):
                action_item.status = ActionItemStatus.ASSIGNED
            elif (
                progress_percentage > 0
                and action_item.status == ActionItemStatus.ASSIGNED
            ):
            ...
            due_soon = (
                ...
                .filter(
                    and_(
                        ...
                        ActionItem.status.in_(
                            [ActionItemStatus.ASSIGNED, ActionItemStatus.IN_PROGRESS]
                        ),
                        or_(
                            ActionItem.last_reminder_sent.is_(None),
                            ActionItem.last_reminder_sent
    ...
    async def _send_reminder_notification(self, action_item: ActionItem):
        ...
        try:
            ...
            time_text = (
                "today" if days_until_due == 0 else f"in {days_until_due} day(s)"
            )
        ...
        try:
            ...
            in_progress_items = len(
                [
                    item
                    for item in action_items
                    if item.status == ActionItemStatus.IN_PROGRESS
                ]
            )
            ...
            return {
                ...
                "status_distribution": {
                    status.value: len(
                        [item for item in action_items if item.status == status]
                    )
