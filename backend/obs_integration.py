...
class OBSWebSocketClient:
    ...
    async def _handle_request_response(self, data: Dict[str, Any]):
        ...
        if request_id in self.pending_requests:
            ...
            if data.get("requestStatus", {}).get("result"):
                ...
                error_msg = data.get("requestStatus", {}).get("comment", "Unknown error")
    ...
    async def _initialize_obs_data(self):
        ...
        try:
            # Get scenes
            scenes_response = await self._send_request(OBSRequestType.GET_SCENE_LIST.value)
            ...
            # Get current scene
            current_scene_response = await self._send_request(OBSRequestType.GET_CURRENT_SCENE.value)
            self.current_scene = current_scene_response.get("currentProgramSceneName")
            
            # Get sources
            sources_response = await self._send_request(OBSRequestType.GET_SOURCE_LIST.value)
            ...
            logger.info(f"Initialized OBS data: {len(self.scenes)} scenes, {len(self.sources)} sources")
    ...
    async def set_scene(self, scene_name: str) -> bool:
        ...
        try:
            await self._send_request(
                OBSRequestType.SET_CURRENT_SCENE.value,
    ...
    async def create_source(self, source_name: str, source_kind: str, source_settings: Dict[str, Any] = None) -> bool:
        """Create a new source"""
        try:
            request_data = {
    ...
    async def remove_source(self, source_name: str) -> bool:
        ...
        try:
            await self._send_request(
                OBSRequestType.REMOVE_SOURCE.value,
                {"sourceName": source_name}
            )
            await self._initialize_obs_data()  # Refresh sources list
            return True
    ...
    async def toggle_source_mute(self, source_name: str) -> bool:
        ...
        try:
            await self._send_request(
                OBSRequestType.TOGGLE_INPUT_MUTE.value,
...
class OBSAutomationManager:
    ...
    async def handle_meeting_event(self, event_type: str, meeting_id: str, event_data: Dict[str, Any] = None):
    ...
    async def _execute_action(self, action: Dict[str, Any], meeting_id: str, event_data: Dict[str, Any] = None):
...
class OBSStatsMonitor:
    ...
    def get_stats_history(self, minutes: int = 60) -> List[OBSStats]:
        ...
        return [
            stats for stats in self.obs_client.stats_history
...
class OBSSceneSwitchRequest(BaseModel):
    ...
    meeting_id: Optional[str] = Field(default=None, description="Meeting ID for context")
...
class OBSSourceControlRequest(BaseModel):
    ...
    value: Optional[float] = Field(default=None, description="Value for volume action (0.0-1.0)")
