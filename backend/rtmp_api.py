...
from rtmp_server import (
    get_rtmp_server, start_rtmp_server, stop_rtmp_server,
    RTMPServerConfig, RTMPStreamInfo, RTMPServerState
)
...
async def disconnect_stream(stream_key: str):
    ...
    try:
        ...
        return RTMPServerResponse(
            success=True,
            message=f"Stream {stream_key} disconnected successfully"
        )
