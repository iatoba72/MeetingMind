...
class StreamRecorder:
    ...
    def __init__(self, source_id: str, config: RecordingConfig = None):
        ...
        ) if self.config.split_on_silence else None
    ...
    async def start_recording(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Recording started for source {self.source_id}, session {self.session_id}")
            return True
    ...
    async def stop_recording(self) -> bool:
        ...
        try:
            ...
            if self.session:
                ...
                self.session.duration_seconds = (self.session.end_time - self.session.start_time).total_seconds() - self.total_paused_duration
    ...
    def add_audio_chunk(self, chunk: AudioChunk) -> bool:
        ...
        try:
            ...
            if self.session:
                ...
                if self.start_time:
                    self.session.duration_seconds = (datetime.now() - self.start_time).total_seconds() - self.total_paused_duration
    ...
    async def _audio_writer_loop(self):
        ...
        try:
            while self.state in [RecordingState.RECORDING, RecordingState.PAUSED]:
                ...
                chunks = self.buffer.get_chunks(count=10)  # Process up to 10 chunks at a time
    ...
    async def _initialize_output_file(self):
        ...
        try:
            ...
            output_path = os.path.join(self.config.output_directory, self.session.output_file)
    ...
    async def _convert_to_target_format(self):
        ...
        try:
            ...
            if process.returncode == 0:
                logger.info(f"Converted recording to {self.config.format.value}: {output_path}")
    ...
    async def _post_process_recording(self):
        ...
        try:
            ...
            input_file = os.path.join(self.config.output_directory, self.session.output_file)
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        if self.stats['total_chunks_received'] > 0:
            stats['average_chunk_size'] = self.stats['total_bytes_recorded'] / self.stats['total_chunks_received']
...
class RecordingManager:
    ...
    async def create_recorder(self, source_id: str, config: RecordingConfig = None) -> StreamRecorder:
    ...
    def get_manager_statistics(self) -> Dict[str, Any]:
        ...
        for source_id, recorder in self.recorders.items():
            ...
            stats["total_bytes_recorded"] += recorder_stats.get("total_bytes_recorded", 0)
...
async def create_recorder(source_id: str, config: RecordingConfig = None) -> StreamRecorder:
