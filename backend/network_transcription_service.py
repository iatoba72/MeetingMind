...
from stream_vad import (
    StreamVAD, StreamVADConfig, VADResult, AudioQualityMetrics,
    get_stream_vad, calculate_audio_quality_metrics
)
...
class StreamTranscriptionBuffer:
    ...
    def add_audio_chunk(self, chunk: NetworkAudioChunk) -> bool:
        ...
        with self._lock:
            ...
            if self.last_sequence >= 0 and chunk.sequence_number > self.last_sequence + 1:
                ...
                if gap_duration < 0.5:  # Fill gaps < 500ms
                    ...
                    for i in range(silence_samples):
                        self.timestamp_buffer.append(chunk.timestamp - gap_duration + (i / self.sample_rate))
    ...
    def get_transcription_chunk(self) -> Optional[Tuple[np.ndarray, float, float, AudioQualityMetrics]]:
        ...
        with self._lock:
            ...
            avg_quality = sum(self.quality_history) / len(self.quality_history) if self.quality_history else 0.5
            avg_packet_loss = sum(self.packet_loss_history) / len(self.packet_loss_history) if self.packet_loss_history else 0.0
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                "buffer_duration_s": len(self.audio_buffer) / self.sample_rate if self.audio_buffer else 0,
                "buffer_fill_percent": (len(self.audio_buffer) / (self.config.max_buffer_duration_s * self.sample_rate)) * 100,
                "resync_count": self.resync_count,
                "average_quality": sum(self.quality_history) / len(self.quality_history) if self.quality_history else 0,
                "average_packet_loss": sum(self.packet_loss_history) / len(self.packet_loss_history) if self.packet_loss_history else 0,
                "last_transcription_age_s": time.time() - self.last_transcription_time if self.last_transcription_time > 0 else 0
            }
...
class ModelManager:
    ...
    def get_model_for_quality(self, quality: float, latency_mode: str = "balanced") -> Tuple[WhisperModel, str]:
...
class NetworkTranscriptionService:
    ...
    def create_stream(self, stream_id: str, config: Optional[StreamTranscriptionConfig] = None) -> bool:
        ...
        self.stream_buffers[stream_id] = StreamTranscriptionBuffer(stream_id, stream_config)
    ...
    async def _transcribe_chunk(self, stream_id: str, audio: np.ndarray, 
                              start_time: float, end_time: float,
                              quality_metrics: AudioQualityMetrics) -> Optional[TranscriptionSegment]:
        ...
        try:
            ...
            if processing_time > config.max_processing_latency_ms:
                logger.warning(f"Transcription latency exceeded: {processing_time:.1f}ms")
            ...
            if segments:
                ...
                if combined_text.strip():
                    ...
                    segment = TranscriptionSegment(
                        ...
                        end_time=end_time,
                        confidence=segments[0].avg_logprob if hasattr(segments[0], 'avg_logprob') else 0.8,
    ...
    def _run_whisper_transcription(self, model: WhisperModel, audio: np.ndarray, 
                                 config: StreamTranscriptionConfig) -> Tuple[List, Any]:
    ...
    def _update_statistics(self, segment: TranscriptionSegment, model_size: str):
        ...
        stats["average_latency_ms"] = stats["total_processing_time"] / stats["total_segments"]
...
async def get_network_transcription_service(config: Optional[StreamTranscriptionConfig] = None) -> NetworkTranscriptionService:
