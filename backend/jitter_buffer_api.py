...
from jitter_buffer import (
    get_jitter_buffer_manager,
    create_jitter_buffer,
    get_jitter_buffer,
    JitterBufferConfig,
    AudioPacket,
    BufferState,
...
async def create_jitter_buffer_endpoint(
    source_id: str, config: Optional[JitterBufferConfigRequest] = None
):
...
async def remove_jitter_buffer(source_id: str):
    ...
    try:
        ...
        if success:
            return JitterBufferResponse(
                success=True, message=f"Jitter buffer removed for source {source_id}"
            )
        else:
            raise HTTPException(
                status_code=404,
                detail=f"Jitter buffer for source {source_id} not found",
            )
...
async def get_jitter_buffer_info(source_id: str):
    ...
    try:
        ...
        return JitterBufferResponse(
            ...
            data={"buffer_status": status, "statistics": statistics},
        )
...
async def add_packet_to_buffer(source_id: str, packet_data: AudioPacketRequest):
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=400, detail="Failed to add packet to buffer"
            )
...
async def jitter_buffer_health_check():
    ...
    try:
        ...
        health_data = {
            ...
            "health_percentage": (
                (healthy_buffers / total_buffers * 100) if total_buffers > 0 else 100
            ),
        }
...
async def get_jitter_buffer_performance():
    ...
    try:
        ...
        overall_loss_rate = (
            (total_lost_packets / total_packets) if total_packets > 0 else 0
        )
        overall_late_rate = (
            (total_late_packets / total_packets) if total_packets > 0 else 0
        )
        overall_concealment_rate = (
            (total_concealed_packets / total_packets) if total_packets > 0 else 0
        )
        overall_avg_jitter = (
            sum(avg_jitter_values) / len(avg_jitter_values) if avg_jitter_values else 0
        )
        overall_avg_buffer_level = (
            sum(avg_buffer_levels) / len(avg_buffer_levels) if avg_buffer_levels else 0
        )
        ...
        performance_data = {
            ...
            "active_buffers": len(
                [
                    b
                    for b in stats["buffers"].values()
                    if b.get("buffer_status", {}).get("state") in ["playing", "filling"]
                ]
            ),
            "buffer_efficiency": (1 - overall_loss_rate)
            * 100,  # Percentage of successful packet delivery
        }
...
async def reset_all_jitter_buffers():
    ...
    try:
        ...
        return JitterBufferResponse(
            success=True, message="All jitter buffers have been reset"
        )
