...
import numpy as np
from jiwer import wer, cer, mer  # Word Error Rate, Character Error Rate, Match Error Rate
...
from cloud_transcription_service import TranscriptionJob, TranscriptionAttempt, TranscriptionProvider
...
class TranscriptionAccuracyAnalyzer:
    ...
    async def analyze_transcription_job(
        ...
        for attempt in job.attempts:
            ...
            if attempt.cost > 0:
                cost_effectiveness[attempt.provider] = metrics.word_accuracy / attempt.cost
            else:
                cost_effectiveness[attempt.provider] = metrics.word_accuracy  # Free service
        ...
        # Detailed analysis
        detailed_analysis = await self._perform_detailed_analysis(job, provider_results, reference_text)
    ...
    async def calculate_accuracy_metrics(
        ...
        if reference:
            ...
            semantic_similarity = await self._calculate_semantic_similarity(reference_clean, hypothesis_clean)
            
            # Detailed error analysis
            substitutions, insertions, deletions, total_words = self._analyze_errors(reference_clean, hypothesis_clean)
            
            # Word and sentence accuracy
            word_accuracy = 1.0 - word_error_rate
            sentence_accuracy = self._calculate_sentence_accuracy(reference_clean, hypothesis_clean)
            
            # Timestamp accuracy
            timestamp_accuracy = self._calculate_timestamp_accuracy(reference, result) if result else 0.0
        ...
        if result and reference:
            speaker_accuracy, speaker_confusion_matrix = self._calculate_speaker_accuracy(result, reference)
    ...
    async def _calculate_semantic_similarity(self, reference: str, hypothesis: str) -> float:
    ...
    def _analyze_errors(self, reference: str, hypothesis: str) -> Tuple[int, int, int, int]:
    ...
    def _calculate_timestamp_accuracy(self, reference: str, result: TranscriptionResult) -> float:
        ...
        return min(1.0, valid_timestamps / total_segments) if total_segments > 0 else 0.0
    
    def _calculate_speaker_accuracy(self, result: TranscriptionResult, reference: str) -> Tuple[float, Dict[str, Dict[str, int]]]:
    ...
    def _calculate_provider_rankings(self, provider_results: Dict[TranscriptionProvider, AccuracyMetrics]) -> List[Tuple[TranscriptionProvider, float]]:
    ...
    async def _perform_detailed_analysis(
        ...
        for attempt in job.attempts:
            if attempt.success:
                analysis["processing_times"][attempt.provider.value] = attempt.processing_time
        
        # Error patterns analysis
        if reference_text:
            analysis["error_patterns"] = await self._analyze_error_patterns(job, reference_text)
        ...
        if provider_results:
            ...
            analysis["quality_distribution"] = {
                ...
                "std_accuracy": statistics.stdev(word_accuracies) if len(word_accuracies) > 1 else 0,
                "min_accuracy": min(word_accuracies),
                "max_accuracy": max(word_accuracies)
            }
        
        # Generate recommendations
        analysis["recommendations"] = self._generate_recommendations(provider_results, job)
        
        return analysis
    
    async def _analyze_error_patterns(self, job: TranscriptionJob, reference_text: str) -> Dict[str, Any]:
            ...
            for tag, i1, i2, j1, j2 in opcodes:
                if tag == 'replace':
                    for k in range(min(i2-i1, j2-j1)):
                        ...
                        error_patterns["common_substitutions"][(ref_word, hyp_word)] += 1
    ...
    def _generate_recommendations(
        ...
        recommendations.append(f"Best accuracy: {best_provider[0].value} ({best_provider[1].word_accuracy:.2%})")
        
        # Cost effectiveness
        cost_effective = None
        best_ratio = 0
        for attempt in job.attempts:
            if attempt.success and attempt.cost > 0 and attempt.provider in provider_results:
        ...
        avg_accuracy = statistics.mean([m.word_accuracy for m in provider_results.values()])
        if avg_accuracy < 0.8:
            recommendations.append("Consider using higher-quality models or improving audio quality")
        
        # Speed recommendations
        if job.audio_duration > 0:
            fast_providers = [
                p.value for p, m in provider_results.items() 
                if m.processing_speed < 1.0  # Real-time capable
            ]
            if fast_providers:
                recommendations.append(f"For real-time use: {', '.join(fast_providers)}")
        
        return recommendations
    
    async def run_benchmark_test(self, providers: List[TranscriptionProvider]) -> Dict[str, ComparisonResult]:
        ...
        for test_name, reference_text in self.benchmark_texts.items():
            try:
                ...
                job = await cloud_transcription_service.transcribe_with_fallback(
                    ...
                    audio_duration=len(reference_text.split()) * 0.5,  # Approximate duration
                    preferred_providers=providers,
                    comparison_mode=True
                )
                
                # Analyze results
                comparison_result = await self.analyze_transcription_job(job, reference_text)
