...
class SettingsDiff:
    ...
    modified: Dict[str, Tuple[Any, Any]] = field(default_factory=dict)  # key -> (old, new)
...
class SettingsVersionManager:
    ...
    def __init__(self, 
        ...
        self.version_chains: Dict[str, List[str]] = {}  # scope:scope_id -> [version_ids]
    ...
    async def create_version(self,
        ...
        try:
            ...
            current_settings = await self.settings_manager.get_all_values(scope, scope_id)
            ...
            if scope_key in self.version_chains and self.version_chains[scope_key]:
                ...
                if latest_version.settings_hash == settings_hash:
                    self.logger.info(f"No changes detected for {scope_key}, skipping version creation")
            ...
            added_keys, modified_keys, removed_keys = self._calculate_changes(
                previous_version.settings if previous_version else {},
                current_settings
            )
            ...
            self.logger.info(f"Created version {version_number} for {scope_key}: {description}")
            return version_id
    ...
    def _calculate_changes(self, 
                          ...
                          new_settings: Dict[str, Any]) -> Tuple[List[str], List[str], List[str]]:
    ...
    async def get_versions(self, 
                          ...
                          scope_id: str,
                          limit: int = 50,
                          offset: int = 0) -> List[SettingsVersion]:
    ...
    async def get_latest_version(self, scope: SettingsScope, scope_id: str) -> Optional[SettingsVersion]:
    ...
    async def compare_versions(self, 
                              from_version_id: str,
                              to_version_id: str) -> Optional[SettingsDiff]:
    ...
    async def create_branch(self,
        ...
        self.logger.info(f"Created branch '{branch_name}' from version {base_version_id}")
    ...
    async def _remove_version(self, version_id: str):
        ...
        if scope_key in self.version_chains:
            self.version_chains[scope_key] = [
                vid for vid in self.version_chains[scope_key] 
                if vid != version_id
            ]
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        avg_versions = total_versions / scopes_with_versions if scopes_with_versions > 0 else 0
...
def initialize_version_manager(settings_manager: SettingsManager, **kwargs) -> SettingsVersionManager:
