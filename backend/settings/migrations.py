...
class BaseMigration(ABC):
    ...
    def __init__(self, migration_id: str, from_version: str, to_version: str, description: str):
...
class MigrationManager:
    ...
    def __init__(self, settings_manager: SettingsManager):
        ...
        self.migration_history: Dict[str, List[str]] = {}  # scope:scope_id -> [migration_ids]
    ...
    async def migrate_settings(self, 
                              ...
                              scope_id: str,
                              target_version: str,
                              current_version: Optional[str] = None) -> List[MigrationResult]:
        ...
        if not migration_path:
            self.logger.info(f"No migrations needed from {current_version} to {target_version}")
            return []
        ...
        for migration in migration_path:
            ...
            if not result.success:
                self.logger.error(f"Migration {migration.migration_id} failed, stopping")
                break
    ...
    async def _apply_migration(self, 
                              ...
                              scope: SettingsScope,
                              scope_id: str,
                              migrating_up: bool) -> MigrationResult:
        ...
        try:
            ...
            self.logger.info(f"Applied migration {migration.migration_id} ({'up' if migrating_up else 'down'})")
    ...
    async def _save_migrated_settings(self, 
                                     settings: Dict[str, Any],
                                     scope: SettingsScope,
                                     scope_id: str):
    ...
    async def _set_current_version(self, scope: SettingsScope, scope_id: str, version: str):
    ...
    async def _update_migration_history(self, 
                                       scope: SettingsScope,
                                       scope_id: str,
    ...
    async def get_pending_migrations(self, 
                                    scope: SettingsScope,
                                    scope_id: str,
                                    target_version: str) -> List[BaseMigration]:
    ...
    def _get_latest_version(self) -> str:
        ...
        return max(self.migrations, key=lambda m: semver.VersionInfo.parse(m.to_version)).to_version
    
    async def rollback_migration(self, 
                                migration_id: str,
                                scope: SettingsScope,
                                scope_id: str) -> MigrationResult:
        ...
        # Apply rollback
        result = await self._apply_migration(migration, settings, scope, scope_id, False)
        
        if result.success:
            await self._save_migrated_settings(result.migrated_settings, scope, scope_id)
    ...
    async def dry_run_migration(self, 
                               scope: SettingsScope,
                               scope_id: str,
                               target_version: str) -> List[MigrationResult]:
