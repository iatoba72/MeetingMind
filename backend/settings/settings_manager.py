...
class SettingsFileWatcher(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        ...
        if not (
            ...
            and any(pattern in file_path.name for pattern in ["settings", "config"])
        ):
            return
...
class SettingsCache:
    ...
    def invalidate(
        self,
        scope: Optional[SettingsScope] = None,
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self.lock:
            return {
                ...
                "expired_entries": sum(
                    1
                    for expire_time in self.ttl.values()
...
class SettingsManager:
    ...
    async def get_value(
        self, key: str, scope: SettingsScope, scope_id: str, use_hierarchy: bool = True
    ) -> Any:
    ...
    async def _resolve_hierarchical_value(
        self, key: str, scope: SettingsScope, scope_id: str
    ) -> Any:
        ...
        scope_index = (
            self.scope_hierarchy.index(scope) if scope in self.scope_hierarchy else 0
        )
    ...
    def _get_scope_id_for_hierarchy(
        self,
        original_scope_id: str,
        original_scope: SettingsScope,
    ...
    async def delete_value(
        self, key: str, scope: SettingsScope, scope_id: str, deleted_by: str = "system"
    ) -> bool:
    ...
    async def restore_snapshot(
        self, snapshot_id: str, restored_by: str = "system"
    ) -> bool:
    ...
    async def import_settings(
        ...
        try:
            ...
            self.logger.info(
                f"Imported {len(settings)} settings to {scope.value}:{scope_id}"
            )
            return True
    ...
    async def clear_scope(self, scope: SettingsScope, scope_id: str) -> int:
        ...
        self.logger.info(
            f"Cleared {len(keys_to_remove)} settings from {scope.value}:{scope_id}"
        )
    ...
    async def validate_all(
        ...
        for composite_key, settings_value in self.values.items():
            ...
            if stored_scope == scope and stored_scope_id == scope_id:
                valid, validation_errors = settings_registry.validate_value(
                    key, settings_value.value
                )
                if not valid:
    ...
    async def reload_from_file(self, file_path: str):
        ...
        try:
            ...
            await self.import_settings(
                {"settings": data}, scope, scope_id, "file_reload"
            )
    ...
    async def _remove_persisted_value(
