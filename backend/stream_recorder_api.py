...
async def create_recorder_endpoint(
    source_id: str, config: Optional[RecordingConfigRequest] = None
):
...
async def remove_recorder(source_id: str):
    ...
    try:
        ...
        if success:
            return RecordingResponse(
                success=True, message=f"Recorder removed for source {source_id}"
            )
        else:
            raise HTTPException(
                status_code=404, detail=f"Recorder for source {source_id} not found"
            )
...
async def start_recording(source_id: str):
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Failed to start recording for source {source_id}",
            )
...
async def stop_recording(source_id: str):
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Failed to stop recording for source {source_id}",
            )
...
async def pause_recording(source_id: str):
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Failed to pause recording for source {source_id}",
            )
...
async def resume_recording(source_id: str):
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Failed to resume recording for source {source_id}",
            )
...
async def get_file_metadata(filename: str, directory: Optional[str] = None):
    ...
    try:
        ...
        metadata_file = os.path.join(
            recordings_dir, os.path.splitext(filename)[0] + ".json"
        )

        if not os.path.exists(metadata_file):
            raise HTTPException(
                status_code=404, detail=f"Metadata for {filename} not found"
            )
        ...
        enhanced_stats = {
            **stats,
            ...
            "recording_efficiency": (
                stats["active_recordings"] / max(stats["total_recorders"], 1)
            )
            * 100,
        }
...
async def stop_all_recordings():
    ...
    try:
        ...
        return RecordingResponse(success=True, message="All recordings stopped")
