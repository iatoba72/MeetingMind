...
class SystemAnalyzer:
    ...
    def analyze_system() -> SystemSpecs:
        ...
        # GPU information
        gpu_name, gpu_memory_gb, has_hw_encoder, encoder_type = (
            SystemAnalyzer._detect_gpu()
        )
                    ...
                    for gpu in c.Win32_VideoController():
                        if gpu.Name:
                            ...
                            if "nvidia" in gpu_name.lower():
                                ...
                                "amd" in gpu_name.lower()
                                or "radeon" in gpu_name.lower()
                            ):
...
class OBSConfigGenerator:
    ...
    def generate_config(
        ...
        stream_settings = self._generate_stream_settings(
            streaming_platform, quality_preset
        )
    ...
    def _generate_video_settings(
        self, meeting_type: MeetingType, quality_preset: QualityPreset
    ) -> OBSVideoSettings:
        ...
        if quality_preset == QualityPreset.ULTRA_LOW_LATENCY:
            ...
            if (
                self.system_specs.cpu_cores >= 6
                and self.system_specs.has_hardware_encoder
            ):
    ...
    def _generate_stream_settings(
        self, platform: StreamingPlatform, quality_preset: QualityPreset
    ) -> OBSStreamSettings:
        ...
        available_bitrate = int(
            self.system_specs.network_upload_mbps * 1000 * 0.8
        )  # 80% of available
        ...
        return OBSStreamSettings(
            ...
            x264_opts=(
                "bframes=0" if quality_preset == QualityPreset.ULTRA_LOW_LATENCY else ""
            ),
        )

    def _generate_record_settings(
    ...
    def _generate_scene_collection(
        ...
        if meeting_type == MeetingType.PRESENTATION:
            scenes = [
                ...
                {"name": "Break", "sources": ["Break Slide", "Background Music"]},
            ]
    ...
    def _generate_hotkeys(self, meeting_type: MeetingType) -> Dict[str, str]:
        ...
        if meeting_type == MeetingType.PRESENTATION:
            hotkeys.update(
                {"Presentation Scene": "P", "Q&A Scene": "Q", "Break Scene": "B"}
            )
    ...
    def _generate_filters(self) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            },
            {
                ...
                "settings": {"brightness": 0.1, "contrast": 0.1, "saturation": 0.05},
            },
        ]
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if self.system_specs.cpu_cores <= 4:
            recommendations.append(
                "Consider upgrading to a CPU with more cores for better streaming performance"
            )
            recommendations.append(
                "Use hardware encoding if available to reduce CPU load"
            )

        # RAM recommendations
        if self.system_specs.ram_gb < 8:
            recommendations.append(
                "Upgrade to at least 8GB RAM for optimal OBS performance"
            )

        # GPU recommendations
        if not self.system_specs.has_hardware_encoder:
            recommendations.append(
                "Consider a GPU with hardware encoding (NVIDIA RTX, AMD VCE, or Intel QSV)"
            )

        # Network recommendations
        if self.system_specs.network_upload_mbps < 5:
            recommendations.append(
                "Upgrade internet connection for higher quality streaming"
            )
...
"""
    ...
    def _generate_service_json(self, config: Dict[str, Any]) -> str:
        ...
        service_data = {
            ...
            "settings": {"server": stream["server"], "key": stream["key"]},
        }
