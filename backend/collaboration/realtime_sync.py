...
class CollaborationSession:
    ...
    async def add_user(self, user_id: str, user_name: str, websocket: WebSocketServerProtocol, avatar_url: str = None):
    ...
    async def _handle_operation(self, user_id: str, message: CollaborationMessage):
        ...
        try:
            ...
            if operation.op_type == OperationType.INSERT:
                node_id = self.document.insert_text(operation.position, operation.content)
            ...
            await self._send_message(user_id, MessageType.OPERATION_ACK, {
                "operation_id": operation.operation_id
            })
            
        except Exception as e:
            # Send rejection to sender
            await self._send_message(user_id, MessageType.OPERATION_REJECT, {
    ...
    async def _handle_cursor_move(self, user_id: str, message: CollaborationMessage):
        ...
        if user_id in self.users:
            ...
            self.document.update_user_presence(user_id, {
    ...
    async def _handle_selection_change(self, user_id: str, message: CollaborationMessage):
    ...
    async def _handle_annotation_update(self, user_id: str, message: CollaborationMessage):
    ...
    async def _handle_annotation_remove(self, user_id: str, message: CollaborationMessage):
        ...
        if annotation_id:
            ...
            # Broadcast to all users
            await self._broadcast_message(
                MessageType.ANNOTATION_REMOVE,
                {"annotation_id": annotation_id}
            )
    
    async def _handle_action_item_add(self, user_id: str, message: CollaborationMessage):
    ...
    async def _handle_action_item_update(self, user_id: str, message: CollaborationMessage):
        ...
        if item_id:
            ...
            # Broadcast to all users
            await self._broadcast_message(
                MessageType.ACTION_ITEM_UPDATE,
                {
                    "item_id": item_id,
                    "item": item_data
                }
    ...
    async def _handle_action_item_remove(self, user_id: str, message: CollaborationMessage):
        ...
        if item_id:
            ...
            # Broadcast to all users
            await self._broadcast_message(
                MessageType.ACTION_ITEM_REMOVE,
                {"item_id": item_id}
            )
    ...
    async def _handle_ping(self, user_id: str, message: CollaborationMessage):
        """Handle ping message"""
        await self._send_message(user_id, MessageType.PONG, {
    ...
    async def _send_document_state(self, user_id: str):
        ...
        state = {
            ...
            "metadata": {
                "title": self.document.metadata.get("title").get() if self.document.metadata.get("title") else None,
                "created_at": self.document.metadata.get("created_at").get() if self.document.metadata.get("created_at") else None,
                "last_modified": self.document.metadata.get("last_modified").get() if self.document.metadata.get("last_modified") else None
            }
        }
    ...
    async def _broadcast_message(self, message_type: MessageType, data: Dict[str, Any], exclude_user: str = None):
    ...
    def is_idle(self, idle_timeout_minutes: int = 30) -> bool:
        ...
        return datetime.utcnow() - self.last_activity > timedelta(minutes=idle_timeout_minutes)
...
class CollaborationServer:
        ...
        try:
            ...
            if message.message_type != MessageType.USER_JOIN:
                await websocket.send(json.dumps({
                    "type": "error",
                    "message": "First message must be USER_JOIN"
                }))
                return
            ...
            if user_id and document_id:
                ...
                self.logger.info(f"User {user_id} disconnected from document {document_id}")
    ...
    async def _get_or_create_session(self, document_id: str) -> CollaborationSession:
        ...
        if document_id not in self.sessions:
            ...
            self.logger.info(f"Created new collaboration session for document {document_id}")
        ...
        while True:
            try:
                ...
                for document_id in idle_sessions:
                    ...
                    self.logger.info(f"Cleaned up idle session for document {document_id}")
