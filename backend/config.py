...
class AudioConfig:
    ...
    max_recording_duration: int = get_env_int(
        "AUDIO_MAX_RECORDING_DURATION", 10800000
    )  # 3 hours
    ...
    supported_formats: List[str] = get_env_list(
        "AUDIO_SUPPORTED_FORMATS", ["wav", "mp3", "mp4", "m4a", "webm", "ogg"]
    )
...
class FeatureFlags:
    ...
    real_time_transcription: bool = get_env_bool(
        "FEATURE_REAL_TIME_TRANSCRIPTION", True
    )
...
class MonitoringConfig:
    ...
    enable_performance_tracking: bool = get_env_bool(
    ...
    def _validate_config(self):
        ...
        if self.environment == "production":
            ...
            if not self.security.secure_cookies:
                logger.warning(
                    "Secure cookies disabled in production - this is a security risk"
                )
