...
class NetworkDiagnostics:
    ...
    async def run_comprehensive_test(
        self, target_host: str = "google.com"
    ) -> Dict[str, Any]:
    ...
    async def ping_test(
        self, host: str, count: int = 10, timeout: int = 5
    ) -> PingResult:
    ...
    async def test_port_connectivity(
        self, host: str, port: int, protocol: str = "tcp", timeout: int = 5
    ) -> PortTestResult:
    ...
    async def bandwidth_test(self, duration_seconds: int = 10) -> BandwidthResult:
        ...
        try:
            ...
            jitter = (
                statistics.stdev(latency_results) if len(latency_results) > 1 else 0
            )
        ...
        try:
            for interface_name, interface_addresses in psutil.net_if_addrs().items():
                ...
                try:
                    ...
                    duplex = {0: "half", 1: "full", 2: "unknown"}.get(
                        stats.duplex, "unknown"
                    )
                except KeyError:
                    is_up = False
                    mtu = 0
    ...
    async def traceroute_test(self, host: str, max_hops: int = 15) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"host": host, "output": "", "success": False, "error": str(e)}

    async def assess_network_quality(
        self,
        ...
        bandwidth_score = self._calculate_bandwidth_score(
            bandwidth_result.download_mbps
        )
        stability_score = self._calculate_stability_score(
            ping_result.packet_loss_percent, ping_result.std_deviation_ms
        )

        # Overall score (weighted average)
        overall_score = int(
            (latency_score * 0.3 + bandwidth_score * 0.4 + stability_score * 0.3)
        )
        ...
        failed_ports = [
            p for p in port_results if not p.is_open and p.port in [80, 443]
        ]
        if failed_ports:
