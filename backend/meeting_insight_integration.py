...
class MeetingInsightIntegration:
    ...
    async def create_meeting_session(
        self, meeting_id: str, user_id: str, session_config: Dict[str, Any] = None
    ) -> str:
        ...
        session = MeetingInsightSession(
            ...
            config={
                ...
                "insight_generation_enabled": config.get(
                ...
                "quality_thresholds": config.get(
                    "quality_thresholds",
                    {"min_confidence": 0.6, "max_insights_per_minute": 3},
                ),
            },
            # Initial state
        ...
        self.logger.info(
            f"Created meeting insight session {session_id} for meeting {meeting_id}"
        )
    ...
    async def _process_segment_pipeline(
        self, session: MeetingInsightSession, segment: TranscriptSegment
    ) -> Dict[str, Any]:
        ...
        if (
            session.integration_mode == IntegrationMode.REAL_TIME
            and session.config["real_time_processing"]
        ):
    ...
    async def _process_custom_rules(
        self,
        session: MeetingInsightSession,
    ...
    def _create_scheduling_context(
        self, session: MeetingInsightSession, segment: TranscriptSegment
    ) -> SchedulingContext:
        ...
        speaking_frequency = len(
            set(seg.speaker for seg in recent_segments if seg.speaker)
        )
        ...
        recent_triggers = [
            t for t in session.triggers if segment.timestamp - t.timestamp < 60
        ]
    ...
    def _get_speaker_context(
        self, session: MeetingInsightSession, speaker: Optional[str]
    ) -> Dict[str, Any]:
        ...
        speaker_segments = [
            seg for seg in session.transcript_segments if seg.speaker == speaker
        ]

        # Calculate speaking time (rough estimate)
        speaking_time = sum(
            len(seg.text) / 150.0 for seg in speaker_segments
        )  # Assume 150 WPM
    ...
    async def _generate_immediate_insights(
        ...
        for trigger in triggers:
            if (
                trigger.should_generate_insight
                and trigger.priority in ["high", "critical"]
                and trigger.confidence > 0.7
            ):
                ...
                insight = await self.insight_service.generate_insight(
                    session.insight_session_id, insight_request
                )
    ...
    async def _update_meeting_phase(
        self, session: MeetingInsightSession, segment: TranscriptSegment
    ):
    ...
    async def get_session_insights(
        self, session_id: str, filters: Dict[str, Any] = None
    ) -> List[GeneratedInsight]:
        ...
        if filters:
            if "min_confidence" in filters:
                insights = [
                    i
                    for i in insights
                    if i.confidence.overall_confidence >= filters["min_confidence"]
                ]
            ...
            if "insight_types" in filters:
                insights = [
                    i
                    for i in insights
                    if i.insight_type.value in filters["insight_types"]
                ]

            if "priorities" in filters:
                insights = [
                    i for i in insights if i.priority.value in filters["priorities"]
                ]

            if "time_range" in filters:
                start_time, end_time = filters["time_range"]
                insights = [
                    i for i in insights if start_time <= i.meeting_timestamp <= end_time
                ]
    ...
    async def get_session_analytics(self, session_id: str) -> Dict[str, Any]:
        ...
        topic_analysis = await self.topic_service.get_session_analysis(
            session.topic_session_id
        )
        trigger_stats = await self.trigger_engine.get_session_statistics(
            session.trigger_session_id
        )
        scheduler_status = await self.scheduler.get_session_status(
            session.scheduler_session_id
        )
    ...
    async def close_session(self, session_id: str) -> Dict[str, Any]:
        ...
        await self._emit_event(
            "session_closed",
            {"session_id": session_id, "final_analytics": final_analytics},
        )
