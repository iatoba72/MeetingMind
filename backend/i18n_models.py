...
class Language(Base):
    ...
    target_translations = relationship(
        "Translation",
        foreign_keys="Translation.target_language_id",
...
class Translation(Base):
    ...
    source_transcript_id = Column(
        UUID(as_uuid=True),
        ForeignKey("multilanguage_transcripts.id", ondelete="CASCADE"),
    )
    ...
    translation_memory_id = Column(
        UUID(as_uuid=True), ForeignKey("translation_memory.id")
    )
    ...
    # Relationships
    source_transcript = relationship(
        "MultiLanguageTranscript", back_populates="translations"
    )
    source_language = relationship(
        "Language", foreign_keys=[source_language_id], back_populates="translations"
    )
    target_language = relationship(
        "Language",
        foreign_keys=[target_language_id],
        back_populates="target_translations",
    )
    translation_memory = relationship(
        "TranslationMemory", back_populates="translations"
    )
    parent_translation = relationship("Translation", remote_side=[id])
    child_translations = relationship(
        "Translation", back_populates="parent_translation"
    )

    # Constraints and indexes
    __table_args__ = (
        CheckConstraint(
            "source_language_id != target_language_id", name="check_different_languages"
        ),
...
class TranslationMemory(Base):
    ...
    __table_args__ = (
        ...
        Index(
            "idx_tm_languages_domain", "source_language", "target_language", "domain"
        ),
...
class LanguageDetectionLog(Base):
    ...
    __table_args__ = (
        ...
        Index(
            "idx_detection_log_language_verified", "detected_language", "human_verified"
        ),
    )
...
class CulturalContextRule(Base):
    ...
    __table_args__ = (
        ...
        Index(
            "idx_cultural_rules_language_context", "language_pair", "business_context"
        ),
        Index(
            "idx_cultural_rules_active_effective", "is_active", "effectiveness_score"
        ),
    )
...
class TranslationQualityMetric(Base):
    ...
    )  # bleu, rouge, bertscore, human, etc.
    ...
    baseline_type = Column(
        String(50)
    )  # reference_human, other_provider, previous_version
...
Index(
    ...
    Language.transcription_supported,
    Language.transcription_accuracy,
)
Index(
    "idx_languages_translation_capability",
    Language.translation_supported,
    Language.translation_quality,
)
Index(
    "idx_ml_transcripts_code_switching",
    MultiLanguageTranscript.is_code_switched,
    MultiLanguageTranscript.detection_confidence,
)
Index(
    "idx_translations_real_time", Translation.provider, Translation.translation_time_ms
)
Index(
    "idx_tm_exact_fuzzy_ratio",
    TranslationMemory.exact_matches,
    TranslationMemory.fuzzy_matches,
)
