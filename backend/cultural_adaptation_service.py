...
class CulturalProfileManager:
    ...
    async def create_cultural_profile(
        ...
        defaults = self.cultural_defaults.get(
            culture_code.lower(), self.cultural_defaults["us"]
        )
    ...
    async def infer_cultural_profile(
        ...
        defaults = self.cultural_defaults.get(
            culture_code, self.cultural_defaults["us"]
        )
        ...
        profile = CulturalProfile(
            ...
            organization_id=(
                participant_info.get("organization_id") if participant_info else None
            ),
...
class CulturalAdaptationEngine:
    ...
    def _load_adaptation_rules(self):
        ...
        rules = (
            self.db.query(CulturalContextRule)
            .filter(CulturalContextRule.is_active == True)
            .all()
        )
    ...
    def _apply_adaptation_rule(
        ...
        if (
            target_profile.communication_style == "indirect"
            and source_profile.communication_style == "direct"
        ):
            ...
            target_profile.communication_style == "direct"
            and source_profile.communication_style == "indirect"
        ):
    ...
    def _generate_cultural_notes(
        self, source_profile: CulturalProfile, target_profile: CulturalProfile
    ) -> str:
...
class CulturalMismatchDetector:
    ...
    async def detect_mismatches(
        ...
        hierarchy_alert = self._check_hierarchy_mismatch(
            meeting_profiles, meeting_transcripts
        )
        if hierarchy_alert:
            alerts.append(hierarchy_alert)

        # Check for communication style mismatches
        comm_alert = self._check_communication_mismatch(
            meeting_profiles, meeting_transcripts
        )
        if comm_alert:
        ...
        decision_alert = self._check_decision_making_mismatch(
            meeting_profiles, meeting_transcripts
        )
        if decision_alert:
        ...
        time_alert = self._check_time_orientation_mismatch(
            meeting_profiles, meeting_transcripts
        )
        if time_alert:
    ...
    def _check_hierarchy_mismatch(
        self, profiles: List[CulturalProfile], transcripts: List[str]
        ...
        if "high" in hierarchy_levels and "low" in hierarchy_levels:
            ...
            if conflicts_found:
                affected = [
                    p.participant_id
                    for p in profiles
                    if p.hierarchy_preference == "high"
                ]
    ...
    def _check_communication_mismatch(
        ...
        if ("direct" in styles and "high_context" in styles) or (
            "direct" in styles and "indirect" in styles
        ):
            ...
            direct_count = sum(
                combined_text.count(indicator) for indicator in direct_indicators
            )
            indirect_count = sum(
                combined_text.count(indicator) for indicator in indirect_indicators
            )
            ...
            if direct_count > indirect_count * 3:  # Very direct communication
                affected = [
                    p.participant_id
                    for p in profiles
                    if p.communication_style in ["indirect", "high_context"]
                ]
    ...
    def _check_decision_making_mismatch(
        ...
        if "consensus" in decision_styles and "authoritative" in decision_styles:
            ...
            auth_count = sum(
                combined_text.count(indicator) for indicator in authoritative_indicators
            )
            consensus_count = sum(
                combined_text.count(indicator) for indicator in consensus_indicators
            )
            ...
            if auth_count > consensus_count and consensus_count == 0:
                affected = [
                    p.participant_id
                    for p in profiles
                    if p.decision_making_style == "consensus"
                ]
    ...
    def _check_time_orientation_mismatch(
        ...
        if "monochronic" in time_orientations and "polychronic" in time_orientations:
            ...
            time_pressure = sum(
                combined_text.count(indicator) for indicator in time_pressure_indicators
            )
            relationship_focus = sum(
                combined_text.count(indicator) for indicator in relationship_indicators
            )
            ...
            if time_pressure > relationship_focus * 2:
                affected = [
                    p.participant_id
                    for p in profiles
                    if p.time_orientation == "polychronic"
                ]
...
class CulturalAdaptationService:
    ...
    async def analyze_meeting_cultural_context(self, meeting_id: str) -> Dict[str, Any]:
        """
        ...
        mismatches = await self.mismatch_detector.detect_mismatches(
            cultural_profiles, all_transcript_texts, CulturalContext.BUSINESS_CASUAL
        )
    ...
    def _calculate_cultural_diversity(self, profiles: List[CulturalProfile]) -> float:
        ...
        actual_diversity = (
            unique_cultures + unique_comm_styles + unique_hierarchy + unique_formality
        ) / 4
