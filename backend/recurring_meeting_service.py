...
class RecurringMeetingService:
    ...
    async def detect_patterns(
        self, user_id: str, organization_id: Optional[str] = None, days_back: int = 90
        ...
        try:
            ...
            if len(meetings) < 3:
                logger.info(
                    f"Not enough meetings ({len(meetings)}) for pattern detection"
                )
                return []
            ...
            for group_name, group_meetings in meeting_groups.items():
                if len(group_meetings) >= 3:  # Need at least 3 meetings for pattern
                    ...
                    if (
                        pattern and pattern.confidence > 0.7
    ...
    async def _analyze_meeting_group(
        self, meetings: List[Meeting]
    ...
    async def create_recurring_series(
        ...
        try:
            ...
            template = (
                ...
                .filter(MeetingTemplate.id == template_id)
                .first()
            )
            ...
            series = RecurringMeetingSeries(
                ...
                end_date=(
                    parse_date(recurrence_config["end_date"])
                    if recurrence_config.get("end_date")
                    else None
                ),
                start_time=recurrence_config.get("start_time", "09:00:00"),
                duration_minutes=recurrence_config.get(
    ...
    async def _generate_upcoming_meetings(
        self, series: RecurringMeetingSeries, look_ahead_days: int = 90
    ) -> List[Meeting]:
        """Generate upcoming meetings for a recurring series"""
        try:
            # Calculate occurrences using dateutil
            start_date = series.start_date
            end_date = series.end_date or (
                datetime.now(timezone.utc) + timedelta(days=look_ahead_days)
            )
            ...
            for occurrence_date in rule:
                ...
                exception = (
                    self.db.query(RecurringMeetingException)
                    .filter(
                        and_(
                ...
                meeting.meeting_number = (
                    f"REC-{series.id.hex[:8]}-{len(meetings)+1:03d}"
                )
    ...
    async def apply_template_to_pattern(
        self,
    ...
    async def suggest_templates_for_pattern(
        self,
        pattern: PatternDetectionResult,
        user_id: str,
        organization_id: Optional[str] = None,
    ...
    async def handle_series_exception(
        self,
        series_id: str,
        ...
        try:
            ...
            if exception_type == "cancelled" and meeting:
                ...
                meeting.scheduled_end = new_date + timedelta(
    ...
    async def get_upcoming_occurrences(
        self, series_id: str, days_ahead: int = 30
    ) -> List[Meeting]:
    ...
    async def update_series_template(
        self, series_id: str, new_template_id: str, apply_to_future: bool = True
    ) -> RecurringMeetingSeries:
        """Update template for recurring series"""
        try:
            series = (
                self.db.query(RecurringMeetingSeries)
                .filter(RecurringMeetingSeries.id == series_id)
                .first()
            )
            ...
            if apply_to_future:
                ...
                new_template = (
                    self.db.query(MeetingTemplate)
                    .filter(MeetingTemplate.id == new_template_id)
                    .first()
                )
