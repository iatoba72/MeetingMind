...
class SpeakerProfile(Base):
    ...
    meeting_participations = relationship(
        "MeetingSpeakerParticipation", back_populates="speaker"
    )
...
class SpeakerTrainingSession(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    speaker_id = Column(
        UUID(as_uuid=True), ForeignKey("speaker_profiles.id"), nullable=False
    )
    ...
    audio_samples = relationship(
        "TrainingAudioSample", back_populates="training_session"
    )
...
class TrainingAudioSample(Base):
    ...
    training_session_id = Column(
        UUID(as_uuid=True), ForeignKey("speaker_training_sessions.id"), nullable=False
    )
    ...
    training_session = relationship(
        "SpeakerTrainingSession", back_populates="audio_samples"
    )
...
class SpeakerDetectionService:
    ...
    async def detect_voice_activity(
        self, audio_data: bytes, sample_rate: int = None
    ...
    async def extract_speaker_embeddings(
        ...
        for segment in vad_segments:
            try:
                ...
                logger.error(
                    f"Failed to extract embedding for segment {segment.start_time}-{segment.end_time}: {e}"
                )
                continue
    ...
    async def perform_speaker_diarization(
        ...
        clustering = AgglomerativeClustering(
            n_clusters=best_n_speakers, linkage="ward", metric="euclidean"
        )
        ...
        logger.info(
            f"Diarization completed: {best_n_speakers} speakers, {len(segments)} segments"
        )
        return segments

    async def _estimate_speaker_count(
        self, embeddings: np.ndarray, min_speakers: int, max_speakers: int
    ) -> int:
        ...
        estimated_speakers = max(
            min_speakers, min(max_speakers, int(np.sqrt(len(embeddings))))
        )
    ...
    async def process_audio_session(
        self, session_id: str, audio_data: bytes, sample_rate: int = None
    ) -> SpeakerSession:
        ...
        try:
            ...
            embeddings = await self.extract_speaker_embeddings(
    ...
    async def _generate_session_analytics(
        self, session_id: str, segments: List[SpeakerSegment]
    ) -> SpeakerSession:
        ...
        for speaker_id in unique_speakers:
            ...
            speaker_stats[speaker_id] = {
                ...
                "speaking_percentage": (
                    (total_speaking_time / total_duration * 100)
                    if total_duration > 0
                    else 0
                ),
                ...
                "average_segment_duration": (
                    total_speaking_time / segment_count if segment_count > 0 else 0
                ),
            }
        ...
        overall_confidence = (
            np.mean([seg.confidence for seg in segments]) if segments else 0.0
        )
    ...
    async def _resample_audio(
        self, audio_data: bytes, from_rate: int, to_rate: int
    ) -> bytes:
        """Resample audio data"""
...
class SpeakerTrainingService:
    ...
    async def start_training_session(
        self, speaker_name: str, speaker_email: str = None, organization_id: str = None
    ) -> str:
    ...
    async def add_training_sample(
        self, session_id: str, audio_data: bytes, sample_rate: int = 16000
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            vad_segments = await self.detection_service.detect_voice_activity(
            ...
            speech_ratio = (
                total_speech_duration / audio_duration if audio_duration > 0 else 0
            )
            ...
            quality_score = min(
                1.0, speech_ratio * 0.7 + min(avg_audio_level / 1000, 0.3)
            )
    ...
    async def complete_training_session(self, session_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.detection_service.known_speakers[speaker_profile.speaker_id] = (
                speaker_profile
            )
    ...
    def get_training_session_status(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "total_duration": sum(s["duration"] for s in session["audio_samples"]),
            "average_quality": (
                np.mean(session["quality_scores"]) if session["quality_scores"] else 0.0
            ),
