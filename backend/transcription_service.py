...
class WhisperTranscriptionService:
    ...
    def __init__(self):
        ...
        logger.info(
            f"Transcription service initialized - Device: {self.device}, Compute: {self.compute_type}"
        )
    ...
    async def load_model(
        self, model_size: WhisperModelSize, force_reload: bool = False
    ) -> bool:
        """
    ...
    async def transcribe_audio_chunk(
        ...
        try:
            ...
            # Preprocess audio
            audio_array, final_sample_rate = await self.preprocess_audio(
            ...
            words_per_second = (
                word_count / (total_processing_time / 1000)
                if total_processing_time > 0
                else 0
            )
            real_time_factor = (
                total_processing_time / audio_duration if audio_duration > 0 else 0
            )

            # Average confidence from segments
            avg_confidence = (
                np.mean([1 - seg.no_speech_prob for seg in transcription_segments])
            ...
            metrics = TranscriptionMetrics(
                ...
                no_speech_probability=(
                    info.no_speech_probability
                    if hasattr(info, "no_speech_probability")
                    else 0
                ),
                compression_ratio=(
                    np.mean([seg.compression_ratio for seg in transcription_segments])
                    if transcription_segments
                    else 0
                ),
    ...
    async def get_session_metrics(self, session_id: str) -> Dict[str, Any]:
        ...
        total_words = sum(
            m.words_per_second * (m.processing_time_ms / 1000) for m in metrics_list
        )
        ...
        return {
            ...
            "model_used": (
                self.current_model_size.value if self.current_model_size else "none"
            ),
...
def process_audio_chunk_task(
    session_id: str, audio_chunk_id: str, audio_data: str, config_dict: Dict[str, Any]
):
