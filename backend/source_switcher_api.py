...
from source_switcher import (
    get_source_switcher,
    start_source_switcher,
    stop_source_switcher,
    SwitchingConfig,
    AudioSourceInfo,
    SourceType,
    SourceState,
...
async def start_source_switcher_endpoint(
    config: Optional[SwitchingConfigRequest] = None,
):
    ...
    try:
        ...
        if config:
            ...
            switcher_config = SwitchingConfig(
                switching_mode=mode_mapping.get(
        ...
        if success:
            ...
            raise HTTPException(
                status_code=500, detail="Failed to start source switcher"
            )
...
async def stop_source_switcher_endpoint():
    ...
    try:
        ...
        if success:
            return SourceSwitcherResponse(
                success=True, message="Source switcher stopped successfully"
            )
        else:
            raise HTTPException(
                status_code=500, detail="Failed to stop source switcher"
            )
...
async def get_active_source():
    ...
    try:
        ...
        if not active_source:
            return SourceSwitcherResponse(
                success=True, message="No active source", data={"active_source": None}
            )
...
async def switch_to_source(source_id: str):
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=400, detail=f"Failed to switch to source {source_id}"
            )
...
async def update_source_metrics(source_id: str, metrics: SourceMetricsUpdate):
    ...
    try:
        ...
        if success:
            return SourceSwitcherResponse(
                success=True, message=f"Metrics updated for source {source_id}"
            )
        else:
...
async def update_source_switcher_config(config_update: Dict[str, Any]):
    ...
    try:
        ...
        if success:
            return SourceSwitcherResponse(
                success=True, message="Source switcher configuration updated"
            )
        else:
            raise HTTPException(
