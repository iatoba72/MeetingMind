...
from network_transcription_service import (
    ...
    get_network_transcription_service,
    StreamTranscriptionConfig,
    TranscriptionSegment,
    NetworkAudioChunk,
)
from video_transcription_sync import (
    VideoTranscriptionSyncService,
    get_video_transcription_sync_service,
    StreamSyncConfig,
...
class TranscriptionConnectionManager:
    ...
    async def broadcast_transcription(
        self, stream_id: str, segment: TranscriptionSegment
    ):
        if stream_id in self.active_connections:
            message = {
                "type": "transcription",
...
async def create_transcription_stream(
    stream_id: str, config: StreamConfigRequest, background_tasks: BackgroundTasks
):
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=400, detail=f"Failed to create stream {stream_id}"
            )
        ...
        return TranscriptionResponse(
            ...
            data={"stream_id": stream_id, "config": transcription_config.__dict__},
        )
...
async def process_audio_chunk(stream_id: str, request: AudioChunkRequest):
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=400,
                detail=f"Failed to process audio for stream {stream_id}",
            )
...
async def process_video_frame(stream_id: str, request: VideoFrameRequest):
    ...
    try:
        ...
        if not synchronizer:
            raise HTTPException(
                status_code=404, detail=f"Stream synchronizer {stream_id} not found"
            )
...
async def get_global_statistics():
    ...
    try:
        ...
        return TranscriptionResponse(
            ...
            data={"transcription": transcription_stats, "synchronization": sync_stats},
        )
...
async def resync_stream(
    stream_id: str, reference_timestamp: float, video_frame_number: Optional[int] = None
):
...
@router.get(
    "/streams/{stream_id}/transcription/{frame_number}",
...
async def get_transcription_at_frame(stream_id: str, frame_number: int):
    ...
    try:
        ...
        logger.error(
            f"Error getting transcription at frame {frame_number} for stream {stream_id}: {e}"
        )
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_available_models():
    ...
    try:
        ...
        return TranscriptionResponse(
            success=True, message="Available models retrieved", data={"models": models}
        )
