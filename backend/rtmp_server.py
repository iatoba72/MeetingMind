...
class RTMPServer:
        ...
        try:
            ...
            if self.server_process:
                try:
                    ...
                    logger.warning("RTMP server process didn't terminate gracefully, killing...")
    ...
    async def _start_ffmpeg_server(self):
        ...
        cmd = [
            ...
            '-f', 'flv',
            '-listen', '1',
            '-timeout', str(self.config.timeout_seconds * 1000000),  # microseconds
            '-i', f'rtmp://localhost:{self.config.port}/live',
            '-c', 'copy',
            '-f', 'null',
            '-'
        ]
    ...
    async def _parse_ffmpeg_log(self, log_line: str):
        ...
        try:
            ...
            if any(keyword in log_line.lower() for keyword in ['error', 'warning', 'connection']):
    ...
    async def _start_stream_processor(self, stream_key: str):
        ...
        try:
            ...
            cmd = [
                ...
                '-hide_banner',
                '-loglevel', 'warning',
                '-i', f'rtmp://localhost:{self.config.port}/live/{stream_info.stream_name}',
                '-f', 'wav',
                '-acodec', 'pcm_s16le',
                '-ar', '44100',
                '-ac', '2',
            ...
            if self.config.recording_enabled:
                ...
                recording_file = f"{self.config.recording_path}/rtmp_{stream_key}_{timestamp}.mp4"
