...
from transcription_service import (
    ...
    TranscriptionMetrics, WhisperModelSize
)
...
class BaseTranscriptionProvider(ABC):
    ...
    def calculate_cost(self, audio_duration: float) -> float:
        ...
        if cost_config.billing_increment > 0:
            ...
            billable_seconds = ((audio_duration + increment_seconds - 1) // increment_seconds) * increment_seconds
            minutes = billable_seconds / 60.0
        
        # Check free tier
        remaining_free = max(0, cost_config.free_tier_minutes - self.total_audio_minutes)
    ...
    def _parse_assemblyai_result(self, data: Dict[str, Any]) -> TranscriptionResult:
        ...
        if "words" in data and data["words"]:
            ...
            for word_data in data["words"]:
                ...
                if (word_data["text"].endswith(('.', '!', '?')) or 
                    len(current_segment_words) >= 20):
                    ...
                    segment = TranscriptionSegment(
                        ...
                        no_speech_prob=1 - (sum(w["probability"] for w in current_segment_words) / len(current_segment_words)),
                        avg_logprob=sum(w["probability"] for w in current_segment_words) / len(current_segment_words),
        ...
        metrics = TranscriptionMetrics(
            ...
            words_per_second=len(data.get("words", [])) / (audio_duration / 1000) if audio_duration > 0 else 0,
...
class OpenAIWhisperProvider(BaseTranscriptionProvider):
    ...
    def _parse_openai_result(self, data: Dict[str, Any]) -> TranscriptionResult:
        ...
        metrics = TranscriptionMetrics(
            ...
            memory_used_mb=0.0,
            gpu_memory_used_mb=0.0,
            words_per_second=len(data.get("text", "").split()) / (duration / 1000) if duration > 0 else 0,
...
class CloudTranscriptionService:
    ...
    def _load_provider_configs(self):
        ...
        if os.getenv("OPENAI_API_KEY"):
            self.provider_configs[TranscriptionProvider.OPENAI_WHISPER] = ProviderConfig(
    ...
    async def _transcribe_with_fallback(self, job: TranscriptionJob) -> TranscriptionJob:
        ...
        for provider_type in job.preferred_providers:
            ...
            if not await provider.health_check():
                logger.warning(f"Provider {provider_type.value} is not available, skipping")
                continue
    ...
    async def _transcribe_comparison_mode(self, job: TranscriptionJob) -> TranscriptionJob:
        ...
        if tasks:
            ...
            # Process results
            successful_attempts = [r for r in results if isinstance(r, TranscriptionAttempt) and r.success]
            
            if successful_attempts:
                # Choose best result (highest confidence or preferred provider)
                best_attempt = max(successful_attempts, key=lambda a: a.confidence_score)
