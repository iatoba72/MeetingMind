...
class SummaryQualityFeedbackService:
    ...
    async def collect_feedback(
        ...
        try:
            ...
            self.logger.info(f"Collected feedback {feedback_id} for summary {summary_id}")
    ...
    def _determine_sentiment(self, feedback_data: Dict[str, Any]) -> FeedbackSentiment:
        ...
        if 'thumbs' in feedback_data:
            return FeedbackSentiment.POSITIVE if feedback_data['thumbs'] else FeedbackSentiment.NEGATIVE
        
        # Text sentiment analysis (simplified)
        if 'text' in feedback_data:
            text = feedback_data['text'].lower()
            positive_words = ['good', 'great', 'excellent', 'perfect', 'accurate', 'helpful', 'clear']
            negative_words = ['bad', 'poor', 'terrible', 'inaccurate', 'confusing', 'missing', 'wrong']
    ...
    async def _update_feedback_summary(self, summary_id: str):
        ...
        for category in FeedbackCategory:
            ...
            if category_feedback:
                category_ratings_list = [f.rating for f in category_feedback if f.rating is not None]
    ...
    def _calculate_feedback_confidence(self, feedback_list: List[FeedbackEntry]) -> float:
        ...
        if ratings:
            ...
            rating_consistency = max(0, 1 - (rating_std / 2))  # Normalize standard deviation
        else:
        ...
        most_common_sentiment_ratio = sentiment_counter.most_common(1)[0][1] / len(sentiments)
        
        # Volume factor (more feedback = higher confidence)
        volume_factor = min(1.0, len(feedback_list) / 10)  # Cap at 10 feedback entries
        
        # Combined confidence
        confidence = (rating_consistency * 0.4 + most_common_sentiment_ratio * 0.4 + volume_factor * 0.2)
    ...
    def _update_stats(self, feedback_entry: FeedbackEntry):
        ...
        if feedback_entry.rating is not None:
            total_ratings = sum(
                1 for f in self.feedback_entries.values() 
            ...
            self.stats['avg_rating'] = total_rating_sum / total_ratings if total_ratings > 0 else 0.0
    ...
    async def _check_quality_alerts(self, summary_id: str):
        ...
        negative_sentiments = (
            summary_feedback.sentiment_distribution.get(FeedbackSentiment.NEGATIVE, 0) +
            summary_feedback.sentiment_distribution.get(FeedbackSentiment.VERY_NEGATIVE, 0)
        )
                ...
                if len(ratings) >= 5:
                    ...
                    if avg_rating < 3.0:
                        insights.append(FeedbackInsight(
                            ...
                            impact_estimate=len(ratings) / len(self.feedback_entries),
                            priority="high" if avg_rating < 2.5 else "medium",
                            created_at=datetime.now()
                        ))
            
            # Insight 3: Response time correlation
            quick_feedback = [f for f in self.feedback_entries.values() if f.time_to_feedback < 30]
            slow_feedback = [f for f in self.feedback_entries.values() if f.time_to_feedback > 120]
            
            if len(quick_feedback) >= 5 and len(slow_feedback) >= 5:
                quick_ratings = [f.rating for f in quick_feedback if f.rating is not None]
    ...
    async def get_quality_metrics(self) -> QualityMetrics:
        ...
        accuracy_feedback = [f for f in all_feedback if f.category == FeedbackCategory.ACCURACY]
        accuracy_ratings = [f.rating for f in accuracy_feedback if f.rating is not None]
        accuracy_score = (statistics.mean(accuracy_ratings) / 5.0) if accuracy_ratings else 0.0
        
        completeness_feedback = [f for f in all_feedback if f.category == FeedbackCategory.COMPLETENESS]
        completeness_ratings = [f.rating for f in completeness_feedback if f.rating is not None]
        completeness_score = (statistics.mean(completeness_ratings) / 5.0) if completeness_ratings else 0.0
        
        clarity_feedback = [f for f in all_feedback if f.category == FeedbackCategory.CLARITY]
        clarity_ratings = [f.rating for f in clarity_feedback if f.rating is not None]
        clarity_score = (statistics.mean(clarity_ratings) / 5.0) if clarity_ratings else 0.0
        ...
        user_retention = repeat_users / len(user_feedback_counts) if user_feedback_counts else 0.0
        ...
        previous_feedback = [f for f in all_feedback if sixty_days_ago <= f.created_at < thirty_days_ago]
        
        improvement_trends = {}
        if recent_feedback and previous_feedback:
            recent_ratings = [f.rating for f in recent_feedback if f.rating is not None]
            previous_ratings = [f.rating for f in previous_feedback if f.rating is not None]
            ...
            if summary_feedback:
                ...
                if summary_feedback.common_issues:
                    recommendations.append({
                        ...
                        'actions': [
                            f'Address issue: {issue}' for issue in summary_feedback.common_issues[:3]
                        ],
                        'affected_summaries': [summary_id]
                    })
        else:
    ...
    async def export_feedback_data(
        ...
        if start_date:
            feedback_entries = [f for f in feedback_entries if f.created_at >= start_date]
