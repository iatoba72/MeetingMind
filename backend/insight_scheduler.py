...
from insight_generation_service import (
    InsightGenerationRequest, GeneratedInsight, InsightType, 
    InsightPriority, InsightStatus
)
...
class InsightScheduler:
    ...
    async def create_session(self, meeting_id: str, session_config: Dict[str, Any] = None) -> str:
        ...
        session_data = {
            ...
            'config': {
                'strategy': SchedulingStrategy(config.get('strategy', self.strategy.value)),
                'max_concurrent_tasks': config.get('max_concurrent_tasks', self.max_concurrent_tasks),
                'batch_window_seconds': config.get('batch_window_seconds', self.batch_window_seconds),
                'enable_batching': config.get('enable_batching', True),
                'enable_adaptive_learning': config.get('enable_adaptive_learning', True),
                'quality_vs_speed_preference': config.get('quality_vs_speed_preference', 0.7),  # 0=speed, 1=quality
                'user_interrupt_sensitivity': config.get('user_interrupt_sensitivity', 0.8)
            },
            'session_queue': [],
            'session_metrics': {},
        ...
        self.logger.info(f"Created scheduling session {session_id} for meeting {meeting_id}")
        
        return session_id
    
    async def schedule_insight(self, 
                             session_id: str,
                             request: InsightGenerationRequest,
                             context: Optional[SchedulingContext] = None) -> str:
        ...
        scheduled_task = ScheduledTask(
            ...
            scheduled_time=self._calculate_scheduled_time(execution_timing, priority_score),
        ...
        self.logger.debug(f"Scheduled insight task {task_id} with priority {priority_score}")
    ...
    def _determine_execution_timing(self, 
                                  request: InsightGenerationRequest,
                                  context: Optional[SchedulingContext],
                                  session: Dict) -> ExecutionTiming:
    ...
    def _calculate_resource_requirements(self, request: InsightGenerationRequest) -> Dict[ResourceConstraint, float]:
    ...
    def _calculate_deadline(self, request: InsightGenerationRequest, timing: ExecutionTiming) -> Optional[float]:
    ...
    def _calculate_scheduled_time(self, timing: ExecutionTiming, priority_score: float) -> float:
    ...
    def _find_dependencies(self, request: InsightGenerationRequest, session: Dict) -> List[str]:
        ...
        if trigger.trigger_type == TriggerType.TOPIC_SHIFT:
            ...
            for task_id, task in self.scheduled_tasks.items():
                if (task.request.trigger.topic_context == trigger.topic_context and
                    task.request.preferred_type == InsightType.SUMMARY):
                    dependencies.append(task_id)
        
        return dependencies
    
    def _determine_context_requirements(self, request: InsightGenerationRequest) -> List[str]:
        ...
        if trigger.trigger_type in [TriggerType.TOPIC_SHIFT, TriggerType.BUSINESS_IMPACT]:
            requirements.append('topic_context')
        
        # Speaker-related insights need speaker context
        if trigger.trigger_type in [TriggerType.SPEAKER_CHANGE, TriggerType.QUESTION_ASKED]:
    ...
    async def _add_to_queue(self, session_id: str, task: ScheduledTask):
        ...
        heapq.heappush(self.priority_queue, (-task.priority_score, task.scheduled_time, task))
        
        # Check if should be batched
        if (session['config']['enable_batching'] and 
            task.execution_timing == ExecutionTiming.BATCH_WINDOW):
    ...
    async def _consider_for_batching(self, task: ScheduledTask):
        ...
        for other_task in self.scheduled_tasks.values():
            if (other_task.id != task.id and
                other_task.execution_timing == ExecutionTiming.BATCH_WINDOW and
                other_task.id not in self.executing_tasks):
        ...
        if len(batch_candidates) >= 2:
            ...
            batch_tasks = [task]
            for compatibility, other_task in batch_candidates[:4]:  # Max 5 tasks per batch
            ...
            batch = InsightBatch(
                ...
                estimated_processing_time=sum(30 for _ in batch_tasks),  # 30s per task estimate
                resource_requirements=self._aggregate_resource_requirements(batch_tasks),
                coherence_score=np.mean([compatibility for compatibility, _ in batch_candidates])
            )
    ...
    def _calculate_batch_compatibility(self, task1: ScheduledTask, task2: ScheduledTask) -> float:
        ...
        # Same topic context
        if (task1.request.trigger.topic_context and 
            task1.request.trigger.topic_context == task2.request.trigger.topic_context):
            compatibility += 0.3
        
        # Same speaker
        if (task1.request.trigger.speaker and 
            task1.request.trigger.speaker == task2.request.trigger.speaker):
            compatibility += 0.2
    ...
    def _aggregate_resource_requirements(self, tasks: List[ScheduledTask]) -> Dict[ResourceConstraint, float]:
            ...
            if scheduled_time <= current_time:
                ...
                if self._check_dependencies(task):
                    ...
                    if self._check_resource_availability(task):
                        ...
                        # Re-queue for later
                        heapq.heappush(self.priority_queue, (neg_priority, current_time + 10, task))
                else:
                    # Re-queue for later (dependencies not ready)
                    heapq.heappush(self.priority_queue, (neg_priority, current_time + 5, task))
            else:
                # Task not ready yet, put back
                heapq.heappush(self.priority_queue, (neg_priority, scheduled_time, task))
                break
    ...
    async def _execute_task(self, task: ScheduledTask):
        ...
        try:
            ...
            self.metrics.average_execution_delay = (
                (self.metrics.average_execution_delay * (self.metrics.total_tasks_scheduled - 1) + execution_delay) /
            ...
            if task.retry_count < task.max_retries:
                task.retry_count += 1
                task.scheduled_time = time.time() + (task.retry_count * 30)  # Exponential backoff
                self.scheduled_tasks[task.id] = task
                heapq.heappush(self.priority_queue, (-task.priority_score, task.scheduled_time, task))
    ...
    async def _update_current_context(self):
        ...
        context = SchedulingContext(
            ...
            recent_insight_density=len([t for t in self.execution_history 
    ...
    async def get_session_status(self, session_id: str) -> Dict[str, Any]:
        ...
        scheduled_count = sum(1 for tid in session_task_ids if tid in self.scheduled_tasks)
        executing_count = sum(1 for tid in session_task_ids if tid in self.executing_tasks)
        completed_count = sum(1 for tid in session_task_ids if tid in self.completed_tasks)
