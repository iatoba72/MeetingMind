...
{insights}""",
    ...
    async def generate_multi_level_summary(
        ...
        if cache_key in self.summary_cache:
            ...
            if (
                datetime.now() - cached_result.generated_at
        ...
        try:
            ...
            for level in levels:
                ...
                request = SummarizationRequest(
                    ...
                    speaker_context=(
                        meeting_metadata.get("speakers", {}) if meeting_metadata else {}
                    ),
            ...
            participants = (
                self._extract_participants(meeting_metadata) if meeting_metadata else []
            )
    ...
    def _create_level_configuration(
        self, level: SummaryLevel, base_config: SummaryConfiguration
    ...
    async def _apply_format_transformation(
        ...
        if target_format == SummaryFormat.BULLET_POINTS:
            formatted_content = self._format_as_bullets(
                original_content, summary_result.key_points
            )
        elif target_format == SummaryFormat.NUMBERED_LIST:
            formatted_content = self._format_as_numbered_list(
                original_content, summary_result.key_points
            )
        elif target_format == SummaryFormat.EXECUTIVE:
            formatted_content = self._format_as_executive(
                original_content, summary_result
            )
        elif target_format == SummaryFormat.REPORT:
            formatted_content = self._format_as_report(
                original_content, summary_result, configuration
            )
...
Processing Time: {summary_result.processing_time_ms:.0f}ms"""
    ...
    def _format_as_report(
        ...
        # Metadata section
        report_sections.append(
            f"Generated: {summary_result.generated_at.strftime('%Y-%m-%d %H:%M:%S')}"
        )
        report_sections.append(
            f"Technique: {summary_result.technique.value.replace('_', ' ').title()}"
        )
...
Best regards"""
    ...
    async def _extract_consolidated_insights(
        self, text: str, summaries: Dict[SummaryLevel, SummaryResult]
    ) -> List[str]:
    ...
    async def _extract_consolidated_actions(
        self, text: str, summaries: Dict[SummaryLevel, SummaryResult]
    ) -> List[str]:
    ...
    async def _extract_consolidated_decisions(
        self, text: str, summaries: Dict[SummaryLevel, SummaryResult]
    ) -> List[str]:
    ...
    async def get_available_templates(
        self, meeting_type: Optional[MeetingType] = None
        ...
        if meeting_type:
            templates = [
                template
                for template in templates
                if template.meeting_type == meeting_type
                or template.meeting_type == MeetingType.GENERAL
            ]
    ...
    async def apply_template(
        self, text: str, template_id: str, substitutions: Dict[str, str] = None
    ) -> str:
        ...
        for placeholder in remaining_placeholders:
            result = result.replace(
                f"{{{placeholder}}}", f'[{placeholder.replace("_", " ").title()}]'
            )
    ...
    def _update_stats(
        self, levels: List[SummaryLevel], configuration: SummaryConfiguration
    ):
    ...
    async def get_service_statistics(self) -> Dict[str, Any]:
        ...
        engine_stats = (
            await self.summarization_engine.get_statistics()
