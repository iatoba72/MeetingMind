        ...
        templates = [
            ...
            RuleTemplate(
                ...
                template_data={
                    ...
                    "regex_patterns": [
                        "\\b(revenue|profit|growth)\\s+(impact|opportunity|risk)\\b"
                    ],
...
Focus on actionable business insights.""",
            ...
            RuleTemplate(
                ...
                template_data={
                    ...
                    "regex_patterns": [
                        "\\b(bug|error|issue|problem)\\s+(in|with|on)\\b"
                    ],
...
Provide technical assessment with clear next steps.""",
            ...
            RuleTemplate(
                ...
                template_data={
                    ...
                    "regex_patterns": [
                        "\\b(deadline|milestone|due)\\s+(by|on|for)\\s+\\w+"
                    ],
...
Provide project management insights with timeline implications.""",
    ...
    async def create_session(
        self, user_id: str, session_config: Dict[str, Any] = None
    ) -> str:
        ...
        self.logger.info(
            f"Created insight studio session {session_id} for user {user_id}"
        )

        return session_id

    async def create_custom_rule(
        self, session_id: str, rule_data: Dict[str, Any]
    ) -> str:
        ...
        custom_rule = CustomRule(
            ...
            trigger_type=TriggerType(
                rule_data.get("trigger_type", TriggerType.QUESTION_ASKED.value)
            ),
            priority=TriggerPriority(
                rule_data.get("priority", TriggerPriority.MEDIUM.value)
            ),
            ...
            # Insight generation
            insight_template=rule_data.get(
                "insight_template", "Default template for {trigger_type} insights..."
            ),
            insight_type=InsightType(
                rule_data.get("insight_type", InsightType.SUMMARY.value)
            ),
    ...
    async def update_custom_rule(
        self, session_id: str, rule_id: str, updates: Dict[str, Any]
    ) -> bool:
        ...
        if rule.created_by != session["user_id"]:
            raise PermissionError(
                f"User does not have permission to modify rule {rule_id}"
            )
    ...
    async def test_rule(
        self, session_id: str, rule_id: str, test_data: Dict[str, Any]
    ) -> RuleTest:
        ...
        rule_test = RuleTest(
            ...
            test_name=test_data.get(
                "test_name", f"Test {len(self.rule_tests[rule_id]) + 1}"
            ),
            test_text=test_text,
        ...
        self.logger.info(
            f"Tested rule {rule_id}: {'PASSED' if rule_test.passed else 'FAILED'}"
        )
    ...
    async def _execute_rule_test(self, rule: CustomRule, text: str) -> Dict[str, Any]:
        ...
        # Phrase matching
        phrase_matches = sum(
            1 for phrase in rule.phrases if phrase.lower() in text_lower
        )
        if rule.phrases:
            ...
            if phrase_matches > 0:
                matched_patterns.append(
                    f"phrases: {phrase_matches}/{len(rule.phrases)}"
                )
        ...
        if rule.regex_patterns:
            ...
            if regex_matches > 0:
                matched_patterns.append(
                    f"regex: {regex_matches}/{len(rule.regex_patterns)}"
                )
    ...
    async def create_rule_from_template(
        self, session_id: str, template_id: str, customizations: Dict[str, Any] = None
    ) -> str:
        ...
        rule_data["description"] = customizations.get(
            "description", template.description
        )
        rule_data["category"] = template.category.value
        rule_data["tags"] = customizations.get(
            "tags", [template.category.value, "template-based"]
        )
    ...
    async def delete_custom_rule(self, session_id: str, rule_id: str) -> bool:
        ...
        if rule.created_by != session["user_id"]:
            raise PermissionError(
                f"User does not have permission to delete rule {rule_id}"
            )
    ...
    async def get_rule_analytics(self, rule_id: str) -> Dict[str, Any]:
        ...
        confidences = [
            test.actual_confidence
            for test in tests
            if test.actual_confidence is not None
        ]
        ...
        recent_success_rate = (
            (sum(1 for test in recent_tests if test.passed) / len(recent_tests))
            if recent_tests
            else 0.0
        )
    ...
    def _calculate_performance_trend(self, tests: List[RuleTest]) -> str:
        ...
        recent_success = sum(1 for test in recent_tests if test.passed) / len(
            recent_tests
        )
    ...
    async def get_studio_analytics(self, session_id: str) -> StudioAnalytics:
        ...
        if (
            ...
            and datetime.now() - self.cache_timestamp < timedelta(minutes=5)
        ):
        ...
        top_performing = sorted(all_rules, key=lambda r: r.success_rate, reverse=True)[
            :5
        ]

        # Most used rules
        most_used = sorted(all_rules, key=lambda r: r.usage_count, reverse=True)[:5]
        ...
        avg_user_satisfaction = (
            sum(
                rule.user_feedback_score
                for rule in all_rules
                if rule.user_feedback_score > 0
            )
            / len([rule for rule in all_rules if rule.user_feedback_score > 0])
            if any(rule.user_feedback_score > 0 for rule in all_rules)
            else 0.0
        )
    ...
    async def export_rules(
        self, session_id: str, rule_ids: List[str] = None
    ) -> Dict[str, Any]:
        ...
        if rule_ids:
            rules_to_export = [
                self.custom_rules[rid] for rid in rule_ids if rid in self.custom_rules
            ]
        else:
        ...
        self.logger.info(
            f"Exported {len(rules_to_export)} rules for user {session['user_id']}"
        )

        return export_data

    async def import_rules(
        self, session_id: str, import_data: Dict[str, Any]
    ) -> List[str]:
        ...
        self.logger.info(
            f"Imported {len(imported_rule_ids)} rules for user {session['user_id']}"
        )
    ...
    async def get_session_status(self, session_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_rules": len(
                [r for r in user_rules if r.status == RuleStatus.ACTIVE]
            ),
            "rules_in_testing": len(
                [r for r in user_rules if r.status == RuleStatus.TESTING]
            ),
