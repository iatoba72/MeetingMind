...
class LanguageDetector:
    ...
    async def _detect_with_langdetect(self, text: str) -> Dict[str, Any]:
        ...
        for lang in detected_langs[1:5]:  # Top 5 alternatives
            alternatives.append({"language": lang.lang, "confidence": lang.prob})
    ...
    async def _detect_with_polyglot(self, text: str) -> Dict[str, Any]:
        ...
        for lang in detector.languages[1:5]:  # Top 5 alternatives
            alternatives.append({"language": lang.code, "confidence": lang.confidence})
    ...
    def _ensemble_decision(
        self, results: List[Dict], context: Optional[Dict] = None
    ) -> LanguageDetectionResult:
        ...
        algorithm_weights = {"langdetect": 0.4, "polyglot": 0.35, "statistical": 0.25}
    ...
    def _create_default_result(
        self, lang: str, confidence: float, reason: str
    ) -> LanguageDetectionResult:
    ...
    def _extract_text_features(self, text: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "punctuation_ratio": len([c for c in text if not c.isalnum()])
            / max(len(text), 1),
            "uppercase_ratio": len([c for c in text if c.isupper()])
            / max(len(text), 1),
        }
    ...
    async def detect_code_switching(
        ...
        for i in range(0, len(words) - 5, 3):  # Step by 3 words
            ...
            if i > 0 and switches:
                ...
                if (
                    detection_result.detected_language != prev_lang
                    ...
                    switches.append(
                        {
                            ...
                            "segment": (
                                segment[:50] + "..." if len(segment) > 50 else segment
                            ),
                        }
                    )
...
class MultiLanguageTranscriptionService:
    ...
    async def transcribe_multilingual_audio(
        ...
        simulated_transcript = (
            "Hello everyone, let's begin the meeting. Hola, ¿cómo están todos?"
        )

        # Detect primary language
        primary_detection = await self.language_detector.detect_language(
            simulated_transcript
        )

        # Detect code-switching
        code_switches = await self.code_switching_detector.detect_code_switching(
            simulated_transcript
        )

        # Create segments based on code-switching points
        segments = await self._create_language_segments(
            simulated_transcript, code_switches, participant_id
        )
    ...
    async def _create_language_segments(
        self, text: str, code_switches: List[Dict], participant_id: Optional[str]
    ...
    async def save_multilingual_transcript(
        ...
        for segment in result.segments:
            ...
            language = (
                ...
                .filter(Language.code == segment.language)
                .first()
            )
            ...
            detection_log = LanguageDetectionLog(
                ...
                detection_algorithm=result.processing_metadata.get(
                    "detection_algorithm", "unknown"
                ),
                processing_time_ms=result.processing_metadata.get(
                    "processing_time_ms", 0
                ),
    ...
    async def get_language_statistics(self, meeting_id: str) -> Dict[str, Any]:
        ...
        sorted_languages = sorted(
            language_stats.values(), key=lambda x: x["duration"], reverse=True
        )
        ...
        return {
            ...
            "primary_language": (
                sorted_languages[0]["code"] if sorted_languages else None
            ),
    ...
    async def validate_language_detection(
        ...
        detection_log = (
            self.db.query(LanguageDetectionLog)
            .filter(LanguageDetectionLog.transcript_id == transcript_id)
            .first()
        )
        ...
        if detection_log:
            ...
            transcript = (
                self.db.query(MultiLanguageTranscript)
                .filter(MultiLanguageTranscript.id == transcript_id)
                .first()
            )
