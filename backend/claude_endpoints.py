...
class ClaudeGenerateRequest(BaseModel):
    ...
    template_variables: Optional[Dict[str, str]] = Field(None, description="Variables for template substitution")
...
class DebugInfo(BaseModel):
    ...
    variables_substituted: Dict[str, str] = Field(default_factory=dict, description="Template variables")
    token_analysis: Dict[str, Any] = Field(default_factory=dict, description="Token analysis")
    cost_breakdown: Dict[str, Any] = Field(default_factory=dict, description="Cost breakdown")
    optimization_suggestions: List[str] = Field(default_factory=list, description="Optimization tips")
...
class StreamChunk(BaseModel):
    ...
    usage: Optional[TokenUsage] = Field(None, description="Usage info (only on completion)")
    debug_info: Optional[DebugInfo] = Field(None, description="Debug info (only on completion)")
...
Format the response in clear, professional language suitable for sharing with stakeholders.""",
        variables=[
            TemplateVariable(name="meeting_title", description="Title of the meeting", required=True),
            TemplateVariable(name="meeting_date", description="Date when meeting occurred", required=True),
            TemplateVariable(name="meeting_duration", description="Duration of the meeting", required=True),
            TemplateVariable(name="participants", description="List of meeting participants", required=True),
            TemplateVariable(name="transcript", description="Meeting transcript content", required=True)
        ],
        metadata={
...
Focus on insights that would be valuable for strategic decision-making and long-term planning.""",
        variables=[
            TemplateVariable(name="meeting_title", description="Title of the meeting", required=True),
            TemplateVariable(name="meeting_date", description="Date when meeting occurred", required=True),
            TemplateVariable(name="meeting_type", description="Type of meeting", required=True),
            TemplateVariable(name="participants", description="List of meeting participants", required=True),
            TemplateVariable(name="transcript", description="Meeting transcript content", required=True)
        ],
        metadata={
...
async def generate_with_claude(request: ClaudeGenerateRequest):
    ...
    """
    if request.stream:
        raise HTTPException(status_code=400, detail="Use /claude/generate/stream for streaming responses")
    ...
    if request.template_id and request.template_variables:
        ...
        for var in template.variables:
            ...
            value = request.template_variables.get(var.name, f"[{var.name.upper()}_NOT_PROVIDED]")
    ...
    try:
        ...
        # Calculate costs
        cost_cents = claude_provider.estimate_cost(input_tokens, output_tokens, request.model)
        ...
        debug_info = DebugInfo(
            ...
            token_analysis={
                ...
                "input_ratio": round(input_tokens / total_tokens * 100, 1) if total_tokens > 0 else 0,
                "efficiency_score": calculate_efficiency_score(input_tokens, output_tokens),
                "context_utilization": round(input_tokens / 200000 * 100, 2)  # Based on 200k context window
            },
            cost_breakdown={
                "input_cost_cents": input_cost,
                "output_cost_cents": output_cost,
                "total_cost_cents": cost_cents,
                "cost_per_token": round(cost_cents / total_tokens, 6) if total_tokens > 0 else 0,
                "projected_monthly_cost": cost_cents * 30 * 24  # Assuming hourly usage
            },
            optimization_suggestions=generate_optimization_suggestions(final_prompt, input_tokens, cost_cents),
            performance_metrics={
                "latency_ms": round(latency_ms, 2),
                "tokens_per_second": round(total_tokens / (latency_ms / 1000), 2) if latency_ms > 0 else 0,
                "cost_per_second": round(cost_cents / (latency_ms / 1000), 6) if latency_ms > 0 else 0
            }
        )
    ...
    async def generate_stream():
        ...
        try:
            ...
            for i, word in enumerate(words):
                ...
                current_cost = claude_provider.estimate_cost(input_tokens, current_output_tokens, request.model)
            ...
            total_tokens = input_tokens + output_tokens
            final_cost = claude_provider.estimate_cost(input_tokens, output_tokens, request.model)
            ...
            final_debug_info = DebugInfo(
                ...
                cost_breakdown={
                    ...
                    "input_cost_cents": claude_provider.estimate_cost(input_tokens, 0, request.model),
                    "output_cost_cents": claude_provider.estimate_cost(0, output_tokens, request.model)
                },
                optimization_suggestions=generate_optimization_suggestions(final_prompt, input_tokens, final_cost),
                performance_metrics={
                    "latency_ms": round(latency_ms, 2),
                    "tokens_per_second": round(total_tokens / (latency_ms / 1000), 2) if latency_ms > 0 else 0
                }
...
async def chat_with_claude(request: ClaudeChatRequest):
    ...
    try:
        ...
        debug_info = DebugInfo(
            ...
            optimization_suggestions=generate_chat_optimization_suggestions(request.messages, input_tokens),
            performance_metrics={
                "latency_ms": round(latency_ms, 2),
                "tokens_per_second": round(total_tokens / (latency_ms / 1000), 2) if latency_ms > 0 else 0
            }
...
def generate_optimization_suggestions(prompt: str, token_count: int, cost_cents: float) -> List[str]:
    """Generate optimization suggestions based on prompt analysis"""
    suggestions = []
    
    if cost_cents > 2.0:
        suggestions.append("High cost prompt - consider using Claude Haiku for simple tasks")
    ...
    if not any(role_word in prompt.lower() for role_word in ["you are", "act as", "imagine you"]):
        suggestions.append("Consider starting with a role definition for better responses")
    
    return suggestions

def generate_chat_optimization_suggestions(messages: List[ClaudeMessage], total_tokens: int) -> List[str]:
    """Generate optimization suggestions for chat conversations"""
    suggestions = []
    
    if len(messages) > 20:
        suggestions.append("Long conversation - consider summarizing older messages to reduce cost")
    ...
    if len(user_messages) > 5:
        ...
        if avg_length > 500:
            suggestions.append("Long user messages - consider more concise communication")
