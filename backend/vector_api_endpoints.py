...
async def add_document(
    ...
    try:
        ...
        return AddDocumentResponse(
            document_id=document_id, success=True, message="Document added successfully"
        )
...
async def find_similar_meetings(
    ...
    similarity_threshold: float = Query(
        0.6, description="Minimum similarity threshold"
    ),
    vector_service: VectorStorageService = Depends(get_vector_service),
):
    """
...
async def generate_similarity_matrix(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Similarity matrix generation failed"
        )
...
async def search_documents_by_metadata(
    ...
    try:
        ...
        documents = await vector_service.get_documents_by_metadata(
            metadata_filter=request.metadata_filter, document_types=document_types
        )
    ...
    try:
        ...
        if request.context:
            context = SearchContextInfo(
                ...
                time_range=(
                    tuple(request.context["timeRange"])
                    if request.context.get("timeRange")
                    else None
                ),
                topic_filter=request.context.get("topicFilter"),
                document_types=[
                    DocumentType(dt) for dt in request.context.get("documentTypes", [])
                ],
        ...
        results = await semantic_service.search(
            query=request.query, context=context, limit=request.limit
        )
...
async def get_search_statistics(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Search statistics retrieval failed"
        )
