        ...
        with self._lock:
            ...
            if self.last_timestamp > 0:
                ...
                if gap > self.gap_threshold:
                    ...
                    self._add_samples(
                        silence, self.last_timestamp + self.expected_interval
                    )
...
class StreamVAD:
    ...
    def create_stream_buffer(self, stream_id: str) -> NetworkAudioBuffer:
        ...
        buffer = NetworkAudioBuffer(
            buffer_size_ms=self.config.buffer_size_ms, sample_rate=self.sample_rate
        )
        ...
        if is_speech:
            ...
            if state["current_speech"]:
                ...
                if speech_duration * 1000 >= self.config.min_speech_duration_ms:
                    ...
                    logger.debug(
                        f"Speech segment detected for stream {stream_id}: "
    ...
    def get_stream_statistics(self, stream_id: str) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "average_quality": (
                sum(state["quality_history"]) / len(state["quality_history"])
                if state["quality_history"]
                else 0
            ),
            "buffer_size_ms": buffer.buffer_size_ms if buffer else 0,
            "buffer_fill_level": (
                len(buffer.buffer) / buffer.buffer_size_samples if buffer else 0
            ),
        }
        ...
        if state["speech_segments"]:
            avg_confidence = sum(
                seg["confidence"] for seg in state["speech_segments"]
            ) / len(state["speech_segments"])
    ...
    async def resync_stream(self, stream_id: str, reference_timestamp: float):
        ...
        if buffer.last_timestamp > 0:
            ...
            if time_diff * 1000 > self.config.resync_threshold_ms:
                logger.info(
                    f"Resyncing stream {stream_id}, time diff: {time_diff*1000:.1f}ms"
                )
                buffer.buffer.clear()
