...
class VectorStorageService:
    ...
    def __init__(
        ...
        self.logger.info(
            f"Initialized VectorStorageService with {embedding_model.value} on {self.device}"
        )
    ...
    def _initialize_embedding_model(self):
        ...
        try:
            if self.embedding_model_name.value.startswith("sentence-transformers"):
                model_name = self.embedding_model_name.value.replace(
                ...
                self.logger.info(
                    f"Loaded SentenceTransformer: {model_name} (dim: {self.embedding_dimension})"
                )
                ...
                self.logger.info(
                    f"Configured for OpenAI embeddings: {self.embedding_model_name.value}"
                )
    ...
    def _initialize_collections(self):
        ...
        for doc_type in DocumentType:
            try:
                ...
                self.logger.error(
                    f"Failed to create collection for {doc_type.value}: {e}"
                )
    ...
    async def add_document(
        ...
        try:
            ...
            self.logger.debug(
                f"Added document {document_id} to {document_type.value} collection"
            )
            return document_id
    ...
    def _generate_search_explanation(
        ...
        confidence_level = (
            ...
            else "medium" if similarity_score > 0.6 else "low"
        )
    ...
    async def find_similar_meetings(
        self, meeting_id: str, limit: int = 5, similarity_threshold: float = 0.6
    ) -> List[SearchResult]:
        """
        ...
        try:
            ...
            meeting_content = " ".join(
                [doc.content for doc in meeting_docs[:10]]
            )  # Limit to avoid too long text
    ...
    async def get_documents_by_metadata(
        self, metadata_filter: Dict[str, Any], document_types: List[DocumentType] = None
    ) -> List[VectorDocument]:
    ...
    async def generate_similarity_matrix(
            ...
            for doc_type in document_types:
                try:
                    ...
                    results = collection.peek(
                        limit=max_documents // len(document_types)
                    )
                    ...
                    self.logger.error(
                        f"Failed to get documents from {doc_type.value}: {e}"
                    )
                    continue
        ...
        for i in range(n_clusters):
            cluster_docs = [
                doc_ids[j] for j, label in enumerate(cluster_labels) if label == i
            ]
            cluster_content = [
                documents[j].content
                    ...
                    float(
                        np.mean(
                            [
                                ...
                                for j in range(len(cluster_docs))
                                for k in range(j + 1, len(cluster_docs))
                            ]
                        )
                    )
        ...
        if len(embeddings) > 3:
            ...
            try:
                ...
                dimensionality_reduction["tsne"] = [
                    (float(x), float(y)) for x, y in tsne_coords
                ]
            ...
            try:
                ...
                dimensionality_reduction["umap"] = [
                    (float(x), float(y)) for x, y in umap_coords
                ]
            except (ValueError, RuntimeError, ImportError) as e:
    ...
    async def get_embedding_statistics(self) -> Dict[str, Any]:
        ...
        for doc_type, collection in self.collections.items():
            try:
                ...
                logger.warning(
                    f"Error getting collection count for {doc_type.value}: {e}"
                )
        ...
        return {
            ...
            "avg_embedding_time": self.stats["embedding_generation_time"]
            / max(1, self.stats["total_documents"]),
