...
class TranscriptionQueueManager:
    ...
    def __init__(self, max_workers: int = 2, redis_client: Optional[redis.Redis] = None):
        ...
        logger.info(f"Transcription queue manager initialized with {max_workers} workers")
    ...
    async def enqueue_audio_chunk(
        ...
        estimated_time = await self._estimate_processing_time(len(audio_data), config.model_size)
    ...
    async def get_queue_position(self, chunk_id: str) -> Optional[int]:
        ...
        try:
            ...
            queue_items = await asyncio.get_event_loop().run_in_executor(
                None,
                lambda: self.redis_client.zrange(self.queue_key, 0, -1)
            )
    ...
    async def get_chunk_status(self, chunk_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            for key in [self.queue_key, self.processing_key, self.completed_key, self.failed_key]:
                items = await asyncio.get_event_loop().run_in_executor(
                    None,
                    lambda: self.redis_client.zrange(key, 0, -1) if key == self.queue_key 
    ...
    async def cancel_chunk(self, chunk_id: str) -> bool:
        ...
        try:
            ...
            queue_items = await asyncio.get_event_loop().run_in_executor(
                None,
                lambda: self.redis_client.zrange(self.queue_key, 0, -1, withscores=True)
            )
            ...
            for item_json, score in queue_items:
                ...
                if item_data['id'] == chunk_id:
                    ...
                    await asyncio.get_event_loop().run_in_executor(
                        None,
                        lambda: self.redis_client.zrem(self.queue_key, item_json)
                    )
            ...
            processing_items = await asyncio.get_event_loop().run_in_executor(
                None,
                lambda: self.redis_client.hgetall(self.processing_key)
            )
            
            if chunk_id in processing_items:
                logger.warning(f"Chunk {chunk_id} is currently processing, cannot cancel")
                return False
    ...
    async def get_queue_stats(self) -> QueueStats:
        ...
        try:
            ...
            queue_count = await asyncio.get_event_loop().run_in_executor(
                None,
                lambda: self.redis_client.zcard(self.queue_key)
            )
            
            processing_count = await asyncio.get_event_loop().run_in_executor(
                None,
                lambda: self.redis_client.hlen(self.processing_key)
            )
            
            completed_count = await asyncio.get_event_loop().run_in_executor(
                None,
                lambda: self.redis_client.hlen(self.completed_key)
            )
            
            failed_count = await asyncio.get_event_loop().run_in_executor(
                None,
                lambda: self.redis_client.hlen(self.failed_key)
            )
            ...
            system_load = min(1.0, (processing_count / self.max_workers) if self.max_workers > 0 else 0)
            ...
            return QueueStats(
                ...
                worker_count=len([w for w in self.worker_stats.values() if w['status'] != 'idle']),
            ...
            return QueueStats(
                total_queued=0, processing=0, completed=0, failed=0,
                pending_by_priority={}, average_processing_time=0,
                average_queue_wait_time=0, worker_count=0,
                system_load=0, estimated_wait_time=0
            )
        ...
        while self.is_running:
            try:
                ...
                queue_item = await asyncio.get_event_loop().run_in_executor(
                    None,
                    lambda: self.redis_client.zpopmax(self.queue_key)
                )
                ...
                try:
                    result = await transcription_service.transcribe_audio_chunk(
                        chunk.session_id,
                        chunk.id,
                        chunk.audio_data.encode() if isinstance(chunk.audio_data, str) else chunk.audio_data,
                        chunk.config,
                        chunk.sample_rate
                    )
                    ...
                    if chunk.retry_count < chunk.max_retries:
                        ...
                        priority_score = (chunk.priority.value - 1) * 1000000 + int(time.time())
                        await asyncio.get_event_loop().run_in_executor(
                            None,
                            lambda: self.redis_client.zadd(
                                self.queue_key,
                                {json.dumps(asdict(chunk), default=str): priority_score}
                            )
                        )
                ...
                await asyncio.get_event_loop().run_in_executor(
                    None,
                    lambda: self.redis_client.hdel(self.processing_key, chunk.id)
                )
                ...
                for key in [self.completed_key, self.failed_key]:
                    ...
                    if len(items) > 1000:
                        ...
                        if to_remove:
                            await asyncio.get_event_loop().run_in_executor(
                                None,
                                lambda: self.redis_client.hdel(key, *to_remove)
                            )
    ...
    async def _estimate_processing_time(self, audio_size: int, model_size: WhisperModelSize) -> float:
        ...
        audio_mb = len(audio_size.encode() if isinstance(audio_size, str) else audio_size) / (1024 * 1024)
        ...
        if self.processing_times:
            avg_recent = sum(self.processing_times[-10:]) / len(self.processing_times[-10:])
            base_time = max(base_time, avg_recent / audio_mb) if audio_mb > 0 else base_time
    ...
    async def _estimate_queue_wait_time(self) -> float:
        ...
        try:
            ...
            avg_processing_time = (
                ...
                if self.processing_times else 10.0
            )
            
            active_workers = len([w for w in self.worker_stats.values() if w['status'] != 'idle'])
...
__all__ = ['queue_manager', 'TranscriptionQueueManager', 'QueuePriority', 'ProcessingStatus']