...
from multi_level_summary_service import (
    get_multi_level_summary_service,
    MultiLevelSummaryService,
...
{sender_name}""",
    ...
    async def _extract_meeting_information(
        ...
        extracted_info = {
            ...
            "meeting_date": meeting_metadata.get(
    ...
    async def _generate_structured_email(
        self, extracted_info: Dict[str, Any], configuration: EmailConfiguration
    ) -> EmailContent:
        ...
        main_content = self._generate_main_content(extracted_info, configuration)
    ...
    def _parse_email_body(self, body: str) -> Tuple[str, str, str]:
        ...
        for i, line in enumerate(lines):
            if any(
                word in line.lower() for word in ["hi ", "dear ", "hello ", "greetings"]
            ):
                greeting = line.strip()
                main_start = i + 1
                break
        ...
        for i in range(len(lines) - 1, -1, -1):
            ...
            if any(
                word in line.lower()
                for word in ["best", "regards", "sincerely", "thanks"]
            ):
                closing = line
                main_end = i
                break
    ...
    def _generate_main_content(
        ...
        # Meeting summary
        if configuration.email_type in [
            EmailType.FOLLOW_UP,
            EmailType.MEETING_RECAP,
            EmailType.TEAM_SUMMARY,
        ]:
        ...
        if next_steps and configuration.email_type in [
            EmailType.FOLLOW_UP,
            EmailType.NEXT_STEPS,
        ]:
    ...
    async def _calculate_email_quality(
        self, email_content: EmailContent, configuration: EmailConfiguration
    ) -> Dict[str, float]:
    ...
    def _calculate_professionalism_score(self, text: str, tone: EmailTone) -> float:
        ...
        unprofessional_count = sum(
            1 for word in unprofessional_words if word in text_lower
        )
    ...
    def _update_stats(
        self, configuration: EmailConfiguration, template_id: Optional[str]
    ):
