...
class BaseAIProvider(ABC):
    ...
    def update_usage_metrics(
        ...
        self.usage_metrics.average_latency_ms = (
            self.usage_metrics.average_latency_ms * (total_requests - 1) + latency_ms
        ) / total_requests
...
class AnthropicProvider(BaseAIProvider):
    ...
    def estimate_cost(
        ...
        model_pricing = pricing.get(
            model or "claude-3-5-sonnet-20241022", pricing["claude-3-5-sonnet-20241022"]
        )
...
class AIProviderRegistry:
    ...
    async def _start_health_monitoring(
        self, provider_id: str, health_config: Dict[str, Any]
    ):
                ...
                try:
                    ...
                    try:
                        ...
                        health_result = await asyncio.wait_for(
                            provider.health_check(), timeout=timeout
                        )
                        ...
                        if health_result:
                            ...
                            logger.debug(
                                f"Health check passed for {provider_id}: {response_time_ms:.1f}ms"
                            )
                        else:
                        ...
                        provider.update_health_metrics(
                            ProviderStatus.UNHEALTHY, 0, str(health_error)
                        )
                        logger.error(
                            f"Health check error for {provider_id}: {health_error}"
                        )

                    # Check if provider should be marked as critically unhealthy
                    if (
                        provider.health_metrics.consecutive_failures
                        >= failure_threshold
                    ):
                        logger.critical(
                            f"Provider {provider_id} has {provider.health_metrics.consecutive_failures} consecutive failures"
                        )
                        ...
                        if (
                            ...
                            logger.critical(
                                f"Disabling provider {provider_id} due to excessive failures"
                            )
        ...
        logger.info(
            f"Started health monitoring for {provider_id} (interval: {interval}s, timeout: {timeout}s)"
        )
    ...
    async def get_provider(self, provider_id: str = None) -> Optional[BaseAIProvider]:
        ...
        for pid, provider in self.providers.items():
            ...
            if config.get("enabled", True) and provider.health_metrics.status in [
    ...
    async def generate_text(
        self, prompt: str, provider_id: str = None, **kwargs
    ) -> str:
    ...
    async def chat_completion(
        self, messages: List[Dict[str, str]], provider_id: str = None, **kwargs
    ) -> Dict[str, Any]:
    ...
    def get_usage_summary(self) -> Dict[str, Any]:
        ...
        total_requests = sum(
            p.usage_metrics.total_requests for p in self.providers.values()
        )
        total_cost = sum(
            p.usage_metrics.total_cost_cents for p in self.providers.values()
        )
        total_tokens = sum(
    ...
    async def test_provider(
        self, provider_id: str, test_prompt: str = "Hello, how are you?"
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "cost_cents": provider.estimate_cost(
                    len(test_prompt.split()), len(response.split())
                ),
            }
