...
from .obs_integration import (
    OBSWebSocketClient,
    OBSAutomationManager,
    OBSStatsMonitor,
    OBSConnectionRequest,
    OBSSceneSwitchRequest,
    OBSSourceControlRequest,
    OBSAutomationRuleRequest,
    OBSStatsResponse,
    get_obs_client,
    get_obs_automation,
    get_obs_monitor,
...
async def connect_obs(
    connection_data: OBSConnectionRequest, background_tasks: BackgroundTasks
):
    ...
    try:
        ...
        if success:
            ...
            return {
                ...
                "version": version,
                "scenes": [
                    {"name": scene.scene_name, "uuid": scene.scene_uuid}
                    for scene in scenes
                ],
    ...
    try:
        ...
        if success:
            ...
            if scene_data.meeting_id:
                logger.info(
                    f"Scene switched to '{scene_data.scene_name}' for meeting {scene_data.meeting_id}"
                )
...
async def get_obs_stats():
    ...
    try:
        ...
        return OBSStatsResponse(
            stats=stats.__dict__, alerts=alerts, timestamp=datetime.now()
        )
...
async def get_automation_rules():
    ...
    try:
        ...
        return {"rules": automation.automation_rules, "enabled": automation.enabled}
...
async def trigger_automation(
    event_type: str, meeting_id: str, event_data: Optional[Dict[str, Any]] = None
):
    ...
    try:
        ...
        return {"success": True, "event_type": event_type, "meeting_id": meeting_id}
...
async def start_meeting_streaming(meeting_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Trigger automation
        automation = await get_obs_automation()
        await automation.handle_meeting_event(
            "meeting_started",
            meeting_id,
            {
                "meeting_title": meeting.title,
                "participants": (
                    len(meeting.participants) if hasattr(meeting, "participants") else 0
                ),
            },
        )
...
async def stop_meeting_streaming(meeting_id: str, db: Session = Depends(get_db)):
