...
class BaseCRUD:
    ...
    def update(
        self, db: Session, *, db_obj: Any, obj_in: Union[BaseModel, Dict[str, Any]]
    ) -> Any:
        ...
        try:
            ...
            if "unique constraint" in str(e).lower() or "duplicate" in str(e).lower():
                raise UniqueConstraintError(
                    f"Update conflicts with existing data: {str(e)}"
                )
            elif "foreign key" in str(e).lower():
                raise ForeignKeyError(f"Referenced entity not found: {str(e)}")
            elif "not null" in str(e).lower() or "check constraint" in str(e).lower():
    ...
    def delete(self, db: Session, *, id: Union[str, uuid.UUID]) -> bool:
        ...
        try:
            ...
            if "foreign key" in str(e).lower():
                raise ConflictError(
                    f"Cannot delete: Resource is referenced by other entities: {str(e)}"
                )
            else:
                raise CRUDError(f"Database error: {str(e)}")
...
class MeetingCRUD(BaseCRUD):
    ...
    def get_with_relationships(
        self, db: Session, id: Union[str, uuid.UUID]
    ) -> Optional[Meeting]:
    ...
    def get_filtered(
        self, db: Session, filters: MeetingFilters, pagination: PaginationParams
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if filters.status:
                query = query.filter(
                    Meeting.status.in_([s.value for s in filters.status])
                )
            ...
            if filters.template_id:
                query = query.filter(
                    Meeting.template_id == uuid.UUID(filters.template_id)
                )
            ...
            if filters.tag_ids:
                ...
                query = query.join(meeting_tags).filter(
                    meeting_tags.c.tag_id.in_(tag_uuids)
                )
    ...
    def create_with_participants(
        ...
        try:
            ...
            if (
                ...
                and "meeting_number" in str(e).lower()
            ):
                raise UniqueConstraintError(
                    f"Meeting number conflict - please retry: {str(e)}"
                )
            elif "foreign key" in str(e).lower() and "tag" in str(e).lower():
                raise ForeignKeyError(
                    f"One or more specified tags do not exist: {str(e)}"
                )
            elif "foreign key" in str(e).lower() and "template" in str(e).lower():
                raise ForeignKeyError(
                    f"Specified meeting template does not exist: {str(e)}"
                )
            elif "foreign key" in str(e).lower():
                raise ForeignKeyError(f"Referenced entity not found: {str(e)}")
    ...
    def update_status(
        self, db: Session, meeting_id: Union[str, uuid.UUID], new_status: MeetingStatus
    ) -> Optional[Meeting]:
        ...
        try:
            ...
            logger.info(
                f"Meeting {meeting_id} status changed from {old_status} to {new_status}"
            )
            return meeting
    ...
    def get_upcoming_meetings(
        self, db: Session, user_id: str, hours_ahead: int = 24
    ) -> List[Meeting]:
    ...
    def get_meeting_statistics(
        self, db: Session, meeting_id: Union[str, uuid.UUID]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            speaking_times = [
                (p.display_name, p.speaking_time_seconds) for p in meeting.participants
            ]
            speaking_times.sort(key=lambda x: x[1], reverse=True)

            # Calculate engagement metrics
            avg_engagement = sum(
                p.engagement_score for p in meeting.participants
            ...
            for insight_type in InsightType:
                count = len(
                    [i for i in meeting.insights if i.insight_type == insight_type]
                )
            ...
            return {
                ...
                "timing": {
                    ...
                    "actual_start": (
                        meeting.actual_start.isoformat()
            ...
            if "timeout" in str(e).lower():
                raise DatabaseTimeoutError(
                    f"Statistics calculation timeout - meeting may have too much data: {str(e)}"
                )
            elif "connection" in str(e).lower():
                raise DatabaseConnectionError(f"Database connection failed: {str(e)}")
            else:
    ...
    def _generate_meeting_number(self, db: Session) -> str:
        ...
        pattern = f"{today}-%"
        result = (
            db.query(func.max(Meeting.meeting_number))
            .filter(Meeting.meeting_number.like(pattern))
            .scalar()
        )
...
class ParticipantCRUD(BaseCRUD):
    ...
    def get_by_meeting_and_email(
        self, db: Session, meeting_id: Union[str, uuid.UUID], email: str
        ...
        try:
            ...
            return (
                ...
                .filter(
                    and_(
                        Participant.meeting_id == meeting_id, Participant.email == email
                    )
                )
                .first()
            )
...
class TranscriptCRUD(BaseCRUD):
    ...
    def create_bulk(
        self, db: Session, transcripts: List[TranscriptCreate]
    ) -> List[Transcript]:
        ...
        try:
            ...
            if "unique constraint" in str(e).lower():
                raise UniqueConstraintError(
                    f"Duplicate transcript entries detected: {str(e)}"
                )
            elif "foreign key" in str(e).lower() and "meeting" in str(e).lower():
                raise ForeignKeyError(
                    f"Invalid meeting ID in transcript data: {str(e)}"
                )
            elif "foreign key" in str(e).lower() and "participant" in str(e).lower():
                raise ForeignKeyError(
                    f"Invalid participant ID in transcript data: {str(e)}"
                )
            elif "foreign key" in str(e).lower():
                raise ForeignKeyError(f"Referenced entity not found: {str(e)}")
            elif "check constraint" in str(e).lower():
    ...
    def get_by_meeting_time_range(
        ...
        try:
            ...
            return (
                ...
                .order_by(Transcript.start_time_seconds)
                .all()
            )
...
class AIInsightCRUD(BaseCRUD):
    ...
    def get_by_meeting_and_type(
        self, db: Session, meeting_id: Union[str, uuid.UUID], insight_type: InsightType
    ) -> List[AIInsight]:
