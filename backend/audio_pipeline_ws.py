...
class AudioPipelineManager:
    ...
    def __init__(self):
        ...
        self.client_sources: Dict[str, List[str]] = {}  # client_id -> list of source_ids
    ...
    async def _handle_start_network_source(self, client_id: str, message: Dict[str, Any]):
        ...
        logger.info(f"Starting network source {source_id} for client {client_id}: {protocol}")
        
        # Determine source type
        source_type = AudioSourceType.NETWORK_RTMP if protocol == "rtmp" else AudioSourceType.NETWORK_SRT
    ...
    async def _handle_stop_network_source(self, client_id: str, message: Dict[str, Any]):
    ...
    async def _handle_audio_chunks(self, client_id: str, message: Dict[str, Any]):
        ...
        for chunk_data in chunks:
            try:
                ...
                await self._send_error_to_client(client_id, f"Chunk processing failed: {e}")
    ...
    async def _handle_get_source_status(self, client_id: str, message: Dict[str, Any]):
        ...
        for stream_id, stream_info in active_streams.items():
            sources_status.append({
                ...
                "last_packet_at": stream_info.last_packet_at.isoformat() if stream_info.last_packet_at else None
            })
    ...
    async def _handle_update_source_config(self, client_id: str, message: Dict[str, Any]):
    ...
    async def _handle_register_local_source(self, client_id: str, message: Dict[str, Any]):
        ...
        # Map source type
        source_type = AudioSourceType.MICROPHONE if source_type_str == "microphone" else AudioSourceType.SYSTEM
        
        logger.info(f"Registering local source {source_id} ({source_type_str}) for client {client_id}")
