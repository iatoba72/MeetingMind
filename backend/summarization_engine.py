...
class SummarizationEngine:
    ...
    async def generate_summary(self, request: SummarizationRequest) -> SummaryResult:
        ...
        try:
            ...
            # Extract key points
            key_points = await self._extract_key_points(processed_text, request)
            ...
            result = SummaryResult(
                ...
                metadata={
                    "meeting_type": (
                        request.meeting_type.value if request.meeting_type else None
                    ),
    ...
    async def _extractive_tfidf_summary(
        self, sentences: List[str], request: SummarizationRequest
    ) -> str:
        ...
        for i, sentence in enumerate(sentences):
            ...
            similarity_score = 1.0 - np.mean(
                similarity_scores
            )  # Prefer unique sentences
    ...
    async def _abstractive_ai_summary(
        self, text: str, request: SummarizationRequest
    ) -> str:
    ...
    async def _hybrid_summary(
        self, sentences: List[str], text: str, request: SummarizationRequest
    ) -> str:
    ...
    async def _template_based_summary(
        ...
        if request.custom_template:
            ...
            return self._apply_meeting_type_template(
                text, request.meeting_type, request
            )
        else:
    ...
    def _apply_custom_template(
        self, text: str, template: str, request: SummarizationRequest
    ) -> str:
        ...
        for placeholder in placeholders:
            if placeholder == "key_points":
                ...
                substitutions[placeholder] = "\n".join(
                    [f"• {decision}" for decision in decisions[:3]]
                )
                ...
                date = request.meeting_metadata.get(
    ...
    def _apply_meeting_type_template(
        self, text: str, meeting_type: MeetingType, request: SummarizationRequest
    ) -> str:
    ...
    async def _extract_key_points(
        ...
        if request.focus_keywords:
            keyword_points = self._extract_keyword_focused_points(
                sentences, request.focus_keywords
            )
    ...
    def _extract_keyword_focused_points(
        ...
        for sentence in sentences:
            keyword_count = sum(
                1 for keyword in keywords if keyword.lower() in sentence.lower()
            )
    ...
    def _remove_duplicate_points(
        self, key_points: List[ExtractiveKeyPoint]
    ) -> List[ExtractiveKeyPoint]:
        ...
        for point in key_points:
            ...
            for seen_text in seen_texts:
                ...
                common_words = set(point.text.lower().split()) & set(
                    seen_text.lower().split()
                )
                similarity = len(common_words) / max(
                    len(point.text.split()), len(seen_text.split())
                )
    ...
    async def _calculate_quality_metrics(
        self, summary: str, original_text: str, request: SummarizationRequest
    ...
    def _calculate_coherence(self, text: str) -> float:
        ...
        for i in range(len(sentences) - 1):
            words1 = set(
                word.lower() for word in word_tokenize(sentences[i]) if word.isalpha()
            )
            words2 = set(
                word.lower()
                for word in word_tokenize(sentences[i + 1])
                if word.isalpha()
            )
    ...
    def _calculate_accuracy(
        self, summary: str, original_text: str, focus_keywords: List[str]
    ) -> float:
        """Calculate accuracy based on information preservation"""

        # Simple accuracy based on important word preservation
        original_words = set(
            word.lower() for word in word_tokenize(original_text) if word.isalpha()
        )
        summary_words = set(
            word.lower() for word in word_tokenize(summary) if word.isalpha()
        )
        ...
        important_words = max(
            10, len(original_words) // 10
        )  # Consider top 10% as important
    ...
    def _calculate_relevance(self, summary: str, focus_keywords: List[str]) -> float:
        ...
        covered_keywords = sum(
            1 for keyword in focus_keywords if keyword.lower() in summary_lower
        )
    ...
    def _calculate_confidence_score(
        self, quality_metrics: Dict[QualityMetric, float], request: SummarizationRequest
    ) -> float:
    ...
    def _get_target_sentences(
        self, summary_type: SummaryType, total_sentences: int
    ) -> int:
    ...
    def _calculate_keyword_score(self, sentence: str, keywords: List[str]) -> float:
        ...
        keyword_matches = sum(
            1 for keyword in keywords if keyword.lower() in sentence_lower
        )
...
{next_steps}"""
    ...
    def _generate_general_meeting_summary(
        self, text: str, request: SummarizationRequest
    ) -> str:
        ...
        try:
            return loop.run_until_complete(
                self._extractive_tfidf_summary(sentences, request)
            )
        finally:
            loop.close()
    ...
    def _generate_executive_summary(
        self, sentences: List[str], topics: List[str], request: SummarizationRequest
    ) -> str:
        ...
        if key_points:
            summary_parts.append(
                f"This meeting focused on {', '.join(topics[:3])} with key discussions around {key_points[0].lower()}."
            )
