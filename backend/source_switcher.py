...
class SourceSwitcher:
    ...
    def register_source(self, source_info: AudioSourceInfo) -> bool:
        ...
        try:
            ...
            logger.info(f"Registered source: {source_info.name} ({source_info.source_type.value})")
    ...
    def update_source_metrics(self, source_id: str, metrics: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if (self.config.auto_switch_enabled and 
                source_id == self.active_source_id and 
                source.quality_score < self.config.quality_threshold):
                asyncio.create_task(self._evaluate_sources())
    ...
    async def switch_to_source(self, source_id: str, manual: bool = False) -> bool:
        ...
        try:
            ...
            if (self.last_switch_time and 
                (datetime.now() - self.last_switch_time).total_seconds() < self.config.switch_cooldown_seconds):
                logger.warning(f"Switch cooldown active, ignoring switch request")
                return False
            
            logger.info(f"Switching to source: {source.name} ({'manual' if manual else 'automatic'})")
            ...
            if await self._activate_source(source_id):
                ...
                self.stats['switch_history'].append({
                    ...
                    'from_source': self.active_source_id if self.active_source_id != source_id else None,
                    'to_source': source_id,
        ...
        while True:
            try:
                ...
                for source_id, source in self.sources.items():
                    ...
                    if time_since_activity > timedelta(seconds=self.config.fallback_timeout_seconds):
                        if source.state in [SourceState.AVAILABLE, SourceState.ACTIVE]:
                            logger.warning(f"Source {source.name} appears stale, marking as unavailable")
    ...
    def _find_best_source(self) -> Optional[str]:
        ...
        try:
            available_sources = {
                sid: source for sid, source in self.sources.items()
                if (source.state == SourceState.AVAILABLE and 
                    sid not in self.config.blacklisted_sources)
            }
    ...
    def _find_best_by_priority(self, sources: Dict[str, AudioSourceInfo]) -> Optional[str]:
        """Find source with highest priority"""
        return min(sources.keys(), key=lambda sid: sources[sid].priority)
    
    def _find_best_by_quality(self, sources: Dict[str, AudioSourceInfo]) -> Optional[str]:
        """Find source with highest quality score"""
        return max(sources.keys(), key=lambda sid: sources[sid].quality_score)
    
    def _find_best_by_combined_score(self, sources: Dict[str, AudioSourceInfo]) -> Optional[str]:
        ...
        def combined_score(source: AudioSourceInfo) -> float:
            ...
            type_weight = self.config.priority_weights.get(source.source_type.value, 0.5)
    ...
    async def _should_switch_to_source(self, source_id: str) -> bool:
        ...
        try:
            ...
            if (self.config.preferred_sources and 
                self.active_source_id in self.config.preferred_sources and
                current_source.quality_score >= self.config.quality_threshold):
                return False
            ...
            if self.config.sticky_switching:
                ...
                if new_source.quality_score < current_source.quality_score + improvement_threshold:
                    return False
            ...
            logger.error(f"Error determining if should switch to source {source_id}: {e}")
            return False
    ...
    def _calculate_quality_score(self, source: AudioSourceInfo) -> float:
        ...
        try:
            ...
            if source.signal_to_noise_ratio >= self.config.min_signal_to_noise_ratio:
                score *= 1.1
            elif source.signal_to_noise_ratio < self.config.min_signal_to_noise_ratio / 2:
                score *= 0.6
            
            # Activity recency bonus
            time_since_activity = (datetime.now() - source.last_activity).total_seconds()
            ...
            logger.error(f"Error calculating quality score for source {source.source_id}: {e}")
