...
class PluginContext:
    ...
    def require_capability(self, capability: PluginCapability):
        ...
        if not self.has_capability(capability):
            raise PermissionError(
                f"Plugin {self.plugin_id} lacks required capability: {capability.value}"
            )
    ...
    def emit_event(
        self, event_type: Union[PluginEventType, str], data: Dict[str, Any] = None
    ):
        """Emit custom event"""
        if self._event_emitter:
            ...
            event = PluginEvent(
                event_type=event_type, source=self.plugin_id, data=data or {}
            )
...
def plugin_hook(
    event_type: PluginEventType, priority: PluginPriority = PluginPriority.NORMAL
):
    ...
    def get_plugin_info(self, plugin_id: str) -> Optional[Dict[str, Any]]:
        ...
        if plugin:
            return {
                ...
                "capabilities": [
                    cap.value for cap in self._contexts[plugin_id].capabilities
                ],
            }
        return None
...
def create_meeting_event(
    event_type: PluginEventType, meeting_data: Dict[str, Any], user_id: str = None
) -> PluginEvent:
...
def create_transcription_event(
    event_type: PluginEventType, transcript_data: Dict[str, Any], meeting_id: str
) -> PluginEvent:
    """Create transcription-related event"""
    return PluginEvent(
        event_type=event_type, data=transcript_data, meeting_id=meeting_id
    )


def create_user_event(
    event_type: PluginEventType, user_data: Dict[str, Any]
) -> PluginEvent:
    """Create user-related event"""
    return PluginEvent(
        event_type=event_type, data=user_data, user_id=user_data.get("id")
    )
