...
class GoogleCalendarProvider(CalendarProvider):
    ...
    async def create_event(self, event: CalendarEvent) -> str:
        ...
        google_event = {
            ...
            "reminders": {
                ...
                "overrides": [
                    {
                        "method": "email",
                        "minutes": self.config.get("reminder_minutes", 15),
                    },
        ...
        if self.config.get("mock_mode", True):
            ...
            print(
                f"Google Calendar: Created mock event '{event.title}' with ID: {event_id}"
            )
            return event_id
        else:
            raise NotImplementedError(
                "Real Google Calendar API integration not available"
            )
...
class OutlookCalendarProvider(CalendarProvider):
    ...
    async def authenticate(self) -> bool:
        ...
        try:
            ...
            if not (client_id and client_secret and tenant_id):
                print(
                    "Outlook Calendar: Missing required credentials (client_id, client_secret, tenant_id)"
                )
                return False
    ...
    async def create_event(self, event: CalendarEvent) -> str:
        ...
        if self.config.get("mock_mode", True):
            ...
            print(
                f"Outlook Calendar: Created mock event '{event.title}' with ID: {event_id}"
            )
            return event_id
        else:
            raise NotImplementedError(
                "Real Outlook Calendar API integration not available"
            )
...
class CalendarIntegrationPlugin(BasePlugin):
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.context.log(
                "info", "Calendar Integration plugin initialized successfully"
            )
            return True
        except Exception as e:
            self.context.log(
                "error", f"Failed to initialize Calendar Integration plugin: {e}"
            )
            return False
    ...
    async def on_meeting_created(self, event: PluginEvent) -> PluginResult:
        ...
        try:
            ...
            self.context.log(
                "info", f"Processing meeting creation for calendar sync: {meeting_id}"
            )
    ...
    async def _sync_meeting_to_calendar(self, sync_item: Dict[str, Any]):
        ...
        try:
            ...
            provider_event_id = await self.calendar_provider.create_event(
                calendar_event
            )
            ...
            if provider_event_id:
                ...
                self.context.log(
                    "info", f"Created calendar event for meeting {meeting_id}"
                )
                ...
                self.context.emit_event(
                    "calendar.event_created",
                    {
                        "meeting_id": meeting_id,
                        "provider_event_id": provider_event_id,
                        "calendar_provider": self.context.get_config(
                            "calendar_provider"
                        ),
                    },
                )
                ...
                self.context.log(
                    "error", f"Failed to create calendar event for meeting {meeting_id}"
                )

        except Exception as e:
            self.context.log(
                "error", f"Failed to sync meeting {meeting_id} to calendar: {e}"
            )
    ...
    async def _update_calendar_event(self, sync_item: Dict[str, Any]):
        ...
        try:
            ...
            # Update in calendar
            success = await self.calendar_provider.update_event(
                provider_event_id, updated_event
            )
            ...
            if success:
                ...
                self.context.log(
                    "info", f"Updated calendar event for meeting {meeting_id}"
                )

                # Emit custom event
                self.context.emit_event(
                    "calendar.event_updated",
                    {"meeting_id": meeting_id, "provider_event_id": provider_event_id},
                )
            else:
                self.context.log(
                    "error", f"Failed to update calendar event for meeting {meeting_id}"
                )

        except Exception as e:
            self.context.log(
                "error",
                f"Failed to update calendar event for meeting {meeting_id}: {e}",
            )
        ...
        try:
            if meeting_id in self.event_cache:
                ...
                await self.calendar_provider.update_event(
                    provider_event_id, calendar_event
                )

                self.context.log(
                    "info", f"Finalized calendar event for meeting {meeting_id}"
                )

        except Exception as e:
            self.context.log(
                "error",
                f"Failed to finalize calendar event for meeting {meeting_id}: {e}",
            )

    def _create_calendar_event_from_meeting(
    ...
    async def _perform_periodic_sync(self):
        ...
        try:
            ...
            self.context.emit_event(
                ...
                {"sync_mode": sync_mode, "timestamp": datetime.utcnow().isoformat()},
            )
    ...
    async def _sync_calendar_to_meetingmind(self):
        ...
        try:
            ...
            calendar_events = await self.calendar_provider.get_events(
                start_date, end_date
            )
    ...
    async def manual_sync(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if meeting_id:
                ...
                # Periodic sync
                await self.sync_queue.put({"type": "periodic_sync"})
    ...
    async def get_calendar_events(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            start_date = datetime.fromisoformat(
                request_data.get("start_date", datetime.utcnow().isoformat())
            )
