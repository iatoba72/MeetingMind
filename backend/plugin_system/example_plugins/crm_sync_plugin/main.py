...
class CRMProvider:
    ...
    async def update_contact(
        self, crm_id: str, contact_data: Dict[str, Any]
    ) -> CRMContact:
...
class SalesforceProvider(CRMProvider):
    ...
    async def add_activity(
        ...
        task_data = {
            ...
            "ActivityDate": activity_data.get(
...
class CRMSyncPlugin(BasePlugin):
    ...
    async def on_meeting_ended(self, event: PluginEvent) -> PluginResult:
        ...
        try:
            ...
            self.context.log(
                "info", f"Processing meeting end for CRM sync: {meeting_id}"
            )
    ...
    async def on_action_item_created(self, event: PluginEvent) -> PluginResult:
        ...
        try:
            ...
            await self.sync_queue.put(
                {"type": "action_item_created", "action_item_data": action_item_data}
            )
        ...
        try:
            ...
            for participant in participants:
                ...
                if not contact:
                    ...
                    self.context.log(
                        "info", f"Created new CRM contact: {contact.email}"
                    )
            ...
            self.context.log(
                "error", f"Failed to sync meeting {meeting_id} to CRM: {e}"
            )
                ...
                for participant in participants:
                    ...
                    if email:
                        ...
                        if contact:
                            ...
                            await self.crm_provider.add_activity(
                                contact.crm_id, activity_data
                            )
                            self.context.log(
                                "info", f"Synced insight to CRM contact: {email}"
                            )
    ...
    async def _sync_action_item_to_crm(self, sync_item: Dict[str, Any]):
        ...
        try:
            ...
            if assignee_email:
                assignee_contact = await self.crm_provider.get_contact_by_email(
                    assignee_email
                )
            ...
            if assignee_contact:
                ...
                self.context.log(
                    "info", f"Synced action item to CRM contact: {assignee_email}"
                )
            else:
                # If no assignee contact found, log warning
                self.context.log(
                    "warning", f"No CRM contact found for assignee: {assignee_email}"
                )
                ...
                for participant in participants[
                    :1
                ]:  # Just first participant as fallback
                    email = participant.get("email")
                    if email:
                        contact = await self.crm_provider.get_contact_by_email(email)
                        if contact:
                            ...
                            await self.crm_provider.add_activity(
                                contact.crm_id, task_data
                            )
                            self.context.log(
                                "info",
                                f"Synced unassigned action item to participant: {email}",
                            )
                            break
    ...
    async def _create_opportunity_from_meeting(self, meeting_data: Dict[str, Any]):
        ...
        try:
            ...
            has_opportunity_indicators = any(
                keyword in content_text for keyword in opportunity_keywords
            )
            ...
            if not has_opportunity_indicators:
                self.context.log(
                    "info", f"Meeting {meeting_id} does not appear to be sales-related"
                )
                return
            ...
            if not primary_contact:
                self.context.log(
                    "warning", f"No CRM contacts found for meeting participants"
                )
                return

            # Extract opportunity details from meeting
            opportunity_name = (
                f"{meeting_title} - {primary_contact.company or primary_contact.name}"
            )
            ...
            self.context.log(
                "info",
                f"Created CRM opportunity: {opportunity.name} (ID: {opportunity.crm_id})",
            )
    ...
    async def manual_sync(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        await self.sync_queue.put({"type": "manual_sync", "meeting_id": meeting_id})
