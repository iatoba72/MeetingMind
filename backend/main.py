...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, client_id: str) -> str:
        ...
        logger.info(
            "Client %s connected with connection ID: %s", client_id, connection_id
        )
    ...
    def disconnect(self, connection_id: str):
        ...
        if connection_id in self.active_connections:
            ...
            logger.info(
                "Client %s disconnected (Connection ID: %s)", client_id, connection_id
            )
        ...
        for connection_id, connection_info in self.active_connections.items():
            if connection_id != exclude_connection_id:
                try:
                    ...
                    print(
                        f"❌ Error broadcasting to {connection_info['client_id']}: {e}"
                    )
...
async def health_check():
    ...
    try:
        ...
        raise safe_http_exception(
            503, "internal_error", "Service temporarily unavailable"
        )
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                if message is None:
                    logger.warning(
                        f"Invalid message from {client_id}, closing connection"
                    )
                ...
                logger.debug(
                    "Received %s from %s: %s", message_type, client_id, message_data
                )
                ...
                if message_type == "chat_message":
                    ...
                    chat_broadcast = {
                        ...
                        "data": {
                            ...
                            "sender_name": sanitize_string(
                                message_data.get("sender_name", client_id), 100
                            ),
                    ...
                    await manager.broadcast_except(
                        json.dumps(typing_broadcast), connection_id
                    )
                    ...
                    try:
                        ...
                        audio_session_id = await audio_processor.create_session(
                            client_id, audio_config
                        )
                        ...
                        await manager.send_personal_message(
                            json.dumps(session_response), websocket
                        )
                    ...
                    if audio_session_id:
                        result = await audio_processor.process_chunk_metadata(
                            audio_session_id, message_data
                        )
                        ...
                        await manager.send_personal_message(
                            json.dumps(metadata_response), websocket
                        )
                    else:
                        error_response = {
                            "type": "error",
                    ...
                    await manager.send_personal_message(
                        json.dumps(echo_response), websocket
                    )
                ...
                error_response = {
                    "type": "error",
                    "data": {"error": "Message processing error", "details": str(e)},
                    "timestamp": datetime.now().isoformat(),
                }
                await manager.send_personal_message(
                    json.dumps(error_response), websocket
...
async def audio_pipeline_websocket_endpoint(
    websocket: WebSocket, client_id: str = None
):
    """
...
async def load_transcription_model(model_size: str):
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to load model {model_size}"
            )
...
async def process_audio_chunk_sync(
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail="Invalid model size or priority"
            )
...
async def enqueue_audio_chunk(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to enqueue chunk: {str(e)}"
        )
...
async def get_chunk_status(chunk_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get chunk status: {str(e)}"
        )
...
async def cancel_chunk(chunk_id: str):
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=404, detail="Chunk not found or cannot be cancelled"
            )
...
async def get_session_metrics(session_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get session metrics: {str(e)}"
        )
...
async def _get_system_recommendations(system_info: Dict[str, Any]) -> List[str]:
    ...
    if gpu_memory > 0 and gpu_memory < 4:
        recommendations.append(
            "Limited GPU memory detected, consider using smaller models (tiny/base)"
        )
    elif gpu_memory >= 8:
...
async def get_cloud_providers():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get provider stats: {str(e)}"
        )
    ...
    try:
        ...
        return {
            ...
            "result": (
                {
                    "full_text": (
                        job.final_result.full_text if job.final_result else None
                    ),
                    "language": job.final_result.language if job.final_result else None,
                    "confidence": (
                        job.final_result.metrics.confidence_score
                        if job.final_result
                        else 0.0
                    ),
                }
        ...
        raise HTTPException(
            status_code=500, detail=f"Cloud transcription failed: {str(e)}"
        )
    ...
    try:
        ...
        for provider_str in providers:
            try:
                ...
                raise HTTPException(
                    status_code=400, detail=f"Unknown provider: {provider_str}"
                )
...
async def get_battle_status(job_id: str):
    ...
    try:
            ...
            attempts.append(
                {
                    ...
                    "result": (
                        {
                            "full_text": (
                                attempt.result.full_text if attempt.result else None
                            ),
                            "segments": (
                                len(attempt.result.segments) if attempt.result else 0
                            ),
                            "metrics": (
                                {
                                    ...
                                    "word_count": (
                                        len(attempt.result.full_text.split())
                                        if attempt.result and attempt.result.full_text
                                        else 0
                                    ),
                                }
                                if attempt.result
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get battle status: {str(e)}"
        )
    ...
    try:
        ...
        for attempt in job.attempts:
            if attempt.success and attempt.result:
                provider_metrics = comparison_result.provider_results.get(
                    attempt.provider
                )
                ...
                results.append(
                    {
                        ...
                        "metrics": {
                            "word_error_rate": (
                                provider_metrics.word_error_rate
                                if provider_metrics
                                else 0.0
                            ),
                            "character_error_rate": (
                                provider_metrics.character_error_rate
                                if provider_metrics
                                else 0.0
                            ),
                            "bleu_score": (
                                provider_metrics.bleu_score if provider_metrics else 0.0
                            ),
                            "confidence_score": attempt.result.metrics.confidence_score,
                            "processing_time_ms": attempt.result.metrics.processing_time_ms,
                            "cost": attempt.cost,
        ...
        speed_winner = min(
            job.attempts, key=lambda a: a.processing_time if a.success else float("inf")
        )
        cost_winner = min(
            [a for a in job.attempts if a.success],
            key=lambda a: a.cost if a.cost > 0 else float("inf"),
            default=None,
        )
        accuracy_winner = (
            comparison_result.best_provider if comparison_result.best_provider else None
        )
        ...
        return {
            ...
            "results": results,
            "rankings": [
                {"provider": provider.value, "score": score, "rank": idx + 1}
                for idx, (provider, score) in enumerate(comparison_result.rankings)
            ],
            "winner": (
                comparison_result.best_provider.value
                if comparison_result.best_provider
                else "none"
            ),
            "analysis": {
                "total_cost": job.total_cost,
                "avg_accuracy": (
                    ...
                    if comparison_result.provider_results
                    else 0.0
                ),
                "speed_winner": (
                    speed_winner.provider.value if speed_winner.success else "none"
                ),
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get battle comparison: {str(e)}"
        )
...
async def get_transcription_costs():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get cost information: {str(e)}"
        )


@app.post("/cloud-transcription/benchmark")
async def run_transcription_benchmark(
    providers: List[str], test_cases: Optional[List[str]] = None
):
...
async def get_providers(ai_registry=Depends(get_registry)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get provider status: {str(e)}"
        )
...
async def get_provider_details(provider_id: str, ai_registry=Depends(get_registry)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get provider details: {str(e)}"
        )


@app.post("/ai/providers/{provider_id}/test")
async def test_provider(
    provider_id: str, request: ProviderTestRequest, ai_registry=Depends(get_registry)
):
    """Test a specific AI provider with a test prompt"""
    try:
        result = await ai_registry.test_provider(
            request.provider_id, request.test_prompt
        )
        return result
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to test provider: {str(e)}"
        )


@app.post("/ai/generate")
async def generate_text(
    request: TextGenerationRequest, ai_registry=Depends(get_registry)
):
    ...
    try:
        ...
        if "No available providers" in str(e):
            raise HTTPException(
                status_code=503, detail="No AI providers are currently available"
            )
...
async def chat_completion(
    request: ChatCompletionRequest, ai_registry=Depends(get_registry)
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to generate chat completion: {str(e)}"
        )
...
async def get_health_summary(ai_registry=Depends(get_registry)):
    ...
    try:
        ...
        healthy_providers = sum(
            1 for p in status.values() if p["health"]["status"] == "healthy"
        )
        unhealthy_providers = sum(
            1 for p in status.values() if p["health"]["status"] == "unhealthy"
        )
        degraded_providers = sum(
            1 for p in status.values() if p["health"]["status"] == "degraded"
        )
        ...
        if total_providers == 0:
            ...
            health_score = (
                (healthy_providers + (degraded_providers * 0.5)) / total_providers * 100
            )

        # Get average response time across all providers
        response_times = [
            p["health"]["response_time_ms"]
            for p in status.values()
            if p["health"]["response_time_ms"] > 0
        ]
        avg_response_time = (
...
async def get_provider_health_history(
    provider_id: str, hours: int = 24, ai_registry=Depends(get_registry)
):
    ...
    try:
        ...
        return {
            ...
            "current_metrics": {
                ...
                "consecutive_failures": provider.health_metrics.consecutive_failures,
                "last_check": (
                    provider.health_metrics.last_check_timestamp.isoformat()
                    if provider.health_metrics.last_check_timestamp
                    else None
                ),
                "last_success": (
                    provider.health_metrics.last_success_timestamp.isoformat()
                    if provider.health_metrics.last_success_timestamp
                    else None
                ),
            },
...
async def reset_provider_health(provider_id: str, ai_registry=Depends(get_registry)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to reset health metrics: {str(e)}"
        )
...
async def update_meeting(
    meeting_id: str, meeting_update: MeetingUpdate, db: Session = Depends(get_db)
):
    ...
    try:
        ...
        updated_meeting = meeting_crud.update(
            db, db_obj=db_meeting, obj_in=meeting_update
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update meeting: {str(e)}"
        )
...
async def update_meeting_status(
    meeting_id: str, status: MeetingStatus, db: Session = Depends(get_db)
):
    ...
    try:
        ...
        return {
            ...
            "status": updated_meeting.status.value,
            "actual_start": (
                updated_meeting.actual_start.isoformat()
                if updated_meeting.actual_start
                else None
            ),
            "actual_end": (
                updated_meeting.actual_end.isoformat()
                if updated_meeting.actual_end
                else None
            ),
            "updated_at": updated_meeting.updated_at.isoformat(),
        }
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update meeting status: {str(e)}"
        )
...
async def get_meeting_statistics(meeting_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get meeting statistics: {str(e)}"
        )
...
async def add_participant(
    meeting_id: str, participant: ParticipantCreate, db: Session = Depends(get_db)
):
    ...
    try:
        ...
        existing = participant_crud.get_by_meeting_and_email(
            db, meeting_id, participant.email
        )
        if existing:
            raise HTTPException(
                status_code=409, detail="Participant already exists in this meeting"
            )
...
async def get_upcoming_meetings(
    user_id: str, hours_ahead: int = 24, db: Session = Depends(get_db)
):
...
async def startup_event():
    ...
    try:
        ...
        logger.warning(
            "   AI endpoints will not be available until configuration is loaded"
        )
    ...
    try:
        ...
        logger.info(
            f"   Available providers: {list(cloud_transcription_service.providers.keys())}"
        )
    except Exception as e:
...
async def create_meeting_stream_key(
    request: StreamKeyRequest, db: Session = Depends(get_db)
):
    """
...
async def revoke_meeting_stream_key(key_id: str, db: Session = Depends(get_db)):
    """Revoke a stream key"""
    try:
    ...
    try:
        ...
        return {
            ...
            "protocols": {
                ...
                "webrtc": {
                    "name": "WebRTC WHIP",
                    "url": format_stream_url(
                        "webrtc", server_ip, "YOUR_STREAM_KEY", meeting_id
                    ),
