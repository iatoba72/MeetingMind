...
def upgrade():
    ...
    op.create_index('idx_meeting_templates_created_by', 'meeting_templates', ['created_by'])
    op.create_index('idx_meeting_templates_organization_public', 'meeting_templates', 
                    ['organization_id', 'is_public'])
    op.create_index('idx_meeting_templates_created_by_category', 'meeting_templates', 
                    ['created_by', 'category'])
    ...
    op.create_index('idx_meetings_external_meeting_id', 'meetings', ['external_meeting_id'])
    
    # Compound indexes for common query patterns
    op.create_index('idx_meetings_status_scheduled_start', 'meetings', 
                    ['status', 'scheduled_start'])
    op.create_index('idx_meetings_created_by_status', 'meetings', 
                    ['created_by', 'status'])
    op.create_index('idx_meetings_organization_status', 'meetings', 
                    ['organization_id', 'status'])
    op.create_index('idx_meetings_template_created_at', 'meetings', 
                    ['template_id', 'created_at'])
    op.create_index('idx_meetings_status_date_range', 'meetings', 
                    ['status', 'scheduled_start', 'scheduled_end'])
    
    # Partial indexes for specific conditions (PostgreSQL)
    if op.get_context().dialect.name == 'postgresql':
        op.execute("CREATE INDEX idx_active_meetings ON meetings(id) WHERE status = 'active'")
        op.execute("CREATE INDEX idx_public_meetings ON meetings(scheduled_start) WHERE is_public = true")
    ...
    op.create_index('idx_participants_meeting_role', 'participants', 
                    ['meeting_id', 'role'])
    op.create_index('idx_participants_meeting_status', 'participants', 
                    ['meeting_id', 'status'])
    op.create_index('idx_participants_user_id_status', 'participants', 
                    ['user_id', 'status'])
    op.create_index('idx_participants_email_status', 'participants', 
                    ['email', 'status'])
    op.create_index('idx_participants_meeting_engagement', 'participants', 
    ...
    op.create_index('idx_transcripts_transcript_type', 'transcripts', ['transcript_type'])
    op.create_index('idx_transcripts_start_time_seconds', 'transcripts', ['start_time_seconds'])
    op.create_index('idx_transcripts_is_processed_for_insights', 'transcripts', 
                    ['is_processed_for_insights'])
    op.create_index('idx_transcripts_contains_action_items', 'transcripts', 
                    ['contains_action_items'])
    
    # Compound indexes for transcript queries
    op.create_index('idx_transcripts_meeting_time', 'transcripts', 
                    ['meeting_id', 'start_time_seconds'])
    op.create_index('idx_transcripts_meeting_type', 'transcripts', 
                    ['meeting_id', 'transcript_type'])
    op.create_index('idx_transcripts_participant_time', 'transcripts', 
                    ['participant_id', 'start_time_seconds'])
    op.create_index('idx_transcripts_processing_flags', 'transcripts', 
                    ['is_processed_for_insights', 'contains_action_items'])
    op.create_index('idx_transcripts_meeting_speaker_time', 'transcripts', 
    ...
    if op.get_context().dialect.name == 'postgresql':
        ...
        op.execute("CREATE INDEX idx_transcripts_content_fts ON transcripts USING gin(content gin_trgm_ops)")
    ...
    op.create_index('idx_ai_insights_is_user_validated', 'ai_insights', ['is_user_validated'])
    op.create_index('idx_ai_insights_is_action_required', 'ai_insights', ['is_action_required'])
    op.create_index('idx_ai_insights_action_due_date', 'ai_insights', ['action_due_date'])
    op.create_index('idx_ai_insights_is_public', 'ai_insights', ['is_public'])
    
    # Compound indexes for AI insights
    op.create_index('idx_ai_insights_meeting_type', 'ai_insights', 
                    ['meeting_id', 'insight_type'])
    op.create_index('idx_ai_insights_type_confidence', 'ai_insights', 
                    ['insight_type', sa.text('confidence_score DESC')])
    op.create_index('idx_ai_insights_action_required_due', 'ai_insights', 
                    ['is_action_required', 'action_due_date'])
    op.create_index('idx_ai_insights_user_validated_created', 'ai_insights', 
                    ['is_user_validated', 'created_at'])
    op.create_index('idx_ai_insights_meeting_confidence', 'ai_insights', 
                    ['meeting_id', sa.text('confidence_score DESC')])
    
    # Full-text search for AI insights
    if op.get_context().dialect.name == 'postgresql':
        op.execute("CREATE INDEX idx_ai_insights_content_fts ON ai_insights USING gin(to_tsvector('english', content))")
        op.execute("CREATE INDEX idx_ai_insights_title_fts ON ai_insights USING gin(to_tsvector('english', title))")
    ...
    op.create_index('idx_tags_parent_usage', 'tags', 
                    ['parent_id', sa.text('usage_count DESC')])
    op.create_index('idx_tags_organization_system', 'tags', 
    ...
    if op.get_context().dialect.name == 'postgresql':
        op.execute("CREATE INDEX idx_pending_insights ON ai_insights(meeting_id) WHERE is_user_validated = false")
        op.execute("CREATE INDEX idx_actionable_insights ON ai_insights(id) WHERE is_action_required = true")
        op.execute("CREATE INDEX idx_unprocessed_transcripts ON transcripts(meeting_id) WHERE is_processed_for_insights = false")
