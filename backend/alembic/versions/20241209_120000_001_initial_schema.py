...
def upgrade():
    ...
    meeting_status_enum = postgresql.ENUM(
        "not_started", "active", "paused", "ended", "cancelled", name="meetingstatus"
    )
    ...
    participant_status_enum = postgresql.ENUM(
        "invited", "joined", "left", "removed", "no_show", name="participantstatus"
    )

    transcript_type_enum = postgresql.ENUM(
        "speech", "system", "chat", "action_item", "summary", name="transcripttype"
    )
