    ...
    async def get_cached_translation(
        ...
        if exact_match:
            ...
            translation = (
                ...
                .filter(Translation.translation_memory_id == exact_match.id)
        ...
        if fuzzy_threshold > 0:
            ...
            for entry in similar_entries:
                ...
                if similarity >= fuzzy_threshold:
                    ...
                    translation = (
                        self.db.query(Translation)
                        .filter(Translation.translation_memory_id == entry.id)
                        .order_by(Translation.created_at.desc())
                        .first()
                    )
...
class TranslationProviderManager:
    ...
    async def translate(
        self, request: RealTimeTranslationRequest
        ...
        try:
            ...
            if fallback_provider:
                return await self._translate_with_provider(request, fallback_provider)
            raise e
    ...
    def _select_optimal_provider(
        ...
        for provider in self.providers.keys():
            if self._supports_language_pair(
                provider, request.source_language, request.target_language
            ):
        ...
        for provider in supported_providers:
            ...
            if request.quality_level == TranslationQuality.PREMIUM:
                score += (
                    0.2
                    if provider
                    in [TranslationProvider.DEEPL, TranslationProvider.OPENAI_GPT]
                    else 0
                )
    ...
    def _get_fallback_provider(
        self, failed_provider: TranslationProvider
    ...
    def _estimate_quality(
        self, translated_text: str, request: RealTimeTranslationRequest
    ) -> float:
        """
    ...
    async def _translate_with_provider(
        self, request: RealTimeTranslationRequest, provider: TranslationProvider
    ) -> RealTimeTranslationResult:
        """
...
class RealTimeTranslationService:
    ...
    async def translate_text(
        ...
        if translation_record:
            await self.cache.cache_translation(translation_record, source_text)

        return result

    async def start_translation_stream(self, stream_config: TranslationStream) -> str:
        """
    ...
    async def add_to_translation_stream(
        ...
        should_translate = (
            len(buffer_info["buffer"]) >= stream_config.buffer_size
            or (time.time() - buffer_info["last_translation"]) * 1000
            >= stream_config.max_delay_ms
            or text_chunk.endswith((".", "!", "?", ";"))  # Sentence ending
        )
    ...
    async def get_translation_statistics(
        self, meeting_id: str, time_range: Optional[Tuple[datetime, datetime]] = None
    ) -> Dict[str, Any]:
        """
        ...
        query = (
            self.db.query(Translation)
            .join(Translation.source_transcript)
            .filter(Translation.source_transcript.has(meeting_id=meeting_id))
        )
        ...
        for translation in translations:
            ...
            stats["language_pairs"][lang_pair] = (
                stats["language_pairs"].get(lang_pair, 0) + 1
            )
            ...
            stats["quality_levels"][quality] = (
                stats["quality_levels"].get(quality, 0) + 1
            )
    ...
    async def _save_translation(
        ...
        try:
            ...
            source_language = (
                ...
                .filter(Language.code == request.source_language)
                .first()
            )

            target_language = (
                self.db.query(Language)
                .filter(Language.code == request.target_language)
                .first()
            )
