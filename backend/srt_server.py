...
class SRTServer:
        ...
        try:
            ...
            if self.server_process:
                try:
                    ...
                    logger.warning(
                        "SRT server process didn't terminate gracefully, killing..."
                    )
    ...
    async def _start_ffmpeg_server(self):
        ...
        cmd = [
            ...
            "1",
            "-i",
            f"srt://0.0.0.0:{self.config.port}?mode=listener&latency={self.config.latency_ms}&rcvbuf={self.config.recv_buffer_size}",
        ]
        ...
        if self.config.passphrase:
            cmd[
                -1
            ] += f"&passphrase={self.config.passphrase}&pbkeylen={self.config.pbkeylen}"
        ...
        if self.server_process.returncode is not None:
            ...
            raise Exception(
                f"FFmpeg SRT server failed to start: {stderr_output.decode()}"
            )
    ...
    async def _parse_ffmpeg_log(self, log_line: str):
        ...
        try:
            ...
            if any(
                ...
                for keyword in ["error", "warning", "connection", "srt"]
            ):
    ...
    async def _start_stream_processor(self, stream_id: str):
        ...
        try:
            ...
            cmd = [
                ...
                "file,udp,rtp,srt",
                "-i",
                f"srt://0.0.0.0:{self.config.port}?mode=listener&latency={self.config.latency_ms}",
                "-f",
                "wav",
                "-acodec",
                "pcm_s16le",
            ...
            if self.config.recording_enabled:
                ...
                recording_file = (
                    f"{self.config.recording_path}/srt_{stream_id}_{timestamp}.mp4"
                )
    ...
    async def _monitor_stream_stats(self, stream_id: str):
        ...
        try:
            while stream_id in self.active_streams:
                ...
                ] += stream_info.packets_retransmitted
                ...
                active_count = len(
                    [
                        ...
                        if s.state == SRTStreamState.PUBLISHING
                    ]
                )
                if active_count > 0:
                    total_latency = sum(
                    ...
                    self.stats["average_bandwidth_mbps"] = (
                        total_bandwidth / active_count
                    )
