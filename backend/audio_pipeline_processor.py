...
class AudioPipelineProcessor:
    ...
    async def add_source(self, source_id: str, client_id: str, source_type: AudioSourceType, config: Dict[str, Any] = None) -> bool:
        ...
        try:
            ...
            if self.config['enable_real_time_transcription']:
                try:
                    ...
                    transcription_session_id = await audio_processor.create_session(
                        client_id=f"{client_id}_{source_id}",
                        audio_config=audio_config
                    )
                    
                    session.transcription_session_id = transcription_session_id
                    logger.info(f"Created transcription session {transcription_session_id} for source {source_id}")
                    
                except Exception as e:
                    logger.error(f"Failed to create transcription session for {source_id}: {e}")
            ...
            logger.info(f"Added audio source {source_id} ({source_type.value}) for client {client_id}")
            return True
    ...
    async def remove_source(self, source_id: str) -> bool:
        ...
        try:
            ...
            if session.transcription_session_id:
                try:
                    await audio_processor.close_session(session.transcription_session_id)
                except Exception as e:
    ...
    async def _process_chunk(self, chunk: PipelineAudioChunk):
        ...
        try:
            ...
            if chunk.has_voice and chunk.rms_level > self.config['voice_activity_threshold']:
                ...
                if len(self.transcription_buffers[source_id]) > max_buffer:
                    self.transcription_buffers[source_id] = self.transcription_buffers[source_id][-max_buffer:]
    ...
    async def _send_for_transcription(self, source_id: str, chunks: List[PipelineAudioChunk]):
        ...
        try:
            ...
            try:
                ...
                await self._trigger_transcription_callbacks(source_id, chunk_metadata, result)
    ...
    async def _trigger_transcription_callbacks(self, source_id: str, metadata: Dict, result: Dict):
        """Trigger transcription callbacks"""
        try:
            transcription_data = {
                'source_id': source_id,
