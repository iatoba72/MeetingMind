...
class InsightTriggerEngine:
    ...
    def __init__(self, topic_service: TopicDetectionService = None):
        ...
        self.topic_stability_threshold = (
            30.0  # Seconds of topic stability before new triggers
        )
    ...
    async def create_session(
        ...
        self.logger.info(
            f"Created insight trigger session {session_id} for meeting {meeting_id}"
        )
    ...
    async def process_segment(
        ...
        for trigger in filtered_triggers:
            session["processing_stats"]["triggers_by_type"][
                trigger.trigger_type.value
            ] += 1

        self.logger.debug(
            f"Processed segment for session {session_id}: {len(filtered_triggers)} triggers detected"
        )
    ...
    def _update_speaker_states(self, session: Dict, segment: Dict):
        ...
        speaker_state["avg_segment_length"] = (
            speaker_state["speaking_time"] / speaker_state["segment_count"]
        )
    ...
    async def _detect_triggers(
            ...
            if trigger_type in self.trigger_patterns:
                ...
                for pattern in patterns:
                    ...
                    match_result = self._check_pattern_match(
                        pattern, segment, context_segments
                    )

                    if (
                        match_result["matches"]
                        and match_result["confidence"] >= pattern.confidence_threshold
                    ):
                        # Create trigger
                        trigger = DetectedTrigger(
                            ...
                            topic_context=(
                                segment.get("topic_context", {}).get("id")
                                if segment.get("topic_context")
                                else None
                            ),
                            previous_triggers=[
                                t.id for t in session["detected_triggers"][-3:]
                            ],
                            metadata={
                                "pattern_id": f"{trigger_type.value}_{hash(str(pattern))}",
                                "match_details": match_result,
                                "speaker_context": self._get_speaker_context(
                                    session, segment
                                ),
                            ...
                            should_generate_insight=self._should_generate_insight(
                                trigger_type, match_result["confidence"]
                            ),
                            insight_type_suggestion=self._suggest_insight_type(
        ...
        composite_triggers = await self._detect_composite_triggers(
            session, segment, context_segments
        )
    ...
    def _check_pattern_match(
        self, pattern: TriggerPattern, segment: Dict, context_segments: List[Dict]
    ) -> Dict[str, Any]:
        ...
        # Keyword matching
        keyword_matches = sum(
            1 for keyword in pattern.keywords if keyword.lower() in text
        )
        if pattern.keywords:
            ...
            if keyword_matches > 0:
                matched_patterns.append(
                    f"keywords: {keyword_matches}/{len(pattern.keywords)}"
                )
        ...
        if pattern.phrases:
            ...
            if phrase_matches > 0:
                matched_patterns.append(
                    f"phrases: {phrase_matches}/{len(pattern.phrases)}"
                )
        ...
        if pattern.regex_patterns:
            ...
            if regex_matches > 0:
                matched_patterns.append(
                    f"regex: {regex_matches}/{len(pattern.regex_patterns)}"
                )
    ...
    async def _detect_composite_triggers(
        self, session: Dict, segment: Dict, context_segments: List[Dict]
    ) -> List[DetectedTrigger]:
        ...
        for i, segment in enumerate(context_segments[-3:]):
            if "?" in segment["text"]:
                ...
                for follow_seg in following_segments:
                    ...
                    if any(
                        word in text
                        for word in ["um", "uh", "well", "hmm", "let me think"]
                    ):
                        return True
    ...
    def _get_speaker_context(self, session: Dict, segment: Dict) -> Dict[str, Any]:
        ...
        return {
            ...
            "question_ratio": speaker_state["question_count"]
            / max(speaker_state["segment_count"], 1),
            "avg_silence": (
                np.mean(speaker_state["silence_periods"])
                if speaker_state["silence_periods"]
                else 0.0
            ),
    ...
    def _get_temporal_context(self, session: Dict, segment: Dict) -> Dict[str, Any]:
        ...
        return {
            ...
            "triggers_in_last_minute": len(
                [
                    t
                    for t in session["detected_triggers"]
    ...
    def _should_generate_insight(
        self, trigger_type: TriggerType, confidence: float
    ) -> bool:
        ...
        if trigger_type in [
            ...
            TriggerType.ACTION_ITEM,
            TriggerType.CONCERN_RAISED,
        ]:
        ...
        if trigger_type in [
            ...
            TriggerType.DISAGREEMENT,
            TriggerType.DEADLINE_MENTIONED,
        ]:
    ...
    def _suggest_insight_type(
        self, trigger_type: TriggerType, segment: Dict
    ) -> Optional[str]:
    ...
    def _filter_triggers(
        self, session: Dict, triggers: List[DetectedTrigger]
    ) -> List[DetectedTrigger]:
        ...
        for trigger in triggers:
            ...
            recent_similar = any(
                ...
                and abs(t.timestamp - trigger.timestamp) < 30  # Within 30 seconds
    ...
    def _find_active_periods(
        self, triggers: List[DetectedTrigger], window_minutes: int = 5
        ...
        while current_time <= end_time:
            ...
            window_triggers = [
                t for t in triggers if current_time <= t.timestamp < window_end
            ]
    ...
    def add_custom_pattern(self, pattern: TriggerPattern) -> str:
        ...
        self.logger.info(
            f"Added custom pattern {pattern_id} for trigger type {pattern.trigger_type.value}"
        )
    ...
    def remove_custom_pattern(self, pattern_id: str) -> bool:
        ...
        if pattern.trigger_type in self.trigger_patterns:
            self.trigger_patterns[pattern.trigger_type] = [
                p for p in self.trigger_patterns[pattern.trigger_type] if p != pattern
            ]
