...
from i18n_models import (
    Language, MultiLanguageTranscript, CulturalContextRule,
    LanguageCode, CulturalContext
)
...
class CulturalAnalyzer:
    ...
    async def _analyze_communication_style(
        ...
        return CulturalInsight(
            ...
            cultural_notes={
                ...
                "typical_cultures": self.communication_styles[dominant_style]['cultures']
            }
        )
    ...
    async def _analyze_hierarchy_patterns(
        ...
        if any(word in text_lower for word in self.hierarchy_markers['low_hierarchy']['challenge_language']):
        ...
        if high_hierarchy_score > low_hierarchy_score:
            ...
            confidence = high_hierarchy_score / (high_hierarchy_score + low_hierarchy_score)
        else:
            hierarchy_style = "low_hierarchy"
            confidence = low_hierarchy_score / (high_hierarchy_score + low_hierarchy_score)
        ...
        return CulturalInsight(
            ...
            confidence=confidence,
            evidence=[f"Contains {hierarchy_style.replace('_', ' ')} communication markers"],
            recommendations=[
...
class LanguagePatternAnalyzer:
    ...
    def _calculate_sentence_complexity(self, text: str) -> float:
        ...
        for sentence in sentences:
            ...
            clause_indicators = sentence.count(',') + sentence.count(';') + sentence.count(':')
    ...
    def _identify_emotional_markers(self, text: str, language: LanguageCode) -> List[str]:
    ...
    def _identify_cultural_references(self, text: str, language: LanguageCode) -> List[str]:
...
class LanguageSpecificInsightsService:
        ...
        if len(language_groups) > 1:
            ...
            for insight in cultural_insights:
                if insight.insight_type == 'hierarchy_awareness':
                    hierarchy_styles.add(insight.cultural_notes.get('hierarchy_style', 'unknown'))
    ...
    def _calculate_communication_effectiveness(
        ...
        if cultural_insights:
            avg_confidence = sum(insight.confidence for insight in cultural_insights) / len(cultural_insights)
        ...
        if language_patterns:
            ...
            for patterns in language_patterns.values():
                ...
                complexity_score = 1.0 - abs(patterns.sentence_complexity - ideal_complexity) / ideal_complexity
    ...
    async def save_insights_to_database(
        self,
        summary: MultilingualInsightSummary
    ) -> List[str]:
        """
        ...
        for cultural_insight in summary.cultural_insights:
            ai_insight = AIInsight(
                ...
                metadata={
                    ...
                    'cultural_context': cultural_insight.cultural_context.value if cultural_insight.cultural_context else None,
        ...
        if summary.cross_cultural_observations or summary.recommendations:
            cross_cultural_insight = AIInsight(
                ...
                metadata={
                    'languages_analyzed': [lang.value for lang in summary.languages_analyzed],
