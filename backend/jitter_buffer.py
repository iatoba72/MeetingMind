...
class AudioPacket:
    ...
    def _validate_packet(self):
        ...
        if not isinstance(self.sequence_number, int):
            raise PacketValidationError(
                f"sequence_number must be int, got {type(self.sequence_number)}"
            )
        if self.sequence_number < 0:
            raise PacketValidationError(
                f"sequence_number must be non-negative, got {self.sequence_number}"
            )
        if self.sequence_number > 0xFFFFFFFF:  # 32-bit limit
            raise PacketValidationError(
                f"sequence_number exceeds 32-bit limit: {self.sequence_number}"
            )

        # Validate timestamp
        if not isinstance(self.timestamp, (int, float)):
            raise PacketValidationError(
                f"timestamp must be numeric, got {type(self.timestamp)}"
            )
        if self.timestamp < 0:
            raise PacketValidationError(
        ...
        if self.timestamp > time.time() + 3600:  # Future timestamp limit (1 hour)
            raise PacketValidationError(
                f"timestamp too far in future: {self.timestamp}"
            )

        # Validate arrival_time
        if not isinstance(self.arrival_time, (int, float)):
            raise PacketValidationError(
                f"arrival_time must be numeric, got {type(self.arrival_time)}"
            )
        ...
        if len(self.data) > 1024 * 1024:  # 1MB limit
            raise PacketValidationError(
                f"data size exceeds limit: {len(self.data)} bytes"
            )

        # Validate source_id
        if not isinstance(self.source_id, str):
            raise PacketValidationError(
                f"source_id must be string, got {type(self.source_id)}"
            )
        ...
        if len(self.source_id) > 256:
            raise PacketValidationError(
                f"source_id too long: {len(self.source_id)} chars"
            )

        # Validate sample_rate
        if not isinstance(self.sample_rate, int):
            raise PacketValidationError(
                f"sample_rate must be int, got {type(self.sample_rate)}"
            )
        valid_sample_rates = [8000, 16000, 22050, 44100, 48000, 96000]
        if self.sample_rate not in valid_sample_rates:
            raise PacketValidationError(
                f"invalid sample_rate: {self.sample_rate}, must be one of {valid_sample_rates}"
            )

        # Validate channels
        if not isinstance(self.channels, int):
            raise PacketValidationError(
                f"channels must be int, got {type(self.channels)}"
            )
        ...
        if not isinstance(self.duration_ms, (int, float)):
            raise PacketValidationError(
                f"duration_ms must be numeric, got {type(self.duration_ms)}"
            )
        ...
        if not isinstance(self.state, PacketState):
            raise PacketValidationError(
                f"state must be PacketState enum, got {type(self.state)}"
            )
        ...
        if self.playout_time is not None:
            if not isinstance(self.playout_time, (int, float)):
                raise PacketValidationError(
                    f"playout_time must be numeric, got {type(self.playout_time)}"
                )
            if self.playout_time < 0:
                raise PacketValidationError(
                    f"playout_time must be non-negative, got {self.playout_time}"
                )
    ...
    def _validate_data_consistency(self):
        ...
        if not (min_bytes <= len(self.data) <= max_bytes):
            logger.warning(
                f"Data size mismatch: expected ~{expected_bytes} bytes, got {len(self.data)} bytes "
...
class JitterBufferConfig:
    ...
    def _validate_config(self):
        ...
        if not isinstance(self.target_delay_ms, int) or self.target_delay_ms <= 0:
            raise ValueError(
                f"target_delay_ms must be positive int, got {self.target_delay_ms}"
            )
        if not isinstance(self.min_delay_ms, int) or self.min_delay_ms <= 0:
            raise ValueError(
                f"min_delay_ms must be positive int, got {self.min_delay_ms}"
            )
        if not isinstance(self.max_delay_ms, int) or self.max_delay_ms <= 0:
            raise ValueError(
                f"max_delay_ms must be positive int, got {self.max_delay_ms}"
            )
        ...
        if not (self.min_delay_ms <= self.target_delay_ms <= self.max_delay_ms):
            raise ValueError(
                f"target_delay_ms ({self.target_delay_ms}) must be between min ({self.min_delay_ms}) and max ({self.max_delay_ms})"
            )

        # Timeout validation
        if not isinstance(self.packet_timeout_ms, int) or self.packet_timeout_ms <= 0:
            raise ValueError(
                f"packet_timeout_ms must be positive int, got {self.packet_timeout_ms}"
            )
        ...
        if not isinstance(self.max_buffer_packets, int) or self.max_buffer_packets <= 0:
            raise ValueError(
                f"max_buffer_packets must be positive int, got {self.max_buffer_packets}"
            )
        ...
        if (
            not isinstance(self.playout_interval_ms, int)
            or self.playout_interval_ms <= 0
        ):
            raise ValueError(
                f"playout_interval_ms must be positive int, got {self.playout_interval_ms}"
            )
        if self.playout_interval_ms > 1000:  # 1 second max
            raise ValueError(
                f"playout_interval_ms too large: {self.playout_interval_ms}"
            )

        # Boolean validation
        if not isinstance(self.adaptive_sizing, bool):
            raise ValueError(
                f"adaptive_sizing must be bool, got {type(self.adaptive_sizing)}"
            )
        if not isinstance(self.loss_concealment, bool):
            raise ValueError(
                f"loss_concealment must be bool, got {type(self.loss_concealment)}"
            )

        # Threshold validation
        if (
            not isinstance(self.late_packet_threshold_ms, int)
            or self.late_packet_threshold_ms < 0
        ):
            raise ValueError(
                f"late_packet_threshold_ms must be non-negative int, got {self.late_packet_threshold_ms}"
            )

        # Statistics window validation
        if (
            not isinstance(self.statistics_window_size, int)
            or self.statistics_window_size <= 0
        ):
            raise ValueError(
                f"statistics_window_size must be positive int, got {self.statistics_window_size}"
            )
        if self.statistics_window_size > 10000:
            raise ValueError(
                f"statistics_window_size too large: {self.statistics_window_size}"
            )
...
class PacketLossConcealer:
    ...
    def conceal_loss(
        self, missing_sequence: int, expected_duration_ms: float
        ...
        try:
            ...
            if (
                not isinstance(expected_duration_ms, (int, float))
            ...
            if not self.last_good_packet:
                ...
                return self._generate_silence_packet(
                    missing_sequence, expected_duration_ms
                )
    ...
    def _generate_silence_packet(
        self, sequence: int, duration_ms: float
    ) -> AudioPacket:
    ...
    def _apply_fadeout(self, data: bytes) -> bytes:
        ...
        return bytes(
            int(b * 0.5) if b < 128 else int(128 + (b - 128) * 0.5) for b in data
        )
    ...
    def _validate_reference_packet(self) -> bool:
        ...
        try:
            ...
            if (
                self.last_good_packet.sample_rate <= 0
                or self.last_good_packet.channels <= 0
                or self.last_good_packet.duration_ms <= 0
            ):
                return False
    ...
    def update_reference(self, packet: AudioPacket):
        ...
        try:
            if packet and packet.state == PacketState.RECEIVED:
                ...
                if (
                    packet.data
                    and len(packet.data) > 0
                    and packet.sample_rate > 0
                    and packet.channels > 0
                ):
...
class JitterBufferStatistics:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        p95_jitter = (
            statistics.quantiles(self.jitter_values, n=20)[18]
            if len(self.jitter_values) >= 20
            else 0
        )

        avg_buffer_level = (
            statistics.mean(self.buffer_levels) if self.buffer_levels else 0
        )
        max_buffer_level = max(self.buffer_levels) if self.buffer_levels else 0

        # Calculate rates
        loss_rate = (
            (self.lost_packets / self.total_packets) if self.total_packets > 0 else 0
        )
        late_rate = (
            (self.late_packets_count / self.total_packets)
...
class JitterBuffer:
    ...
    def _validate_incoming_packet(self, packet: AudioPacket) -> bool:
        ...
        try:
            ...
            if not PacketSecurityValidator.validate_packet_structure(packet):
                security_monitor.record_validation_failure(
                    "Invalid packet structure", f"type: {type(packet)}"
                )
                return False

            # Security validation of all fields
            if not PacketSecurityValidator.validate_sequence_number(
                packet.sequence_number
            ):
                security_monitor.record_validation_failure(
                    "Invalid sequence number", f"seq: {packet.sequence_number}"
                )
                return False

            if not PacketSecurityValidator.validate_timestamp(
                packet.timestamp, packet.arrival_time
            ):
                security_monitor.record_validation_failure(
                    "Invalid timestamp", f"ts: {packet.timestamp}"
                )
                return False
            ...
            if not PacketSecurityValidator.validate_audio_data(packet.data):
                security_monitor.record_validation_failure(
                    "Invalid audio data",
                    f"size: {len(packet.data) if packet.data else 0}",
                )
                return False

            if not PacketSecurityValidator.validate_source_id(packet.source_id):
                security_monitor.record_validation_failure(
                    "Invalid source ID", f"id: {packet.source_id}"
                )
                return False

            if not PacketSecurityValidator.validate_audio_parameters(
                packet.sample_rate, packet.channels, packet.duration_ms
            ):
                security_monitor.record_validation_failure(
            ...
            if packet.source_id != self.source_id:
                security_monitor.record_blocked_packet(
                    "Source ID mismatch",
            ...
            if self.base_sequence is not None:
                ...
                if packet.sequence_number < self.base_sequence - 10000:
                    security_monitor.record_suspicious_packet(
                        "Extremely old packet",
            ...
            if (
                ...
                and packet.timestamp < self.first_packet_time - 300
            ):  # 5 minutes old
                security_monitor.record_suspicious_packet(
                    "Very old packet timestamp",
            ...
            if packet.duration_ms > 200:  # Suspiciously long duration
                security_monitor.record_suspicious_packet(
                    "Unusually long packet duration",
        ...
        try:
            ...
            with self.lock:
                ...
                if packet.sequence_number < self.next_expected_sequence:
                    ...
                    self.statistics.add_packet_arrival(packet, current_time)
                    logger.debug(
                        f"Late packet {packet.sequence_number} received (expected {self.next_expected_sequence})"
                    )
                ...
                try:
                    ...
                    logger.error(
                        f"Memory error adding packet to buffer for source {self.source_id}"
                    )
                    return False
                except Exception as e:
                ...
                if (
                    self.state == BufferState.FILLING
                    and len(self.buffer) >= self._get_target_buffer_size()
                ):
                    self.state = BufferState.PLAYING
                    logger.info(
                        f"Jitter buffer {self.source_id} transitioned to PLAYING state"
                    )
    ...
    def _detect_and_handle_loss(self):
        ...
        try:
            ...
            missing_sequences = []
            for seq in range(
                self.next_expected_sequence, self.highest_sequence_received
            ):
                if seq not in self.received_packets:
                    ...
                    if time.time() - expected_arrival > (
                        self.config.packet_timeout_ms / 1000
                    ):
    ...
    def _update_adaptive_sizing(self):
        ...
        try:
            ...
            if len(self.statistics.jitter_values) >= 10:
                recent_jitter_ms = (
                    statistics.mean(list(self.statistics.jitter_values)[-10:]) * 1000
                )
            ...
            if len(self.statistics.loss_events) >= 5:
                recent_losses = len(
                    [t for t in self.statistics.loss_events if time.time() - t < 10]
                )  # Last 10 seconds
...
class JitterBufferManager:
    ...
    async def create_buffer(
        self, source_id: str, config: JitterBufferConfig = None
    ) -> JitterBuffer:
    ...
    def get_manager_statistics(self) -> Dict[str, Any]:
        ...
        stats = {"total_buffers": len(self.buffers), "buffers": {}}
...
async def create_jitter_buffer(
    source_id: str, config: JitterBufferConfig = None
) -> JitterBuffer:
