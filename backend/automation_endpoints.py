...
from models import (
    Meeting, MeetingTemplate, RecurringMeetingSeries, ActionItem, ActionItemUpdate,
    MeetingWorkflow, WorkflowNotification, TemplateRating, RecurringMeetingException,
    ActionItemStatus, WorkflowState, NotificationType
)
...
async def create_recurring_series(
    ...
    try:
        ...
        return {
            ...
            "upcoming_count": len(await service.get_upcoming_occurrences(str(series.id)))
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_recurring_series(
    ...
    try:
        ...
        if organization_id:
            query = query.filter(RecurringMeetingSeries.organization_id == organization_id)
        ...
        for series in series_list:
            ...
            next_meeting = db.query(Meeting).filter(
                Meeting.recurring_series_id == series.id,
                Meeting.scheduled_start > datetime.utcnow()
            ).order_by(Meeting.scheduled_start).first()
            ...
            result.append({
                ...
                "duration_minutes": series.duration_minutes,
                "is_active": series.is_active,
                "next_occurrence": next_meeting.scheduled_start.isoformat() if next_meeting else None,
...
async def create_series_exception(
    ...
    try:
        exception = await service.handle_series_exception(
            ...
            new_date=datetime.fromisoformat(exception_data["new_date"]) if exception_data.get("new_date") else None,
...
async def get_action_items(
    ...
    try:
        ...
        if meeting_id:
            ...
            query = service.db.query(ActionItem).filter(ActionItem.meeting_id == meeting_id)
        else:
            # Get items for user
            include_assigned_by = True  # Include items assigned by user
            items = await service.get_action_items_for_user(user_id, None, include_assigned_by)
            query = service.db.query(ActionItem).filter(ActionItem.id.in_([item.id for item in items]))
        ...
        for item in action_items:
            # Get meeting title
            meeting = service.db.query(Meeting).filter(Meeting.id == item.meeting_id).first()
...
async def update_action_item_progress(
    ...
    try:
        action_item = await service.update_action_item_progress(
            ...
            new_status=ActionItemStatus(update_data["new_status"]) if update_data.get("new_status") else None
        )
...
async def extract_action_items_from_text(
    ...
    try:
        ...
        if extraction_data.get("meeting_id"):
            ...
            participants = service.db.query(Participant).filter(
                Participant.meeting_id == extraction_data["meeting_id"]
            ).all()
...
async def get_workflow(
    ...
    try:
        workflow = db.query(MeetingWorkflow).filter(MeetingWorkflow.id == workflow_id).first()
        if not workflow:
...
async def get_workflow_executions(
    ...
    try:
        ...
        executions = [{
            ...
            "completed_at": workflow.updated_at.isoformat() if workflow.current_state == WorkflowState.COMPLETED else None,
            "status": "completed" if workflow.current_state == WorkflowState.COMPLETED else 
                     "paused" if workflow.paused else "running"
        }]
...
async def get_community_templates(
    ...
    try:
        ...
        for template in templates:
            ...
            ratings = db.query(TemplateRating).filter(TemplateRating.template_id == template.id).all()
...
async def get_my_templates(
    ...
    try:
        ...
        ).order_by(MeetingTemplate.created_at.desc()).all()
