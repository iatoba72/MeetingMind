...
class TestRunner:
    ...
    async def run_test(
        self, test: Dict[str, Any], environment: Environment, settings: Dict[str, Any]
    ) -> ExperimentResult:
        """Run a single test"""
        ...
        try:
            ...
            result = await asyncio.wait_for(
                test_function(test, environment, settings), timeout=timeout
            )
    ...
    async def _test_font_size_scaling(
        self, test: Dict, environment: Environment, settings: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _test_theme_transition(
        self, test: Dict, environment: Environment, settings: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        transition_time = 200 + (
            50 if theme == "auto" else 0
        )  # Auto theme takes longer
    ...
    async def _test_compact_mode(
        ...
        if simulated_button_size < min_touch_target:
            return {
                ...
                "metrics": {
                    "button_size": simulated_button_size,
    ...
    async def _measure_render_time(
        self, test: Dict, environment: Environment, settings: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Measure render time"""
        ...
        device_factor = {"mobile": 1.5, "tablet": 1.2, "desktop": 1.0}[
    ...
    async def _measure_memory_usage(
        self, test: Dict, environment: Environment, settings: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _test_contrast_ratio(
        self, test: Dict, environment: Environment, settings: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Test contrast ratio"""
    ...
    async def _test_keyboard_navigation(
        self, test: Dict, environment: Environment, settings: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if accessible_elements < elements_tested:
            return {
                ...
                "metrics": {
                    ...
                    "total_elements": elements_tested,
                },
            }
...
async def run_experiment(
    experiment_id: str, request: RunExperimentRequest, background_tasks: BackgroundTasks
):
    """Run an experiment"""
    if experiment_id not in experiments_storage:
        raise HTTPException(status_code=404, detail="Experiment not found")
...
async def _run_experiment_background(
    ...
    try:
        ...
        test_suites = [
            suite for suite in DEFAULT_TEST_SUITES if suite.id in test_suite_ids
        ]
        environments = [
            env for env in DEFAULT_ENVIRONMENTS if env.id in environment_ids
        ]
                ...
                for test in test_suite.tests:
                    try:
                        result = await test_runner.run_test(
                            test, environment, experiment.settings
                        )
        ...
        logger.info(
            f"Completed experiment {experiment_id} in {duration}s with {len(results)} results"
        )
...
@router.get(
    "/experiments/{experiment_id}/results", response_model=List[ExperimentResult]
)
...
async def get_lab_stats():
    ...
    completed_experiments = sum(
        1 for exp in experiments_storage.values() if exp.status == "completed"
    )
    total_tests_run = sum(
        len(exp.results or []) for exp in experiments_storage.values()
    )
