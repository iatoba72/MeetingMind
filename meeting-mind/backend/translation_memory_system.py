...
from i18n_models import (
    TranslationMemory, Translation, Language, LanguageCode,
    TranslationProvider, TranslationQuality
)
...
class FuzzyMatcher:
    ...
    def calculate_similarity(self, text1: str, text2: str) -> float:
        ...
        len_diff = abs(len(text1_norm) - len(text2_norm)) / max(len(text1_norm), len(text2_norm))
...
class QualityScorer:
    ...
    def calculate_quality_score(
        ...
        if translation.user_feedback_score is not None:
            factors.append(('user_feedback', translation.user_feedback_score / 5.0, 0.2))
...
class TranslationMemorySystem:
        ...
        for candidate in candidates:
            ...
            if similarity >= request.min_similarity:
                ...
                if request.context_tags and candidate.context_tags:
                    context_similarity = self.fuzzy_matcher.calculate_context_similarity(
    ...
    async def validate_entry(
        ...
        """
        entry = self.db.query(TranslationMemory).filter(
    ...
    async def optimize_memory(
        ...
        if remove_low_quality:
            ...
            low_quality_entries = [
                e for e in entries 
                if e.quality_score < low_quality_threshold and 
                   e.usage_frequency < 2 and 
                   not e.is_validated
            ]
        ...
        if merge_duplicates:
            ...
            hash_groups = {}
            for entry in entries:
                if entry in [e for e in entries if e.quality_score < 0.3]:  # Skip already removed
                    continue
    ...
    def _export_as_csv(self, entries: List[TranslationMemory]) -> str:
        ...
        # Header
        writer.writerow([
            'Source Text', 'Target Text', 'Source Language', 'Target Language',
            'Domain', 'Quality Score', 'Usage Frequency', 'Created At'
        ])
    ...
    def _export_as_tmx(self, entries: List[TranslationMemory], source_lang: LanguageCode, target_lang: LanguageCode) -> str:
