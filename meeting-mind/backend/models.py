...
class MeetingTemplate(Base):
    ...
    category = Column(String(100), index=True)  # e.g., "weekly-standup", "quarterly-review"
    ...
    __table_args__ = (
        ...
        Index('idx_meeting_templates_organization_public', 'organization_id', 'is_public'),
    )
...
class Meeting(Base):
    ...
    description = Column(Text)
    meeting_number = Column(String(50), unique=True, index=True)  # Human-readable meeting ID
    ...
    status = Column(Enum(MeetingStatus), default=MeetingStatus.NOT_STARTED, nullable=False, index=True)
    ...
    created_by = Column(String(255), nullable=False, index=True)  # User ID
    organization_id = Column(String(255), index=True)  # For multi-tenant support
    template_id = Column(UUID(as_uuid=True), ForeignKey('meeting_templates.id', ondelete='SET NULL'))
    ...
    __table_args__ = (
        ...
        CheckConstraint('scheduled_end > scheduled_start', name='check_meeting_schedule_order'),
    ...
    def validate_status_transition(self, key, value):
        ...
        if hasattr(self, 'status') and self.status:
            valid_transitions = {
                MeetingStatus.NOT_STARTED: [MeetingStatus.ACTIVE, MeetingStatus.CANCELLED],
...
class Participant(Base):
    ...
    user_id = Column(String(255), index=True)  # Internal user ID (nullable for external participants)
    email = Column(String(255), nullable=False, index=True)
    display_name = Column(String(255), nullable=False)
    avatar_url = Column(String(500))
    
    # Participant role and permissions
    role = Column(Enum(ParticipantRole), default=ParticipantRole.PARTICIPANT, nullable=False, index=True)
    status = Column(Enum(ParticipantStatus), default=ParticipantStatus.INVITED, nullable=False, index=True)
    ...
    def participation_duration_minutes(self) -> Optional[int]:
        ...
        if self.joined_at and self.left_at:
            ...
            return int((datetime.utcnow() - self.joined_at.replace(tzinfo=None)).total_seconds() / 60)
        return None
    
    @property
    def engagement_score(self) -> float:
        """Calculate participant engagement score (0.0 to 1.0)"""
        if not self.participation_duration_minutes or self.participation_duration_minutes == 0:
            return 0.0
        ...
        microphone_ratio = min(self.microphone_on_duration_seconds / duration_seconds, 1.0) * 0.2
        camera_ratio = min(self.camera_on_duration_seconds / duration_seconds, 1.0) * 0.2
        chat_participation = min(self.chat_messages_count / 10, 1.0) * 0.2  # Normalize to 10 messages
...
class Transcript(Base):
    ...
    transcript_type = Column(Enum(TranscriptType), default=TranscriptType.SPEECH, nullable=False, index=True)
    ...
    __table_args__ = (
        ...
        CheckConstraint('end_time_seconds >= start_time_seconds', name='check_transcript_time_order'),
        # Ensure sequence number is unique within meeting
        UniqueConstraint('meeting_id', 'sequence_number', name='uq_transcript_meeting_sequence'),
        ...
        Index('idx_transcripts_participant_time', 'participant_id', 'start_time_seconds'),
        Index('idx_transcripts_processing_flags', 'is_processed_for_insights', 'contains_action_items'),
        # Full-text search index on content (PostgreSQL specific)
        Index('idx_transcripts_content_fts', 'content', postgresql_using='gin', 
...
class AIInsight(Base):
    ...
    source_time_range = Column(JSONB, default=dict)  # Start/end times for time-based insights
    ...
    __table_args__ = (
        ...
        Index('idx_ai_insights_action_required', 'is_action_required', 'action_due_date'),
        Index('idx_ai_insights_user_validated', 'is_user_validated', 'created_at'),
        # Full-text search on content and title
        Index('idx_ai_insights_content_fts', 'content', 'title', postgresql_using='gin',
...
class Tag(Base):
    ...
    description = Column(Text)
    parent_id = Column(UUID(as_uuid=True), ForeignKey('tags.id', ondelete='CASCADE'), index=True)
...
Index('idx_meetings_status_date_range', Meeting.status, Meeting.scheduled_start, Meeting.scheduled_end)
Index('idx_participants_meeting_engagement', Participant.meeting_id, Participant.speaking_time_seconds.desc())
Index('idx_transcripts_meeting_speaker_time', Transcript.meeting_id, Transcript.participant_id, Transcript.start_time_seconds)
Index('idx_ai_insights_meeting_confidence', AIInsight.meeting_id, AIInsight.confidence_score.desc())
...
class RecurringMeetingSeries(Base):
    ...
    template_id = Column(UUID(as_uuid=True), ForeignKey('meeting_templates.id', ondelete='CASCADE'), nullable=False)
    ...
    exceptions = relationship("RecurringMeetingException", back_populates="series", cascade="all, delete-orphan")
    
    # Indexes
    __table_args__ = (
        Index('idx_recurring_series_template_active', 'template_id', 'is_active'),
        Index('idx_recurring_series_organization_type', 'organization_id', 'recurrence_type'),
...
class RecurringMeetingException(Base):
    ...
    # Foreign keys
    series_id = Column(UUID(as_uuid=True), ForeignKey('recurring_meeting_series.id', ondelete='CASCADE'), nullable=False)
    ...
    exception_type = Column(String(50), nullable=False)  # 'cancelled', 'rescheduled', 'modified'
    reason = Column(Text)
...
class ActionItem(Base):
    ...
    insight_id = Column(UUID(as_uuid=True), ForeignKey('ai_insights.id', ondelete='SET NULL'))
...
class ActionItemUpdate(Base):
    ...
    # Foreign key
    action_item_id = Column(UUID(as_uuid=True), ForeignKey('action_items.id', ondelete='CASCADE'), nullable=False)
...
class MeetingWorkflow(Base):
    ...
    current_state = Column(Enum(WorkflowState), default=WorkflowState.TEMPLATE_SELECTED, nullable=False, index=True)
    ...
    # Indexes
    __table_args__ = (
        Index('idx_meeting_workflows_state_scheduled', 'current_state', 'next_scheduled_action'),
...
class WorkflowNotification(Base):
    ...
    # Foreign key
    workflow_id = Column(UUID(as_uuid=True), ForeignKey('meeting_workflows.id', ondelete='CASCADE'), nullable=False)
    ...
    recipient_type = Column(String(50), nullable=False)  # 'participants', 'host', 'specific', 'external'
    ...
    delivery_status = Column(String(50), default='pending', index=True)  # pending, sent, failed, cancelled
    ...
    # Indexes
    __table_args__ = (
        Index('idx_workflow_notifications_scheduled', 'scheduled_for', 'delivery_status'),
        Index('idx_workflow_notifications_workflow_type', 'workflow_id', 'notification_type'),
    )
...
Meeting.recurring_series_id = Column(UUID(as_uuid=True), ForeignKey('recurring_meeting_series.id', ondelete='SET NULL'))
Meeting.recurring_series = relationship("RecurringMeetingSeries", back_populates="meetings")
Meeting.workflow = relationship("MeetingWorkflow", uselist=False, back_populates="meeting", cascade="all, delete-orphan")
Meeting.action_items = relationship("ActionItem", back_populates="meeting", cascade="all, delete-orphan")

# Update existing AIInsight model to support action items
AIInsight.action_items = relationship("ActionItem", back_populates="insight", cascade="all, delete-orphan")

# Additional indexes for new automation features
Index('idx_recurring_series_next_occurrence', RecurringMeetingSeries.start_date, RecurringMeetingSeries.is_active)
Index('idx_action_items_overdue', ActionItem.due_date, ActionItem.status, postgresql_where=ActionItem.due_date < func.now())
Index('idx_workflow_notifications_pending', WorkflowNotification.scheduled_for, postgresql_where=WorkflowNotification.delivery_status == 'pending')