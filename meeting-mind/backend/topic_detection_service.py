...
class TopicDetectionService:
    ...
    def __init__(self, 
        ...
        self.stop_words.update([
            'meeting', 'discussion', 'talk', 'discuss', 'say', 'said',
            'think', 'thought', 'know', 'see', 'look', 'well', 'okay',
            'alright', 'yeah', 'yes', 'sure', 'right', 'like', 'um', 'uh'
        ])
        ...
        self.topic_categories = {
            'technical': ['system', 'software', 'code', 'development', 'bug', 'feature', 'api', 'database'],
            'business': ['revenue', 'sales', 'customer', 'market', 'strategy', 'budget', 'profit', 'growth'],
            'project': ['deadline', 'milestone', 'task', 'sprint', 'release', 'delivery', 'timeline'],
            'team': ['hire', 'team', 'staff', 'resource', 'role', 'responsibility', 'collaboration'],
            'product': ['feature', 'design', 'user', 'interface', 'experience', 'feedback', 'requirement'],
            'finance': ['cost', 'budget', 'expense', 'investment', 'funding', 'price', 'financial'],
            'operations': ['process', 'workflow', 'efficiency', 'automation', 'infrastructure', 'scaling'],
            'legal': ['contract', 'compliance', 'regulation', 'policy', 'agreement', 'legal', 'terms']
        }
    ...
    async def create_session(self, meeting_id: str, session_config: Dict[str, Any] = None) -> str:
        ...
        session_data = {
            ...
            'config': {
                ...
                'change_threshold': config.get('change_threshold', self.change_threshold),
                'min_topic_duration': config.get('min_topic_duration', self.min_topic_duration),
                'enable_entity_recognition': config.get('enable_entity_recognition', True),
                'enable_sentiment_analysis': config.get('enable_sentiment_analysis', True),
                'topic_labeling_mode': config.get('topic_labeling_mode', 'automatic')  # automatic, manual, hybrid
            },
            'text_segments': [],
            'detected_topics': [],
            'topic_transitions': [],
        ...
        self.logger.info(f"Created topic detection session {session_id} for meeting {meeting_id}")
        
        return session_id
    
    async def process_text_segment(self, 
                                 session_id: str, 
                                 text: str, 
                                 timestamp: float,
        ...
        if len(processed_text.strip()) < 10:  # Skip very short segments
            return {'topic_change': False, 'current_topic': session.get('current_topic')}
        ...
        session['processing_stats']['confidence_scores'].append(topic_analysis.get('confidence', 0.0))
        
        if topic_analysis.get('topic_change', False):
            session['processing_stats']['topic_changes_detected'] += 1
        
        self.logger.debug(f"Processed text segment for session {session_id} in {processing_time:.1f}ms")
        
        return topic_analysis
    
    async def _analyze_topic_change(self, session: Dict, segment: Dict) -> Dict[str, Any]:
        ...
        similarity = self._calculate_topic_similarity(current_topic, segment, recent_segments)
    ...
    def _preprocess_text(self, text: str) -> str:
        ...
        filler_pattern = r'\b(um|uh|ah|er|hmm|well|you know|i mean|like|sort of|kind of)\b'
    ...
    def _extract_keywords(self, text: str) -> List[str]:
        ...
        important_pos = ['NN', 'NNS', 'NNP', 'NNPS', 'VB', 'VBG', 'VBN', 'VBP', 'VBZ', 'JJ', 'JJR', 'JJS']
        keywords = []
    ...
    def _analyze_sentiment(self, text: str) -> Dict[str, Any]:
        ...
        positive_words = ['good', 'great', 'excellent', 'amazing', 'wonderful', 'fantastic', 'positive', 'agree', 'yes', 'success']
        negative_words = ['bad', 'terrible', 'awful', 'horrible', 'negative', 'disagree', 'no', 'problem', 'issue', 'concern']
        question_words = ['what', 'when', 'where', 'why', 'how', 'who', 'which', 'could', 'would', 'should']
    ...
    def _calculate_topic_similarity(self, current_topic: Dict, segment: Dict, recent_segments: List[Dict]) -> float:
        ...
        try:
            ...
            recent_text = ' '.join([seg['processed_text'] for seg in recent_segments[-3:]])
            
            # Get topic keywords
            topic_keywords = current_topic['keywords']['primary_keywords'] + current_topic['keywords']['secondary_keywords']
    ...
    def _detect_change_indicators(self, segment: Dict, recent_segments: List[Dict]) -> Dict[str, Any]:
        """Detect linguistic indicators of topic change"""
        text = segment['text'].lower()
        
        # Transition phrases
        transition_phrases = [
            'moving on', 'next topic', 'speaking of', 'by the way', 'on another note',
            'changing subjects', 'let\'s talk about', 'now let\'s discuss', 'shifting gears',
            'different topic', 'new subject', 'anyway', 'meanwhile', 'also', 'additionally',
            'furthermore', 'however', 'but', 'wait', 'actually', 'oh', 'so'
        ]
        
        # Question indicators
        question_starters = [
            'what about', 'how about', 'what if', 'why don\'t we', 'should we',
            'can we', 'could we', 'would you', 'do you think', 'what do you'
        ]
        
        # Decision/action indicators
        decision_phrases = [
            'let\'s decide', 'we need to', 'action item', 'to do', 'follow up',
            'next steps', 'assign', 'responsible', 'deadline', 'due date'
        ]
        
        indicators = {
            'has_transition_phrase': any(phrase in text for phrase in transition_phrases),
            'starts_with_question': any(text.startswith(phrase) for phrase in question_starters),
        ...
        if len(recent_segments) > 0:
            ...
            indicators['speaker_change'] = last_speaker != current_speaker and last_speaker is not None
    ...
    def _calculate_change_confidence(self, similarity: float, indicators: Dict, segment: Dict, recent_segments: List[Dict]) -> float:
    ...
    def _is_abrupt_change(self, segment: Dict, recent_segments: List[Dict]) -> bool:
        ...
        abrupt_indicators = [
            'wait', 'stop', 'hold on', 'actually', 'by the way',
            'oh', 'speaking of', 'that reminds me'
        ]
    ...
    def _generate_topic_label(self, keywords: TopicKeywords, segment: Dict) -> str:
        ...
        for category, category_keywords in self.topic_categories.items():
            ...
            if overlap > 0:
                ...
                top_keyword = keywords.primary_keywords[0] if keywords.primary_keywords else category
    ...
    def _calculate_importance_score(self, keywords: TopicKeywords, segment: Dict) -> float:
        ...
        decision_words = ['decide', 'action', 'assign', 'deadline', 'priority', 'important']
    ...
    def _update_current_topic(self, topic: Dict, segment: Dict):
        ...
        topic['keywords']['primary_keywords'] = [k for k, c in keyword_counts.most_common(10)]
    ...
    def _create_transition(self, 
        ...
        return {
            ...
            'trigger_text': segment['text'][:200] + '...' if len(segment['text']) > 200 else segment['text'],
    ...
    def _find_transition_markers(self, text: str) -> List[str]:
        ...
        transition_markers = [
            'moving on', 'next topic', 'speaking of', 'by the way', 'on another note',
            'changing subjects', 'let\'s talk about', 'now let\'s discuss', 'shifting gears',
            'different topic', 'new subject', 'anyway', 'meanwhile', 'also'
        ]
    ...
    async def get_session_analysis(self, session_id: str) -> Optional[MeetingTopicAnalysis]:
        ...
        topic_coherence = np.mean([t['coherence_score'] for t in topics]) if topics else 0.0
        ...
        return MeetingTopicAnalysis(
            ...
            session_id=session_id,
            topics=[DetectedTopic(**{k: v for k, v in topic.items() if k != 'keywords'} | 
                                 {'keywords': TopicKeywords(**topic['keywords']),
                                  'confidence_level': self._get_confidence_level(topic['confidence'])}) 
                   for topic in topics],
