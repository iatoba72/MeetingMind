    ...
    async def start(self) -> bool:
        ...
        try:
            cmd = [
                ...
                '-f', 'flv',
                '-listen', '1',
                '-i', f'rtmp://localhost:{self.port}/live',
                '-f', 'wav',
                '-acodec', 'pcm_s16le',
                '-ar', '44100',
                '-ac', '2',
...
class SRTAudioReceiver:
    ...
    async def start(self) -> bool:
        ...
        try:
            ...
            cmd = [
                'ffmpeg',
                '-f', 'mpegts',
                '-i', f'srt://:{self.port}?mode=listener',
                '-f', 'wav',
                '-acodec', 'pcm_s16le',
                '-ar', '44100',
                '-ac', '2',
    ...
    async def _read_audio_data(self):
        ...
        try:
            ...
            self.jitter_buffers[stream_id] = JitterBuffer(target_delay_ms=50, max_delay_ms=200)
            ...
            while self.is_running and self.process:
                ...
                if smoothed_chunk:
                    ...
                    # Calculate jitter
                    current_time = time.time()
                    expected_time = stream_info.created_at.timestamp() + (sequence * chunk.duration_ms / 1000)
...
class NetworkAudioServer:
    ...
    async def start(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Network Audio Server started - RTMP:{rtmp_started}, SRT:{srt_started}")
            return True
    ...
    async def _handle_audio_chunk(self, chunk: AudioChunk, stream_info: AudioStreamInfo):
    ...
    async def _handle_auto_switching(self, stream_id: str, stream_info: AudioStreamInfo):
        ...
        if self.primary_source in self.active_streams:
            ...
            if primary_info.last_packet_at:
                ...
                if time_since_last > timedelta(seconds=5):  # 5 second timeout
                    ...
                    logger.info(f"Primary source {self.primary_source} timed out, switching to {stream_id}")
                    self.fallback_source = self.primary_source
                    self.primary_source = stream_id
        
        # Quality-based switching (prefer lower latency)
        if stream_info.latency < self.active_streams.get(self.primary_source, stream_info).latency - 50:  # 50ms threshold
    ...
    def _write_wav_header(self, file_handle, sample_rate: int, channels: int, sample_count: int):
    ...
    async def _stop_all_recordings(self):
        ...
        for stream_id, recording in self.recorded_streams.items():
            try:
                ...
                file_handle.close()
                logger.info(f"Stopped recording for stream {stream_id}: {recording['filename']}")
    ...
    async def _monitoring_loop(self):
        ...
        while True:
            try:
                ...
                # Calculate metrics
                total_bytes = sum(stream.bytes_received for stream in self.active_streams.values())
                total_packets = sum(stream.packets_received for stream in self.active_streams.values())
                total_lost = sum(stream.packets_lost for stream in self.active_streams.values())
                ...
                if self.active_streams:
                    self.metrics.average_jitter = sum(s.jitter for s in self.active_streams.values()) / len(self.active_streams)
                    self.metrics.average_latency = sum(s.latency for s in self.active_streams.values()) / len(self.active_streams)
