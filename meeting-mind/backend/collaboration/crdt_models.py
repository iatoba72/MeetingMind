...
class LWWRegister(CRDT):
    ...
    def __init__(self, replica_id: str, initial_value: Any = None):
        ...
        ) if initial_value is not None else None
    ...
    def merge(self, other: 'LWWRegister'):
        ...
        if other.lww_value is None:
            ...
            if (other.lww_value.timestamp > self.lww_value.timestamp or
                (other.lww_value.timestamp == self.lww_value.timestamp and
                 other.lww_value.replica_id > self.lww_value.replica_id)):
                self.lww_value = other.lww_value
...
class RGA(CRDT):
    ...
    def merge(self, other: 'RGA'):
        ...
        sorted_nodes = sorted(
            all_nodes.values(),
            key=lambda n: (n.timestamp, n.replica_id, n.id)
        )
...
class CollaborativeDocument:
    ...
    def __init__(self, document_id: str, replica_id: str):
        ...
        self.metadata.set("created_at", LWWRegister(replica_id, datetime.utcnow().isoformat()))
        self.metadata.set("last_modified", LWWRegister(replica_id, datetime.utcnow().isoformat()))
