...
from network_diagnostics import (
    get_network_diagnostics, run_quick_network_test,
    PingResult, PortTestResult, BandwidthResult, NetworkQualityResult
)
...
async def dns_resolution_test(domain: str = Query(..., description="Domain to resolve")):
    """Test DNS resolution"""
    try:
        diagnostics = await get_network_diagnostics()
...
async def start_comprehensive_test(
    ...
    try:
        ...
        background_tasks.add_task(
            run_comprehensive_test_background,
            test_id,
            request.target_host
        )
...
async def streaming_specific_test():
    ...
    try:
        ...
        avg_latency = sum(p["latency_ms"] for p in ping_results if p["success"]) / len([p for p in ping_results if p["success"]])
        ...
        if bandwidth_result.download_mbps < 10:
            issues.append(f"Low upload bandwidth: {bandwidth_result.upload_mbps:.1f} Mbps")
...
async def delete_test_result(test_id: str):
    ...
    try:
        if test_id in background_test_results:
            ...
            return NetworkDiagnosticsResponse(
                success=True,
                message=f"Test result {test_id} deleted"
            )
        else:
            raise HTTPException(status_code=404, detail="Test not found")
