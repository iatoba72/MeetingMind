...
class AudioSession:
    ...
    async def _analyze_audio_chunk(self, chunk_id: str, audio_bytes: bytes) -> Dict:
        ...
        try:
            ...
            bitrate = (actual_size * 8) / duration_seconds if duration_seconds > 0 else 0
    ...
    async def _cleanup_old_chunks(self):
        ...
        for chunk_id, chunk_info in self.chunks.items():
            ...
            if (chunk_age > 30 or 
                self.buffer_size > 5 * 1024 * 1024):  # 5MB limit
    ...
    def get_statistics(self) -> Dict:
        ...
        processing_rate = (
            self.chunks_processed / session_duration 
        ...
        return {
            ...
            "processing": {
                ...
                "success_rate": (
                    ...
                    if self.chunks_received > 0 else 0
                ),
