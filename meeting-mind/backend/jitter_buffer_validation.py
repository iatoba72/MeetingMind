...
class PacketSecurityValidator:
    ...
    def validate_packet_structure(packet: Any) -> bool:
        ...
        try:
            ...
            required_fields = [
                'sequence_number', 'timestamp', 'arrival_time', 'data',
                'source_id', 'sample_rate', 'channels', 'duration_ms', 'state'
            ]
    ...
    def validate_timestamp(timestamp: Any, arrival_time: Any = None) -> bool:
        ...
        try:
            ...
            if timestamp > current_time + PacketSecurityValidator.MAX_FUTURE_TIMESTAMP_OFFSET:
                logger.error(f"Timestamp too far in future: {timestamp}")
                return False
            
            if timestamp < current_time - PacketSecurityValidator.MAX_PAST_TIMESTAMP_OFFSET:
            ...
            if arrival_time is not None:
                if not isinstance(arrival_time, (int, float)):
                    logger.error(f"Arrival time must be numeric, got {type(arrival_time)}")
                    return False
        ...
        try:
            ...
            if len(data) > 0:
                ...
                if len(set(data)) == 1 and len(data) > 1000:
                    logger.warning(f"Audio data contains suspicious pattern (all same byte)")
    ...
    @staticmethod
    def validate_audio_parameters(sample_rate: Any, channels: Any, duration_ms: Any) -> bool:
    ...
    def sanitize_packet_data(packet_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if isinstance(seq_num, (int, float)):
                sanitized['sequence_number'] = max(0, min(int(seq_num), PacketSecurityValidator.MAX_SEQUENCE_NUMBER))
            else:
            ...
            if isinstance(timestamp, (int, float)):
                ...
                min_time = current_time - PacketSecurityValidator.MAX_PAST_TIMESTAMP_OFFSET
                max_time = current_time + PacketSecurityValidator.MAX_FUTURE_TIMESTAMP_OFFSET
            ...
            if isinstance(data, (bytes, bytearray)):
                ...
                if len(data) > PacketSecurityValidator.MAX_PACKET_SIZE:
                    logger.warning(f"Truncating oversized packet data: {len(data)} bytes")
            ...
            if isinstance(source_id, str):
                ...
                safe_chars = ''.join(c for c in source_id if c.isprintable() and c not in '<>"\'&\x00\n\r')
                sanitized['source_id'] = safe_chars[:PacketSecurityValidator.MAX_SOURCE_ID_LENGTH] or 'unknown'
            else:
            ...
            if isinstance(duration_ms, (int, float)) and 0 < duration_ms <= PacketSecurityValidator.MAX_DURATION_MS:
