...
export class PerformanceProfiler {
  ...
  private addMemorySnapshot(snapshot: { usedJSHeapSize: number; totalJSHeapSize: number; jsHeapSizeLimit: number }): void {
  ...
  private memorySnapshots: { usedJSHeapSize: number; totalJSHeapSize: number; jsHeapSizeLimit: number; timestamp: number }[] = [];
  ...
  profile<T>(name: string, fn: () => T, options: { tags?: string[]; metadata?: Record<string, unknown> } = {}): T {
    ...
    try {
      ...
      if (result instanceof Promise) {
        return result.finally(() => this.endSample(sample.id)) as Promise<T>;
      } else {
  ...
  async profileAsync<T>(name: string, fn: () => Promise<T>, options: { tags?: string[]; metadata?: Record<string, unknown> } = {}): Promise<T> {
    if (!this.options.enabled || !this.shouldSample()) {
      return fn();
    }
  ...
  profileComponent(name: string, props?: Record<string, unknown>): { onRender: (id: string, phase: string, actualDuration: number) => void } {
  ...
  private startSample(name: string, type: PerformanceSample['type'], options: { tags?: string[]; metadata?: Record<string, unknown> } = {}): PerformanceSample {
    ...
    if (this.options.trackMemory && 'memory' in performance) {
      sample.memoryBefore = (performance as Performance & { memory?: { usedJSHeapSize: number } }).memory?.usedJSHeapSize || 0;
    }
  ...
  private endSample(id: string): void {
    ...
    if (this.options.trackMemory && 'memory' in performance) {
      sample.memoryAfter = (performance as Performance & { memory?: { usedJSHeapSize: number } }).memory?.usedJSHeapSize || 0;
    }
  ...
  async benchmark(name: string, fn: () => unknown, options: {
    iterations?: number;
  ...
  private generateRecommendations(slowestOperations: PerformanceSample[], memoryUsage: Record<string, unknown>): string[] {
...
export function useProfiler(name: string, enabled: boolean = true) {
  ...
  const profile = React.useCallback(<T,>(fn: () => T, options?: Record<string, unknown>): T => {
    if (!enabled) return fn();
    return profiler.profile(name, fn, options);
  }, [profiler, name, enabled]);

  const profileAsync = React.useCallback(<T,>(fn: () => Promise<T>, options?: Record<string, unknown>): Promise<T> => {
    if (!enabled) return fn();
    return profiler.profileAsync(name, fn, options);
  }, [profiler, name, enabled]);

  const mark = React.useCallback((markName: string, metadata?: Record<string, unknown>) => {
    if (!enabled) return;
...
export function withProfiler<P extends object>(
  ...
  const ComponentWithProfiler = React.forwardRef<unknown, P>((props, ref) => {
    const profiler = PerformanceProfiler.getInstance();
    const name = profilerName || WrappedComponent.displayName || WrappedComponent.name || 'Component';

    return (
      <React.Profiler id={name} onRender={profiler.profileComponent(name, props).onRender}>
